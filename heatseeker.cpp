...
static char *ms_apszResNames[] = { "3d/Gmissile.sop",
                                   "3d/Gmissile.mesh",
                                   "3d/Gmissile.tex",
                                   "3d/Gmissile.hot",
                                   "3d/Gmissile.bounds",
    ...
    if (!m_sSuspend)
    {
        ...
        switch (m_eState)
        {
            ...
            case CWeapon::State_Fire:
                PlaySample(g_smidRocketFire,     // In:  Sample to play
            ...
            case CWeapon::State_Chase:
            {
                ...
                if (m_dZ > ms_sOffScreenDist + sRealmH || m_dZ < -ms_sOffScreenDist ||
                    m_dX > ms_sOffScreenDist + sRealmW || m_dX < -ms_sOffScreenDist)
                {
                    short sTargetAngle = FindAngleTo(sRealmW / 2, sRealmH / 2);
                ...
                if (sHeight > m_dY || !m_pRealm->IsPathClear( // Returns true, if the entire path is clear.
                ...
                if (m_bArmed)
                {
                    ...
                    if (m_pRealm->m_smashatorium.QuickCheckClosest(&m_smashSeeker,
                                                                   ...
                                                                   m_u32SeekBitsExclude,
                                                                   &pSmashed))
                    // Find the angle to the closest thing
                    {
                        if (m_pRealm->IsPathClear((short)m_dX,
                                                  (short)m_dY,
                                                  (short)m_dZ,
                                                  ms_dLineCheckRate,
                                                  (short)pSmashed->m_sphere.sphere.X,
                                                  (short)pSmashed->m_sphere.sphere.Z))
                        {
...
short CHeatseeker::Preload(CRealm *prealm) // In:  Calling realm.
{
    CAnim3D anim;
    RImage *pimage;
