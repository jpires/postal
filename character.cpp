...
// scene scaling.
////////////////////////////////////////////////////////////////////////////////
inline long GetRandSway( // Returns sway value.
...
void CCharacter::Update() // Returns nothing.
{
    if (m_u16IdWeapon != CIdBank::IdNil)
    {
        ...
        if (m_pRealm->m_idbank.GetThingByID((CThing **)&pweapon, m_u16IdWeapon) == 0)
        {
            ...
            // Position weapon.
            PositionChild(pweapon->GetSprite(), // In:  Child sprite to position.
...
void CCharacter::OnDead()
{
    ...
    m_pRealm->m_scene.DeadRender3D(phood->m_pimBackground, // Destination image.
  ...
  Shot_Message *pshotmsg)   // In:  Message to handle.
{
    ...
    {
        MakeBloody(pshotmsg->sDamage, pshotmsg->sAngle, CHUNKS_DAMAGE_DIR_SWAY);
    }
}
  ...
  Explosion_Message *pexplosionmsg) // In:  Message to handle.
{
    ...
    MakeBloody(pexplosionmsg->sDamage,
...
////////////////////////////////////////////////////////////////////////////////
void CCharacter::MakeBloody(short sDamage,      // In:  Damage to base carnage on.
...
void CCharacter::MakeBloodPool()
{
    ...
    if (pat != nullptr)
    {
        ...
        Map3Dto2D(dHitX, dTerrainH, dHitZ, &(pat->m_msg.msg_DrawBlood.s2dX), &(pat->m_msg.msg_DrawBlood.s2dY));
...
////////////////////////////////////////////////////////////////////////////////
void CCharacter::BloodToBackground(short sAnimX2d, // Position of animation in 2d.
                                   short sAnimY2d) // Position of animation in 2d.
{
    ...
    if (rspGetResource(&g_resmgrGame, m_pRealm->Make2dResPath(BLOOD_POOL_RES_NAME), &paachannel) == 0)
    {
        ...
        if (paa->m_pimAlphaArray != nullptr)
        {
            ...
            // Do the Alpha blit.
            rspGeneralAlphaBlit(m_pRealm->m_phood->m_pmaTransparency, // Levels multialpha table.
  ...
  CSmash::Bits bitsExclude)       // Bits to use for bullet collision
{
    ...
    GetLinkPoint(m_panimCur->m_ptransRigid->GetAtTime(m_lAnimTime), &dWeaponRelX, &dWeaponRelY, &dWeaponRelZ);

    RP3d pt3WeaponRel = { static_cast<float>(dWeaponRelX),
                          static_cast<float>(dWeaponRelY),
                          static_cast<float>(dWeaponRelZ),
                          1 };
    ...
    switch (m_eWeaponType)
    {
        ...
        case CDoubleBarrelID:
            FireBullets(&pt3WeaponRel,
                        2 * NUM_SHOTS_PER_SHELL,
                        MAX_SHELL_RANGE,
                        g_smidDeathWadLaunch,
                        bitsInclude,
                        bitsDontcare,
                        bitsExclude);
  ...
  CSmash::Bits bitsExclude)  // In:  Optional bits excluded from collision
{
    ...
    for (i = 0; i < sNumShots; i++)
    {
        ...
        bool bResult = m_bullets.FireDeluxe(
          ...
          GetRandSway(FIRE_ANGLE_Z_SWAY,
                      m_pRealm->m_phood->m_dScale3d), // In:  Angle of launch in degrees (on X/Y plane).
    ...
    if (Construct(CChunkID, m_pRealm, (CThing **)&pchunk) == 0)
    {
        pchunk->Setup(m_dX + ppt3d->x,                                         // Source position.
  ...
  CSmash *psmashExclude /*= NULL*/) // In:  Optional CSmash to exclude or NULL, if none.
{
    ...
    // Scan while in realm.
    while (fPosX > sMinX && fPosY > sMinY && fPosZ > sMinZ && fPosX < sMaxX && fPosY < sMaxY && fPosZ < sMaxZ &&
           fTotalDistXZ < sRangeXZ)
    {
        ...
        if ((u16Attribute & REALM_ATTR_NOT_WALKABLE) || (sCurH - fPosY > sVerticalTolerance))
        {
            break;
        }
...
// (static).
////////////////////////////////////////////////////////////////////////////////
short CCharacter::Preload(CRealm *prealm) // In:  Calling realm.
{
