...
char *g_pszNetStat_ServerVersionMismatchError_lu_lu =
  "Version mismatch--dropping client (Client ver is %lu -- Our ver is %lu)";
...
char *g_pszNetProb_General = "Network not responding.\nYou can wait or\npress " NET_PROB_GUI_ABORT_KEY_TEXT " to abort";
...
static GuiLink ms_aglServerLinkage[] = {
    {
      GUI_ID_DISCONNECT_PLAYER,
      GuiLink::Gui,
      &ms_pguiDisconnectPlayer,
    },
    //		{ GUI_ID_RESPAWN_PLAYERS,		GuiLink::Short,	&g_GameSettings.m_sHostRejuvenate,	},
    {
      GUI_ID_ENABLE_TIME_LIMIT,
      GuiLink::Bool,
      &ms_bTimeLimit,
    },
    {
      GUI_ID_EDIT_TIME_LIMIT,
      GuiLink::Short,
      &g_GameSettings.m_sHostTimeLimit,
    },
    {
      GUI_ID_ENABLE_KILL_LIMIT,
      GuiLink::Bool,
      &ms_bKillLimit,
    },
    {
      GUI_ID_EDIT_KILL_LIMIT,
      GuiLink::Short,
      &g_GameSettings.m_sHostKillLimit,
    },
    {
      GUI_ID_LEVEL_LISTBOX,
      GuiLink::Gui,
      &ms_plbLevelBrowse,
    },
    {
      GUI_ID_SHOW_COOP_LEVELS,
      GuiLink::Gui,
      &ms_pmbCoopLevels,
    },
    {
      GUI_ID_ENABLE_COOP_MODE,
      GuiLink::Gui,
      &ms_pmbCoopMode,
    },
    {
    ...
    {
      static_cast<long>(TERMINATING_GUI_ID),
    }, // Terminator.
};

static GuiLink ms_aglClientLinkage[] = {
    {
      GUI_ID_OPTIONS_STATIC,
      GuiLink::Gui,
      &ms_pguiOptions,
    },
    {
...
static short FindSpecificSystem(RSocket::Address *paddress); // Out: Address returned here (if successfull)

static short BrowseForSelf(CNetServer *pserver,         // I/O: Server interface
                           RSocket::Address *paddress); // Out: Address returned here (if successfull)
...
//
//////////////////////////////////////////////////////////////////////////////
static void CleanClientDlg(CNetClient *pnet, bool bRetryButton)
{
    if (ms_pguiOptions)
    {
        ...
        if (sRes == 0)
        {
            ...
            switch (type)
            {
                case DLG_CLIENT:
                {
                    ...
                    // Set title text.
                    ms_pguiRoot->SetText("%s [%s]", ms_pguiRoot->m_szText, g_GameSettings.m_szPlayerName);
                    ms_pguiRoot->Compose();
            ...
            // Center.
            ms_pguiRoot->Move(g_pimScreenBuf->m_sWidth / 2 - ms_pguiRoot->m_im.m_sWidth / 2,
  ...
  ...)                     // In:  Optional arguments based on context of pszFrmt.
{
    if (ms_plbNetConsole != NULL)
    {
        ...
        if (pguiConsoleMsg)
        {
            ...
            // these.
            short sOrigTotalBorderThickness = pguiConsoleMsg->GetTopLeftBorderThickness() +
                                              pguiConsoleMsg->GetBottomRightBorderThickness();
            // No lines.
    ...
    if (pmsg->msg.nothing.ucType == NetMsg::ERR)
    {
        switch (pmsg->msg.err.error)
        {
            ...
            case NetMsg::ServerVersionMismatchError:
                ...
                sprintf(pText,
                        g_pszNetStat_ServerVersionMismatchError_lu_lu,
                        pmsg->msg.err.ulParam,
                        CNetMsgr::CurVersionNum & ~CNetMsgr::MacVersionBit);
                break;
            case NetMsg::ClientVersionMismatchError:
                pText = szStaticErrorText;
                sprintf(pText,
                        g_pszNetStat_ClientVersionMismatchError_lu_lu,
                        pmsg->msg.err.ulParam,
                        CNetMsgr::CurVersionNum & ~CNetMsgr::MacVersionBit);
                break;
                                         ...
                                         // pszRealmFileName.
{
    ...
    if (sResult == 0)
    {
        ...
        {
            // Form section name.
            sprintf(szSection, "Realm%sNet%d", bCoopLevel ? "Coop" : "", sRealmNum++);
...
//
//////////////////////////////////////////////////////////////////////////////
static void OnDroppedMsg(CNetClient *pnet, // In:  Network interface.
  ...
  bool bServer)           // In:  true if in server mode; false if client.
{
    ...
    sprintf(szPlayer,
            "%s   (%s)",
            pmsg->msg.joined.acName,
            CGameSettings::ms_apszPlayerColorDescriptions[ucColorIndex]);
...
//
//////////////////////////////////////////////////////////////////////////////
static void OnChatMsg(CNetClient *pnet, // In:  Network interface.
...
//
//////////////////////////////////////////////////////////////////////////////
static void OnSetupGameMsg(CNetClient *pnet, // In:  Network interface.
                           NetMsg *pmsg)     // In:  Message.
{
    ...
    if (ms_pguiOptions)
    {
        ...
        if (pmsg->msg.setupGame.sRealmNum > -1)
        {
            ...
            if (Play_GetRealmInfo(true,                                           // Multiplayer
        ...
        else
        {
            ...
            if (GetRealmFileFromRealmTitle(pmsg->msg.setupGame.sCoopLevels ? true
                                                                           : false, // Cooperative or Deathmatch levels
        ...
        if (bAvailable)
        {
            ...
            ms_pguiOptions->SetText("Host has selected %s%s in %s mode on difficulty %hd with %s and %s.",
                                    szCoop,
                                    szRealm,
                                    szPlayMode,
                                    pmsg->msg.setupGame.sDifficulty,
...
#ifdef ENABLE_PLAY_SPECIFIC_REALMS_ONLY
                sprintf(szTmp,
                        "%s\"%s\" only has the \"" SPECIFIC_MP_REALM_TEXT "\" hood -- please pick it!",
                        g_pszNetStatusMsgPrefix,
                        pnet->GetPlayerName(pnet->GetID()));
                pnet->SendText(szTmp);
#else
                sprintf(szTmp,
                        "%s\"%s\" does not have the selected hood -- please pick another one!",
                        g_pszNetStatusMsgPrefix,
                        pnet->GetPlayerName(pnet->GetID()));
                pnet->SendText(szTmp);
#endif
...
void ProtoNotSupported(void)
{
    ...
    {
        // Only one protocol
        rspMsgBox(RSP_MB_BUT_OK | RSP_MB_ICN_INFO,
                  g_pszAppName,
  ...
  NetMsg *pmsgOut)            // Out: NetMsg::NOTHING or NetMsg::START_GAME
{
    ...
    do
    {
        ...
        if (pserver)
            sResult = pserver->Startup(g_GameSettings.m_usServerPort,
                                       g_GameSettings.m_szHostName,
                                       &NetBlockingCallback);
        if (sResult == 0)
        {
            ...
            if (sResult == 0)
            {
                ...
                sResult = SetupDlg(pserver ? FullPathHD(SERVER_GUI) : FullPathVD(CLIENT_GUI),
                                   pserver ? DLG_SERVER : DLG_CLIENT);
                if (sResult == 0)
                {
                    ...
                    if (sResult == 0)
                    {
                        ...
                        do
                        {
                            ...
                            sResult = pclient->StartJoinProcess(&addressHost,
                                ...
                                while (!(bClientDone && bServerDone && (bAbort || bStart || bRetry)) && !bUserAbortNow)
                                {
                                    ...
                                    switch (action)
                                    {
                                        ...
                                        case DLG_OK:
                                            ...
                                            if (pserver)
                                            {
                                                // Make sure neither OK nor Cancel were pressed, and that we have at
                                                // least 1 client
                                                if (!bStart && !bAbort && pserver->GetNumberOfClients())
                                                {
                                                    ...
                                                    pserver->StartGame(pclient->GetID(),
#ifdef ENABLE_PLAY_SPECIFIC_REALMS_ONLY
                                                                       -1,
                                                                       ...
                                                                       ms_bTimeLimit ? g_GameSettings.m_sHostTimeLimit
                                                                                     : -g_GameSettings.m_sHostTimeLimit,
                                                                       ms_bKillLimit ? g_GameSettings.m_sHostKillLimit
                                                                                     : -g_GameSettings.m_sHostKillLimit,
                                                                       ms_bCoopLevels ? 1 : 0,
                                                                       ms_bCoopMode ? 1 : 0,
                                        ...
                                        case DLG_CANCEL:
                                            ...
                                            else
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    // Start with a clean slate (no RETRY button because user selected
                                                    // ABORT)
                                        ...
                                        // Disconnect player was pressed.  We don't allow this if OK or Cancel were
                                        // pressed.
                                            ...
                                            if (!bStart && !bAbort)
                                            {
                                                ...
                                                if (pguiPlayerSel)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        AddConsoleMsg(false,
                                                                      g_pszNetStat_AttemptToDrop_s,
                                                                      pserver->GetPlayerName(id));
                                        ...
                                        case DLG_RETRY:
                                            if (!bStart && !bAbort)
                                            {
                                                // Clean the slate (without RETRY since they just hit it and may not
                                                // need it again)
                                                CleanClientDlg(pclient, false);
                                            ...
                                            switch (msg.msg.nothing.ucType)
                                            {
                                                case NetMsg::ERR:
                                                    ...
                                                    else if (msg.ucSenderID == pclient->GetID())
                                                    {
                                                        ...
                                                        // local client with the server, so it will have aborted the
                                                        // game.
                                                        AddConsoleMsg(false, NetErrorText(&msg));
                                        ...
                                        else
                                        {
                                            ...
                                            switch (msg.msg.nothing.ucType)
                                            {
                                                ...
                                                case NetMsg::ERR:
                                                    ...
                                                    // Play a sound of badness and clean the slate (and show RETRY
                                                    // button)
                                                    PlaySample(g_smidEmptyWeapon, SampleMaster::UserFeedBack);
                                                ...
                                                case NetMsg::STAT:
                                                    switch (msg.msg.stat.status)
                                                    {
                                                        ...
                                                        case NetMsg::LoginAccepted:
                                                            ...
                                                            // that it realizes that if there's an error on our
                                                            // connection, it will abort the whole game.
                                                            if (pserver)
                                                                pserver->SetLocalClientID(pclient->GetID());
                                                            AddConsoleMsg(false,
                                                                          g_pszClientStat_LoginAccepted_hd,
                                                                          (short)pclient->GetID());
                                                            break;
...
static short FindSpecificSystem(RSocket::Address *paddress) // Out: Address returned here (if successfull)
{
    ...
    sResult = CNetBrowse::LookupHost(g_GameSettings.m_szServerName, g_GameSettings.m_usServerPort, paddress);
    if (sResult == 0)
    {
        // Success!
    }
    else
    {
        rspMsgBox(RSP_MB_BUT_OK | RSP_MB_ICN_INFO,
                  g_pszAppName,
                  "The specified computer ('%s') could not be found.  Please verify that the specified name or address "
                  "is correct.",
                           ...
                           RSocket::Address *paddress) // Out: Address returned here (if successfull)
{
    ...
    if (sResult == 0)
    {
        ...
        while (!bFoundSelf && (rspGetMilliseconds() < lTime))
        {
            ...
            for (CNetBrowse::Hosts::Pointer p = hostsAll.GetHead(); p; p = hostsAll.GetNext(p))
            {
                ...
                if ((strcmp(phost->m_acName, pserver->GetHostName()) == 0) &&
                    (phost->m_lMagic == pserver->GetHostMagic()))
                {
    ...
    if (sResult != 0)
    {
        rspMsgBox(RSP_MB_BUT_OK | RSP_MB_ICN_INFO,
                  g_pszAppName,
                  "This computer's network address could not be determined.  If this is a multi-homed host, you may "
                  "need to disable all but one address.");
    }
