...
void CExplode::Render(void)
{
    ...
    if (pAnim)
    {
        ...
        Map3Dto2D((short)(m_dX + pAnim->m_sX),
                  (short)m_dY,
                  (short)(m_dZ + pAnim->m_sY),
                  &m_sprite.m_sX2,
                  &m_sprite.m_sY2);
  ...
  short sAnim)         // In:  Which animation to use
{
    ...
    m_pRealm->m_smashatorium.QuickCheckReset(&m_smash,
                                             CSmash::Character | CSmash::Misc | CSmash::Barrel | CSmash::Mine |
                                               CSmash::AlmostDead | CSmash::Sentry,
...
short CExplode::Preload(CRealm *prealm) // In:  Calling realm.
{
    ChannelAA *pRes;
