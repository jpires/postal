...
class CBackgroundChange
{
  public:
    ...
    void Activate(RImage *pimCurBack, RImage *pBackgrounds[])
    {
        ...
        if (!m_pimNewBackground)
            m_pimNewBackground = pimCurBack;
  ...
  public:
    ...
    void RenderChunk(short sW, RPrint *pPrint)
    {
        ...
        while (pCur != &m_tTail)
        {
            ...
            if (pCur->m_sColorIndex == 245)
                pPrint->SetColor(pCur->m_sColorIndex, 0, 244);
            else
                pPrint->SetColor(pCur->m_sColorIndex, 0, 243); // until we have comand support

            if (pCur->m_sFontSize)
                pPrint->SetFont(pCur->m_sFontSize);

            short sTabX = pCur->m_sLocalX;
            if (sTabX < 0)
                sTabX += sW; // from the right...
            ...
            switch (pCur->m_eJust)
            {
                ...
                case CTextPhrase::Center:
                    ...
                    pPrint->SetColumn(sTabX - sRadius + wideScreenOffset, 0, sRadius << 1, m_pimCache->m_sHeight);
                    // trick print into thinking it's a new line!
                    pPrint->print(-1, pCur->m_sLocalY, "%s", pCur->m_szText);
                    break;
            }
...
class CScrollMaster
{
  public:
    ...
    ~CScrollMaster()
    {
        ...
        {
            if (m_pimBackgrounds[i])
                g_resmgrShell.Release(m_pimBackgrounds[i]);
    ...
    void Start(RPrint *pPrint)
    {
        ...
        for (i = 0; i < m_sNumBackgrounds; i++)
        {
            if (rspGetResource(&g_resmgrShell, m_szBackgroundNames[i], &m_pimBackgrounds[i]) != SUCCESS)
            {
    ...
    short Update(RPrint *pPrint)
    {
        ...
        m_lCurrentBottomY = long(double(rspGetMilliseconds() - m_lActivationTime) * m_dScrollRate * m_rDisplay.sH /
                                 1000.0);
        m_lCurrentTopY = (m_lCurrentBottomY - m_rDisplay.sH);

        if (m_lCurrentTopY > m_lGlobalHeight)
            return FAILURE; // scroll over!
    ...
    void Render(RImage *pimDst)
    {
        ...
        for (pCur = m_pTopActiveChunk; pCur != m_pBottomActiveChunk->m_pNext; pCur = pCur->m_pNext)
        {
            if (pCur != &m_cTail)
            {
                ...
                if (pCur->m_pimCache)
                {
                    rspBlit(pCur->m_pimCache,
                            pimDst,
                            m_rDisplay.sX,
                            pCur->m_sGlobalTopY - m_lCurrentTopY + m_rDisplay.sY,
                            &m_rDisplay);
                }
            }
        }
...
class CFileTextInput
{
  public:
    RBatch m_bf;
    //-----------------------
    CFileTextInput() { m_pStream = nullptr; };
...
short CFileTextInput::ParseTextInput(FILE *fp)
{
    ...
    while ((pszToken = m_bf.NextToken()) != nullptr)
    {
        ...
        if (!strcmp(pszToken + 1, "tab"))
        {
            ...
            {
                case 'l':
                    eCurJust = pCurPhrase->m_eJust = CTextPhrase::Left;
                    break;
                case 'r':
                    eCurJust = pCurPhrase->m_eJust = CTextPhrase::Right;
                    break;
                case 'c':
                    eCurJust = pCurPhrase->m_eJust = CTextPhrase::Center;
                    break;
            }
            continue;
        }
        ...
        if (!strcmp(pszToken, "scene")) // describe a scene change:
        {
            ...
            if (sOnExit)
                pNew->m_eType = CBackgroundChange::OnExit;
...
short ScrollPage(char *pszBackground, char *pszScrollScript, double dScrollRate, RRect *prWindow)
{
    ...
    */
    ...
    if (lKey || sButtons || sJoyPress || rspGetQuitStatus())
        return FAILURE;
...
short Credits(SampleMasterID *pMusic, char *pszBackground, char *pszCredits)
{
    short sResult = 0;
    ...
    if (sResult == 0)
    {
        ...
        // Show title image.
        rspLockBuffer();
        rspBlit(pimBackground, g_pimScreenBuf, 0, 0, BG_X, BG_Y, pimBackground->m_sWidth, pimBackground->m_sHeight);
        rspUnlockBuffer();
    }
    ...
    if (pMusic)
        psmidMusic = pMusic;
    if (pszBackground)
        strcpy(szBackground, pszBackground);
    if (pszCredits)
        strcpy(szCredits, pszCredits);
    ...
    PlaySample(g_smidDemonLaugh2, SampleMaster::Unspecified, 0, &siLaughter, nullptr, 0, 0, true);

    long lTimeToFade = 15000;
    long lStartTime = rspGetMilliseconds();

    // Copy the palette:
    UCHAR PaletteCopy[256 * 3] = {
        0,
    };
