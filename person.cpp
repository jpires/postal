...
#else
                if (rspMsgBox(RSP_MB_ICN_EXCLAIM | RSP_MB_BUT_YESNO,
                              g_pszAppName,
  ...
  RGuiItem *pgui)            // In: GUI pressed.
{
    ...
    if (rspOpenBox("Choose logic file", szLogicFile, szLogicFile, sizeof(szLogicFile), "lgk.") == 0)
    {
...
short CPerson::EditModify()
{
    ...
    if (pGui)
    {
        ...
        if (pWeaponList && pPersonalityList && pmbtnLogAI && pmbtnShowState && peditLogicFile && pbtnLogicUserBrowse &&
            peditStartBouy && peditEndBouy)
        {
            ...
            for (i = 0; i < NumWeaponTypes; i++)
            {
                if ((i != DeathWad || CStockPile::ms_sEnableDeathWad) &&
                    (i != DoubleBarrel || CStockPile::ms_sEnableDoubleBarrel) && (i != ProximityMine) &&
            ...
            switch (sResult)
            {
                ...
                case 1:                    // OK
                {
                    ...
                    if (sResult == ID_GUI_EDIT_TEXTURES)
                    {
                        ...
                        CTexEdit te;
                        te.DoModal(&m_animStand,
                                   m_pRealm->m_phood->m_pltAmbient,
                                   m_pRealm->m_phood->m_pltSpot,
                                   strFile);
                    }
                    break;
                }
...
SampleMaster::SoundInstance CPerson::PlaySoundWrithing(long *plDuration) // Out:  Duration of sample, if not NULL.
{
    m_siPlaying = 0;
