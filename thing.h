...
class CListNode
{
  ...
  public:
    ...
    // Note:  This function can only be used with a list that has
    // dummy nodes for head and tail.
    void InsertBefore(Node *pn) // In:  Node to insert before.
    {
    ...
    // Note:  This function can only be used with a list that has
    // dummy nodes for head and tail.
    void AddAfter(Node *pn) // In:  Node to add after.
    {
        ASSERT(m_pnNext == NULL && m_pnPrev == NULL);
...
class CThing
{
  ...
  protected:
    // Constructor
    CThing(CRealm *pRealm,  // In:  Pointer to realm
           ClassIDType id); // In:  Class ID
  ...
  public:
    ...
    U16 GetInstanceID(void) { return m_u16InstanceId; }
    ...
    static // Static for your usage pleasure.
      long
      DoGui(                 // Returns ID of item that terminated looping.
  ...
  public:
    ...
    long rspGetMicroseconds(short /*sReset	= FALSE*/)
    {
        ASSERT(0);
        return 0;
    }
