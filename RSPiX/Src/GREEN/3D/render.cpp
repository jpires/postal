...
//
void DrawTri_ZColorFog(UCHAR *pDstOffset,
                       long lDstP,
                       RP3d *p1,
                       RP3d *p2,
                       RP3d *p3,
                       ...
                       short sOffsetY /* = 0*/) // In: 2D offset for pZB.
{
    ...
    pt1.x.frac = pt2.x.frac = pt3.x.frac = USHORT(32768); // offset each by 1/2
    ...
    */
    ...
    if (pv1->y.mod > pv2->y.mod)
        SWAP(pv1, pv2);
    if (pv1->y.mod > pv3->y.mod)
        SWAP(pv1, pv3);
    if (pv2->y.mod > pv3->y.mod)
        SWAP(pv2, pv3);
    ...
    if (y2 + y3 == 0)
        return; // don;t bother drawing horiz line
    ...
    if (y2 == 0) // p1.y == p2.y
    {
        ...
        for (y = y3; y; y--)
        {
            ...
            if (xdel)
                fzinc.val = long(z2.mod - z1.mod) * RInitNum::OneOver[xdel];
    ...
    if (fx2inc <= fx3inc)
    {
        ...
        for (y = y2; y; y--)
        {
            ...
            // if (hzdel) hzinc.val = (z3.val - z2.val) / hzdel;
            if (xdel)
                fzinc.val = ULONG(z3.mod - z2.mod) * RInitNum::OneOver[xdel];
            // if (hzdel) Mul(hzinc,z3.val - z2.val,CInitNum::OneOver[hzdel]);
            //  Assume 2 to 3:
        ...
        if (ybot)
        {
            ...
            */
            ...
            for (y = ybot; y; y--)
            {
                ...
                // if (hzdel) hzinc.val = (z3.val - z2.val) / hzdel;
                if (xdel)
                    fzinc.val = long(z3.mod - z2.mod) * RInitNum::OneOver[xdel];
                // if (hzdel) Mul(hzinc,z3.val - z2.val,CInitNum::OneOver[hzdel]);
...

    {
        ...
        for (y = y2; y; y--)
        {
            ...
            if (xdel)
                fzinc.val = long(z2.mod - z3.mod) * RInitNum::OneOver[xdel];
        ...
        if (ybot)
        {
            ...
            */
            ...
            for (y = ybot; y; y--)
            {
                ...
                // if (hzdel) hzinc.val = (z2.val - z3.val) / hzdel;
                if (xdel)
                    fzinc.val = ULONG(z2.mod - z3.mod) * RInitNum::OneOver[xdel];
                // Full accuracy fxMul!
                // if (hzdel) Mul(hzinc,z2.val - z3.val,CInitNum::OneOver[hzdel]);
                // Assume 2 to 3:
                for (x = x3.mod; x <= x2.mod; x++)
...
//==================================================
// For debugging:
void DrawTri_wire(RImage *pimDst, short sX, short sY, RP3d *p1, RP3d *p2, RP3d *p3, UCHAR ucColor)
{
    rspLine(ucColor, pimDst, sX + short(p1->x), sY + short(p1->y), sX + short(p2->x), sY + short(p2->y));
    rspLine(ucColor, pimDst, sX + short(p1->x), sY + short(p1->y), sX + short(p3->x), sY + short(p3->y));
    rspLine(ucColor, pimDst, sX + short(p3->x), sY + short(p3->y), sX + short(p2->x), sY + short(p2->y));
}
...
// sX and sY are additional offsets into pimDst
//
void DrawTri_ZColor(UCHAR *pDstOffset,
                    long lDstP,
                    RP3d *p1,
                    RP3d *p2,
                    RP3d *p3,
...
//
void DrawTri(UCHAR *pDstOffset, long lDstP, RP3d *p1, RP3d *p2, RP3d *p3, UCHAR ucFlatColor)
{
    //////////////////////////////////////////////////////////////////
    //****************************************************************
    //======================  INITIAL SET UP  ========================
