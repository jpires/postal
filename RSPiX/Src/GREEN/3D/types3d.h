...
class RTexture
{
  ...
  public:
    ...
    void Remap(short sStartIndex, short sNumIndex, UCHAR *pr, UCHAR *pg, UCHAR *pb, long linc);
    ...
    void Unmap(UCHAR *pr, UCHAR *pg, UCHAR *pb, long lInc);

    // Muddy or brighten or darken.  Applies the specified brightness value
    // to every nth color (where n == lInc).
    void Adjust(float fAdjustment, // In:  Adjustment factor (1.0 == same, < 1 == dimmer, > 1 == brighter).
