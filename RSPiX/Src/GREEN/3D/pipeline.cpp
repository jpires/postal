...
short RPipeLine::CreateShadow(short sAngleY, double dTanDeclension, short sBufSize)
{
    ...
    if (sBufSize <= 0) // default case:
    {
        if (m_pimShadowBuf == NULL) // do a default allocation
        {
            if (m_pimClipBuf) // use as reference:
            {
                ...
                m_pimShadowBuf->CreateImage(m_pimClipBuf->m_sWidth * 2, m_pimClipBuf->m_sWidth * 2, RImage::BMP8);
                return SUCCESS;
            }
...
void RPipeLine::TransformShadow(RSop *pPts, RTransform &tObj, short sHeight, short *psOffX, short *psOffY)
{
...
//
void RPipeLine::Render(RImage *pimDst, short sDstX, short sDstY, RMesh *pMesh, UCHAR ucColor) // wire!
{
    long i;
    long v1, v2, v3;
    ...
    for (i = 0; i < pMesh->m_sNum; i++)
    {
        ...
        {
            // Render the sucker!
            DrawTri_wire(pimDst, sDstX, sDstY, ms_pPts + v1, ms_pPts + v2, ms_pPts + v3, ucColor);
        }
        else
        {
...
void RPipeLine::RenderShadow(RImage *pimDst, RMesh *pMesh, UCHAR ucColor)
{
    ...
    for (i = 0; i < pMesh->m_sNum; i++)
    {
        ...
        {
            // Render the sucker!
            DrawTri(pimDst->m_pData, pimDst->m_lPitch, ms_pPts + v1, ms_pPts + v2, ms_pPts + v3, ucColor);
        }
    }
}
...
// Uses the static transformed point buffer.
//
void RPipeLine::Render(RImage *pimDst,
                       short sDstX,
                       short sDstY,
                       RMesh *pMesh,
                       RZBuffer *pZB,
...
//
void RPipeLine::BoundingSphereToScreen(RP3d &ptCenter, RP3d &ptRadius, RTransform &tObj)
{
    ...
    double dModelRadius = sqrt(SQR(ptCenter.x - ptRadius.x) + SQR(ptCenter.y - ptRadius.y) +
