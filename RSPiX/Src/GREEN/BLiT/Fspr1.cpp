...
short GetOverRuns()
{
    return gFSPR1.sOverRun;
}

//----------------  Set Conversion Information  ----------------

void SetConvertToFSPR1(U32 u32TransparentColor,
...
void SetConvertFromFSPR1(U32 u32ForeColor,
                         S16 sTransparent,
...
IMAGELINKLATE(FSPR1, ConvertToFSPR1, ConvertFromFSPR1, LoadFSPR1, SaveFSPR1, NULL, DeleteFSPR1);
...
short ConvertToFSPR1(RImage *pImage)
{
    ...
    if (sY < 0)
        sY = 0;
    if ((sTemp = sX + sW - pImage->m_sWidth) > 0)
        sW -= sTemp;
    if ((sTemp = sY + sH - pImage->m_sHeight) > 0)
        sH -= sTemp;
...
//
void _rspBLiT(UCHAR ucColor, RImage *pimSrc, RImage *pimDst, short sDstX, short sDstY)
{
    ...
    if ((sDstX < 0) || (sDstY < 0) || ((sDstX + pimSrc->m_sWidth) > pimDst->m_sWidth) ||
...
short ConvertFromFSPR1(RImage *pImage)
{
    ...
    {
        rspRect(gFSPR1.u32BackColor, pImage, 0, 0, pImage->m_sWidth, pImage->m_sHeight);
    }
...
//
short rspBlit(ULONG ulForeColor, RImage *pimSrc, RImage *pimDst, short sDstX, short sDstY, const RRect *prDst)
{
#ifdef _DEBUG
    ...
    else
    {
        ...
        if (sClipR > 0)
            sW -= sClipR;                        // positive = clipped
        sClipB = sDstY + sH - pimDst->m_sHeight; // positive = clipped
        if (sClipB > 0)
            sH -= sClipB; // positive = clipped
...
//
short rspBlit(ULONG ulForeColor, // will draw color 0
              RImage *pimSrc,
              RImage *pimDst,
              short sDstX,
              short sDstY,
...
// Make s for an easier transition!
//
short rspBlit(ULONG ulForeColor,
              RImage *pimSrc,
              RImage *pimDst,
              short sDstX,
              short sDstY,
