...
char *RPrint::ScanLine(char *pszInput)
{
    ...
    else // I hit a newline or return but STILL wish to eat whitespace
    {
        ...
        i--;
        while (i && IsWhiteSpace(ms_szLineText[i]))
            i--;
        i++;
    }
...
short RPrint::GetPropCellX(short sChar, short *psX, short *psE, short *psNext, short sFirst)
{
    ...
    if (!sSpecial) // a normal ascii character
    {
        ...
        if (!m_pCurFontSet)
            return -1;
        if (!(m_pCurFontSet->m_ppimCharacters))
            return -1;
        if (!(pimLetter = m_pCurFontSet->m_ppimCharacters[sChar]))
            return -1;
    ...
    if (!sSpecial)
        GetPropEffX(a, a + b, a + b + c, psX, psE, psNext);
...
//
void RPrint::GetPropEffX(short sX, short sE, short sNext, short *psEffX, short *psEffE, short *psEffNext)
{
    ...
    if (m_aAbsEffect[ITALIC] < 0)
        sEffX += m_aAbsEffect[ITALIC];
    else
        sEffE += m_aAbsEffect[ITALIC];

    sEffE += m_aAbsEffect[BOLD];
    sEffNext += m_aAbsEffect[BOLD];

    // ALWAYS POSITIVE EXTENT!
    if (m_aAbsEffect[SHADOW_X] < 0)
        sEffX -= m_aAbsEffect[SHADOW_X];
    else
        sEffE += m_aAbsEffect[SHADOW_X];
...
void RPrint::ResetEffects()
{
    ...
    m_aAbsFlag[TAB] = 1;
    m_aAbsEffect[TAB] = m_sCellH;
    m_aAbsFlag[UP_W] = 1;
    m_aAbsEffect[UP_W] = m_sCellW;
    m_aAbsFlag[UP_H] = 1;
    m_aAbsEffect[UP_H] = m_sCellH;
}
...
short RPrint::SetColumn(short sX, short sY, short sW, short sH)
{
    ...
    if (sW > m_pimDst->m_sWidth)
        sW = m_pimDst->m_sWidth;
    if (sH > m_pimDst->m_sHeight)
        sH = m_pimDst->m_sHeight;

    if (sX + sW > m_pimDst->m_sWidth)
        sW = m_pimDst->m_sWidth - sX;
    if (sY + sH > m_pimDst->m_sWidth)
        sH = m_pimDst->m_sWidth - sY;
...
short RPrint::SetColor(ULONG ulForeColor, ULONG ulBackColor, ULONG ulShadowColor)
{
    ...
    if (ulBackColor)
        m_BackgroundColor = ulBackColor;
    if (ulShadowColor)
        m_ShadowColor = ulShadowColor;
...
{
    if (sOn)
        m_eModes = (Mode)(m_eModes | WORD_WRAP);
    else
        m_eModes = (Mode)(m_eModes & (~WORD_WRAP));
}
