...
//
short RFont::AddLetter(RImage *pimLetter, short sASCII, short sKernL, short sKernR)
{
    ...
    while (pFont)
    {
        if (pFont->m_sCellHeight == pimLetter->m_sHeight)
            break;
        pFont = pFont->m_pNext;
    }
...
short RFont::Load(RFile *pcf)
{
    ...
    while (!pcf->IsEOF())
    {
        ...
        else if (!strcmp(&string[0], "ENDFONT"))
            break; // equal
        else
        {
            TRACE("RFont::Load: Bad font file!\n");
