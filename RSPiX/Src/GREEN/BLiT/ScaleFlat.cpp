...
//
#define LCLIP_PIX(sCount, sShow) if (sCount < sShow)
...
//
#define INIT_PROPER_FRACTION(sLarge, sNumi) sNumi = (sLarge >> 1);
...
//
inline void _Blit_HS_VS_C(UCHAR *pSrc,
                          long lSrcP,
                          short sSrcW,
                          short sSrcH,
                          UCHAR *pDst,
                          ...
                          short sMirrorV)
{
    ...
    if (sMirrorV == -1)
        lDstP = -lDstP; // make sure pDst is correctly positioned
...
// I believe the memptrs are to the UNCLIPPED rects...
//
inline void _Blit_HM_VM_C(UCHAR *pSrc,
                          long lSrcP,
                          short sSrcW,
                          short sSrcH,
                          UCHAR *pDst,
...
// I believe the memptrs are to the UNCLIPPED rects...
//
inline void _Blit_HS_VM_C(UCHAR *pSrc,
                          long lSrcP,
                          short sSrcW,
                          short sSrcH,
                          UCHAR *pDst,
...
// I believe the memptrs are to the UNCLIPPED rects...
//
inline void _Blit_HM_VS_C(UCHAR *pSrc,
                          long lSrcP,
                          short sSrcW,
                          short sSrcH,
                          UCHAR *pDst,
...
//
inline short
_rspBlit(UCHAR *pSrc, long lSrcP, short sSrcW, short sSrcH, UCHAR *pDst, long lDstP, short sDstW, short sDstH)
{
    return 0;
}
...
//
short rspBlitT(RImage *pimSrc,
               RImage *pimDst,
               RRect *prSrc,
               const RRect *prDst,
               const RRect *prDstClip,
               const RRect *prSrcClip)
{
    ...
    sClipL = sDstClipX - sDstX;
    if (sClipL < 0)
        sClipL = 0;
    sClipT = sDstClipY - sDstY;
    if (sClipT < 0)
        sClipT = 0;
    sClipR = (sDstX + sDstW - sDstClipX - sDstClipW);
    if (sClipR < 0)
        sClipR = 0;
    sClipB = (sDstY + sDstH - sDstClipY - sDstClipH);
    if (sClipB < 0)
        sClipB = 0;
...
#endif
  short
  rspBlitT(RImage *pimSrc, RImage *pimDst, short sDstX, short sDstY, short sDstW, short sDstH, const RRect *prDstClip)
{
    RRect rSrc, rDst;
...
#endif
  short
  rspBlitT(RImage *pimSrc, RImage *pimDst, short sDstX, short sDstY, double dRatW, double dRatH, const RRect *prDstClip)
{
#ifdef _DEBUG
    ...
    return rspBlitT(pimSrc,
                    pimDst,
                    sDstX,
                    sDstY,
                    (short)(dRatW * pimSrc->m_sWidth),
                    (short)(dRatH * pimSrc->m_sHeight),
                    prDstClip);
}
