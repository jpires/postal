...
//
void rspGeneralAlphaBlit(RMultiAlpha *pX,
                         RImage *pimMask,
                         RImage *pimSrc,
                         RImage *pimDst,
                         short sDstX,
                         ...
                         RRect &rDstClip)
{
    ...
    if (rspSimpleClip(sSrcX, sSrcY, sDstX, sDstY, sDstW, sDstH, rDstClip.sX, rDstClip.sY, rDstClip.sW, rDstClip.sH) ==
        -1)
        return; // clipped out
...
//
void rspGeneralAlphaBlit(short sLevel,
                         RMultiAlpha *pX,
                         RImage *pimMask,
                         RImage *pimSrc,
                         RImage *pimDst,
...
// This does a pre-dimming of the mask based on sLevel -> 255 = as is!
//
extern void rspGeneralAlphaBlitT(short sLevel,
                                 RMultiAlpha *pX,
                                 RImage *pimMask,
                                 RImage *pimSrc,
                                 RImage *pimDst,
...
//
void rspFastMaskAlphaBlit(UCHAR ***pfaX,
                          RImage *pimMask,
                          RImage *pimSrc,
                          RImage *pimDst,
                          short sDstX,
...
//
void rspFastMaskAlphaBlitT(UCHAR ***pfaX,
                           RImage *pimMask,
                           RImage *pimSrc,
                           RImage *pimDst,
                           short sDstX,
...
//
void rspAlphaBlitT(short sAlphaLevel,
                   RMultiAlpha *pMultiX,
                   RImage *pimSrc,
                   RImage *pimDst,
                   short sDstX,
                   ...
                   RRect *prDst)
{
    ...
    {
        if (rspSimpleClip(sSrcX, sSrcY, sDstX, sDstY, sDstW, sDstH, prDst->sX, prDst->sY, prDst->sW, prDst->sH) == -1)
            return; // clipped out
    }
    ...
    // Must round up!
    if (sAlphaLevel <= short((ucTransparent + 1) >> 1))
        return;
...
// As the name implies, this does not consider source colors of index zero.
//
void rspFastAlphaBlitT(short sAlphaLevel,
                       UCHAR ***pMultiX,
                       RImage *pimSrc,
                       RImage *pimDst,
                       short sDstX,
                       ...
                       RRect *prDst)
{
    ...
    // Test out trivial conditions:
    if (sAlphaLevel <= short(ucTransparent))
        return;
...
void rspMakeMask(RImage *pimSrc, UCHAR ucVal)
{
    ...
    for (j = 0; j < pimSrc->m_sHeight; j++, pSrcLine += lSrcP)
    {
        ...
        {
            if (*pSrc)
                *pSrc = ucVal; // replace all as mask
        }
    }
}
...
void rspCopyAsMask(RImage *pimSrc, RImage *pimDst, UCHAR ucVal)
{
    ...
    for (j = 0; j < sDstH; j++, pSrcLine += lSrcP, pDstLine += lDstP)
    {
        ...
        {
            if (*pSrc)
                *pDst = ucVal; // replace all as mask
            else
                *pDst = 0;
        }
    }
