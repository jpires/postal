...
void rspClipPlot(PIXSIZE color, RImage *pimDst, short sX, short sY, const RRect *prClip)
{
    ...
    if ((sX < rClip.sX) || (sY < rClip.sY) || (sX >= (rClip.sX + rClip.sW)) || (sY >= (rClip.sY + rClip.sH)))
    {
        // Live clipping.
        return;
    }
...
//
template<class PIXSIZE>
short rspLasso(PIXSIZE ignoreColor,
               RImage *pimSrc,
               short &x,
               short &y,
               short &w,
               ...
               int bFlag)
{
    ...
    if (lFlag)
    {
        ...
        for (i = w; i > 0; i--) // w will change!
        {
            ...
            for (j = 0; j < h; j++)
            {
                if ((*pCursor) != ignoreColor)
                    goto DoneL; // break all looping!
                pCursor += lSkipV;
            }
            x++;
            w--;
...
// force instantiation
template short rspLasso<UCHAR>(UCHAR ignoreColor,
                               RImage *pimSrc,
                               short &x,
                               short &y,
                               short &w,
                               ...
                               int tFlag,
                               int bFlag);
template short rspLasso<USHORT>(USHORT ignoreColor,
                                RImage *pimSrc,
                                short &x,
                                short &y,
                                short &w,
                                ...
                                int tFlag,
                                int bFlag);
template short rspLasso<ULONG>(ULONG ignoreColor,
                               RImage *pimSrc,
                               short &x,
                               short &y,
                               short &w,
...
//
template<class PIXSIZE>
inline void _BLiT(PIXSIZE *pSrc, PIXSIZE *pDst, long lSrcPitch, long lDstPitch, short sHeight, short sByteWidth)
{
    union
    {
        PIXSIZE *w;
...
inline void _BLiT_MA(UCHAR *pSrc, UCHAR *pDst, long lSrcPitch, long lDstPitch, short sHeight, short sWidth)
{
#ifdef __ARM_NEON__
    while (sHeight--)
    {
              ...
              const RRect *prSrc)
{
    ...
    if (!ImageIsUncompressed(pimSrc->m_type) || !ImageIsUncompressed(pimDst->m_type))
    {
    ...
    {
        if ((sSrcX < 0) || (sSrcY < 0) || ((sSrcX + sW) > pimSrc->m_sWidth) || ((sSrcY + sH) > pimSrc->m_sHeight))
        {
    ...
    if (pimSrc->m_type == RImage::IMAGE_STUB)
        sBlitTypeSrc = (short)((long)pimSrc->m_pSpecial);
    ...
    short sAlign = sSrcX | sDstX | sW | (short)(ABS(pimSrc->m_lPitch) | ABS(pimDst->m_lPitch));
...
inline void _ClearRect(WORDSIZE color, WORDSIZE *pDst, long lDstPitch, short sHeight, short sByteWidth)
{
    union
    {
        WORDSIZE *w;
              ...
              short sAlign) // sAlign in BYTES
{
    ...
    if ((sX < 0) || (sY < 0) || ((sX + sW) > pimSrc->m_sWidth) || ((sY + sH) > pimSrc->m_sHeight))
    {
    ...
    RImage imDst;
    imDst.CreateImage(sW, sH, pimSrc->m_type, lNewPitch, pimSrc->m_sDepth);
...
//
// Modify current:
short rspPad(RImage *pimSrc,
             short sX,
             short sY, // where to move the old image to
             short sW,
             short sH,     // new width and height
             short sAlign) // sAlign in BYTES
{
#ifdef _DEBUG
    ...
    */
    ...
    else
    {
        ...
        rspBlit(pimSrc, &imDst, rCopy.sX, rCopy.sY, sX + rCopy.sX, sY + rCopy.sY, rCopy.sW, rCopy.sH);
    }
