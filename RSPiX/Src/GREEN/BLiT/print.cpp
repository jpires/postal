...
{
    m_pszFontName = (char *)calloc(1, strlen(pszName) + 1);
...
void RFontOld::AddLetter(RImage *pimLetter)
{
    ...
    { // so char ptrs are NULL:
        ...
        m_sMaxCellHeight = MAX(m_sMaxCellHeight, pimLetter->m_sHeight);
    ...
    else
    {
        ...
        fCurSize->sUnproportionalWidth = MAX(pimLetter->m_sWidth, fCurSize->sUnproportionalWidth);
                ...
                *pCode++ = UCHAR(0xff);
                ...
                if (ucCheck != ucCheckSum)
                {
                    TRACE("ERROR: character %02x, parity error\n"
                          ...
                          (unsigned long)ucCheckSum,
                          (unsigned short)ucCheck);
...
RPrint::RPrint()
{
    ...
    m_fWide = (float)1.0;
    for (short i = 0; i < cgsMAX_FONT_SIZE; i++)
        m_psItalic[i] = 0;
}
...
void RPrint::print(short sX, short sY, RImage *pimTarget)
{
    ...
    m_sCurY = sY;
    if (pimTarget != NULL)
        SetColumn(0, 0, pimTarget->m_sWidth, pimTarget->m_sHeight, pimTarget);
    if (m_sCurX < m_sL)
        m_sCurX = m_sL;
    if (m_sCurY < m_sL)
        m_sCurY = m_sL;
...
void RPrint::GetCell(RImage *pimLetter,
                     short *psH,
...
)
{
    ...
    if (m_sATTRIB & TXT_WIDE)
        w = (short)(m_fWide * w);
    if (psStretchW)
        *psStretchW = w;

    if (m_sATTRIB & TXT_BOLD)
        w += m_sBold;
    if (m_sATTRIB & TXT_ITALIC)
        w += ABS(m_sItalic); // either way adds the same.
    w += m_sSpace;
    if (psCellW)
        *psCellW = w;
}
...
//
void RPrint::PrintLine(short sNumChar, short /*sJustDelta*/, short /*sJustFraction*/, short /*sDen*/)
{
    ...
    for (short i = 0; i < sNumChar; i++)
    {
        ...
        */
        // New...can't clip...
        rspBlit((ULONG)m_clrFGD, pimCur, m_pimTarget, m_sCurX, m_sCurY, sStretchW, sH, m_psItalic);
