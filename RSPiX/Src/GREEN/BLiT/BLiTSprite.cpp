...
short ConvertToFSPR8(RImage *pImage)
{
    ...
    for (i = 0; i < pucCPos - pucOldBuf; i++)
        pHeader->pCBuf[i] = pucOldBuf[i];
    free(pucOldMem);

    // Now update the indexes (pLineArry) which point into PCBuf:
    for (y = 0; y < sH; y++)
        pHeader->pLineArry[y] += (long)(pHeader->pCBuf);
    ...
    for (y = 0; y < sH; y++)
        pHeader->pCtlArry[y] += (long)(pHeader->pControlBlock);
...
short rspBlit(RImage *pimSrc, RImage *pimDst, short sDstX, short sDstY, const RRect *prDst)
{
    ...
    if ((!ImageIsCompressed(pimSrc->m_type)) || (!ImageIsUncompressed(pimDst->m_type)))
    {
    ...
    if (gsScreenLocked || gsBufferLocked)
        goto BLIT_PRELOCKED;
    ...
    if (pimDst->m_type == RImage::IMAGE_STUB)
        sBlitTypeDst = (short)pimDst->m_pSpecial;
