...
//
inline void _BlitRot(short sDeg,
                     short sHeight, // = 2R + 1?
                     UCHAR *pSrcData,
                     long lSrcXP,
                     long lSrcP,
                     ...
                     long lDstP,
                     short sDstW,
                     short sDstH, // Target this for inlining!
                     short sClipL,
                     short sClipR,
                     short sClipT,
                     short sClipB)
...
#define sProperRungY 0
    ...
    for (j = sDstH; j != 0; j--)
    {
        ...
        IMPROPER improper =
          0; // Value doesn't matter since var isn't REALLY used -- just want to avoid unitialized warning
        POSITIVE positive =
          0; // Value doesn't matter since var isn't REALLY used -- just want to avoid unitialized warning
        NEGATIVE negative =
          0; // Value doesn't matter since var isn't REALLY used -- just want to avoid unitialized warning
...
)
{
    ...
    if (sClipT < 0)
        sClipT = 0;
    sClipR = (sDstX + sDstW - sClipX - sClipW);
    if (sClipR < 0)
        sClipR = 0;
    sClipB = (sDstY + sDstH - sClipY - sClipH);
    if (sClipB < 0)
        sClipB = 0;
...
short rspAddRotationPadding(RImage *pimSrc, short sHotX, short sHotY)
{
    ...
    //
    lR = MAX(lR,
             (long)SQR((long)sHotX - (pimSrc->m_sWidth - 1)) +
...
short rspRemovePadding(RImage *pimSrc)
{
    ...
    imDst.CreateImage(pimSrc->m_sWinWidth, pimSrc->m_sWinHeight, pimSrc->m_type, lNewPitch, pimSrc->m_sDepth);

    // 3) Copy the new one in...
    rspBlit(pimSrc, &imDst, pimSrc->m_sWinX, pimSrc->m_sWinY, 0, 0, pimSrc->m_sWinWidth, pimSrc->m_sWinHeight);
