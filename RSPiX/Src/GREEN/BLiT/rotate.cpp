...
short ConvertToROTBUF(RImage *pImage)
{
    ...
    if (gsCX == csFlag)
        gsCX = (pImage->m_sWidth >> 1);
    if (gsCY == csFlag)
        gsCY = (pImage->m_sHeight >> 1);
    ...
    // Create a new buffer:
    imTemp.CreateImage((short)lNewW, (short)lNewW, pImage->m_type, lNewP, pImage->m_sDepth);
    ...
    rspBlit(pImage, &imTemp, (short)0, (short)0, pImage->m_sWinX, pImage->m_sWinY, pImage->m_sWidth, pImage->m_sHeight);
...
inline void initLine(Line1 *pLine, long x1, long y1, long x2, long y2, long d)
{
...
// d = the number of steps from pt 1 to pt 2...
inline void initLine(Line2 *pLine, UCHAR *pBase, long lPitch, long x1, long y1, long x2, long y2, long d)
{
    long lDelX = x2 - x1, lDelY = y2 - y1;
...
inline void incLine(Line1 &Line)
{
    ...
    if (Line.lNumX > Line.lDen)
    {
        Line.lNumX -= Line.lDen;
        Line.lCurX += Line.lPitchX;
    }
    if (Line.lNumY > Line.lDen)
    {
        Line.lNumY -= Line.lDen;
        Line.lCurY += Line.lPitchY;
    }
}
...
inline void incLine(Line2 &Line)
{
    ...
    {
        Line.lNumX -= Line.lDen;
        Line.pCur += Line.lPitchX;
    }
    if (Line.lNumY > Line.lDen)
    {
        Line.lNumY -= Line.lDen;
        Line.pCur += Line.lPitchY;
    }
}
...
//
void _RotateShrink(float fDeg,
                   RImage *pimSrc,
                   RImage *pimDst,
                   short sDstX,
                   short sDstY,
                   short sDstW,
                   short sDstH, // = 2R = lBufferWidth
                   short sFlipCode)
{
#ifdef _DEBUG
    ...
    if ((sClip = gsClipX - sDstX) > 0)
        sClipL = sClip;
    if ((sClip = gsClipY - sDstY) > 0)
        sClipT = sClip;
    if ((sClip = sDstX + sDstW - gsClipX - gsClipW) > 0)
        sClipR = sClip;

    if ((sClip = sDstY + sDstH - gsClipY - gsClipH) > 0)
        sClipB = sClip;
    ...
    if (!sClipL && !sClipR && !sClipT && !sClipB) // not clipped
    {
        ...
        if (sFlipCode)
        {
            if (sFlipCode & PRE_FLIP_H) // HFLIP
                for (i = 0; i < 4; i++)
                    pCorners[i].x = -pCorners[i].x;

            if (sFlipCode & PRE_FLIP_V) // VFLIP
                for (i = 0; i < 4; i++)
                    pCorners[i].y = -pCorners[i].y;
        ...
        */
        ...
        initLine(&l1Left, pCorners[0].x, pCorners[0].y, pCorners[1].x, pCorners[1].y, sDstH); // length of side = 2R

        initLine(&l1Right, pCorners[2].x, pCorners[2].y, pCorners[3].x, pCorners[3].y, sDstH);
                      ...
                      long lStructSize)
{
    ...
    //
    if (!pIm)
        ppBuf.ppI = &(((CStrafe *)pReturnArray)->pImage);
    if (!psHotX)
        pHotX.pL = &((CStrafe *)pReturnArray)->sHotX;
    if (!psHotY)
        pHotY.pL = &((CStrafe *)pReturnArray)->sHotY;
    if (!ppsX)
        ppLinkX.ppL = &((CStrafe *)pReturnArray)->psLinkX;
    if (!ppsY)
        ppLinkY.ppL = &((CStrafe *)pReturnArray)->psLinkY;

    double dCurDeg = dStartDeg;
    // calculate degree increment in default case:
    if (dDegInc == 0.0)
        dDegInc = 360.0 / (double)sNumFrames;
