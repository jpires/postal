    ...
    if (m_sSuspending == FALSE && m_sPauseLevel == 0)
    {
        ...
        do
        {
            if (m_ulAmount == 0L)
            {
                ...
                m_pucData =
                  (UCHAR *)(*m_mcUser)(Data, m_pucData, &m_ulAmount, m_ulUser, &m_ucVolume, &m_ucSecondaryVolume);
            }
                          ...
                          // mixed or 0 for no preference.
{
    ...
    if (ms_sSetMode == FALSE)
    {
        ...
        sRes = rspSetSoundOutMode(lSamplesPerSec,
                                  lDstBitsPerSample,
                                  lNumChannels,
                                  lBufferTime,
                                  lMaxBufferTime,
...
//
//////////////////////////////////////////////////////////////////////////////
short RMix::OpenChannel(long lSampleRate, long lBitsPerSample, long lNumChannels)
{
    short sRes = 0; // Assume success.
...
//
//////////////////////////////////////////////////////////////////////////////
short RMix::Start(RMixCall mcUser, ULONG ulUser, UCHAR ucVolume /* = 255 */, UCHAR ucVol2 /* = 255 */)
{
    short sRes = 0; // Assume success.
...
short RMix::GetMode(        // Returns 0 on success;
  ...
  long *plSrcBitsPerSample) // Bits per sample at which samples must
                            ...
                            // if not NULL.
{
    short sRes = rspGetSoundOutMode(plSamplesPerSec, plDevBitsPerSample, plNumChannels, plBufferTime, plMaxBufferTime);
