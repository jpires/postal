...
class RMixBuf
{
  ...
  public: // Methods.
    ...
    void SetDest(UCHAR *pu8Dst, // In:  Destination buffer.
                 ULONG ulSize); // In:  Size of destination buffer in bytes.
  ...
  public: // Statics.
    ...
    static void SetVolume(UCHAR ucVolume) { ms_ucGlobalVolume = ucVolume; }

    // Reads the current global volume for mixed sounds:
    static UCHAR GetVolume() { return ms_ucGlobalVolume; }
...
class CDVA // a complete dummy
{
  public:
    ...
    {
        return UCHAR(CDVA::ms_asHighByte[DVA_SIZE + (ucLevel >> DVA_SHIFT)][ucByte]);
    }

    // More efficient for a block of data scaled the same:
    inline void ScaleBytes(short sNumBytes, UCHAR *pucBytesIn, UCHAR *pucBytesOut, UCHAR ucLevel)
    {
        ASSERT(pucBytesIn);
        ASSERT(pucBytesOut);
