...
class CRamFlx
{
  public:
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short ReadNextFrame(CImage *pbufRead); // Buffer for frame being read

    // Return the current buffer state
    CImage *GetBuffer(void) { return &m_imagePrev; };
    ...
    short IsPixelsModified(void) { return m_sPixelsModified; };
    ...
    short IsColorsModified(void) { return m_sColorsModified; };
    ...
    static short DoReadFrame(CImage *pbufRead, CNFile *pfile, short *psPixelsModified, short *psColorsModified);
  ...
  private:
    ...
    static short ReadDataColor(CImage *pbufRead, short sDataType, CNFile *pfile, short *psColorsModified);
