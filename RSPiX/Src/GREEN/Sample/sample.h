...
class RSample
{
  public:
    ...
    RSample(void *pData, long lBufSize, long lSamplesPerSec, short sBitsPerSample, short sNumChannels);
    // Destructor.
    ~RSample();
  ...
  public:
    ...
    long GetDuration(void) { return GetTime(m_lBufSize); }

    // Get the time in milliseconds indicated by the specified byte position
    // in the sample.
    long GetTime(long lPos) // In:  Position in bytes.
    {
    ...
    // in the sample.
    long GetPos(long lTime) // In:  Time in milliseconds.
    {
