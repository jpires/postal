...
class RSnd
{
  ...
  public:
    ...
    RSample *GetSample(void) { return m_psample; }

    // Query paused status of current play or stream.
    short IsPaused(void) { return m_mix.IsChannelPaused(); }
  ...
  public:
    ...
    void Pause(void) { m_mix.PauseChannel(); }

    // Resume current play or stream.
    void Resume(void) { m_mix.ResumeChannel(); }
  ...
  protected:
    ...
    // Returns pointer to next buffer to play or NULL to end.
    void *PlayCall(RMix::Msg msg, void *pData, ULONG *pulNewBufSize, UCHAR *pucVolume = NULL, UCHAR *pucVol2 = NULL);
