...
class RPal
{
  ...
  public:
    ...
    short CreatePalette(Type ulNewType);
    ...
    short CreateData(ULONG ulSize,        // Size of data
    ...
    unsigned char *Red(short sStart = 0) // In:  Starting palette entry
    {
    ...
    unsigned char *Green(short sStart = 0) // In:  Starting palette entry
    {
        // Calculate pointer to specified entry
        unsigned char *pucDst = m_pData + ((sStart - m_sStartIndex) * m_sPalEntrySize);
    ...
    unsigned char *Blue(short sStart = 0) // In:  Starting palette entry
    {
        // Calculate pointer to specified entry
        unsigned char *pucDst = m_pData + ((sStart - m_sStartIndex) * m_sPalEntrySize);
    ...
    short GetEntries(short sStart,             // In:  Starting palette entry
    ...
    short SetEntries(short sStart,             // In:  Starting palette entry
                     short sCount,             // In:  Number of entries to do
