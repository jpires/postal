    ...
    switch (pImage->m_type)
    {
        ...
        case RImage::SCREEN8_555:
        {
            ...
            *p555Pal = *pImage->m_pPalette;
            ...
            pImage->m_pPalette->CreateData((p555Pal->m_sStartIndex + p555Pal->m_sNumEntries) *
                                           RPal::GetPalEntrySize(RPal::PDIB));
            pImage->m_pPalette->m_type = RPal::PDIB;
        ...
        case RImage::SCREEN8_565:
        {
            ...
            *p565Pal = *pImage->m_pPalette;
            ...
            // Create new palette data for the pImage
            pImage->m_pPalette->CreateData((p565Pal->m_sStartIndex + p565Pal->m_sNumEntries) *
                                           RPal::GetPalEntrySize(RPal::PDIB));
            pImage->m_pPalette->m_type = RPal::PDIB;
            pImage->m_pPalette->m_sPalEntrySize = RPal::GetPalEntrySize(RPal::PDIB);
...
short ConvertToSCREEN16_555(RImage *pImage)
{
    ...
    switch (pImage->m_type)
    {
        ...
        case RImage::BMP24:
        {
            ...
            for (r = 0; r < height; r++)
                for (c = 0; c < width; c++)
                    usp16[r * dPitch + c] = (USHORT)(((((USHORT)ucp24[r * sPitch + 3 * c + 2]) & 0x00f8) << 7) + // Red
                                                     ((((USHORT)ucp24[r * sPitch + 3 * c + 1]) & 0x00f8)
                                                      << 2) + // Green
...
short ConvertToSCREEN16_565(RImage *pImage)
{
    ...
    switch (pImage->m_type)
    {
        ...
        case RImage::BMP24:
        {
            ...
            for (r = 0; r < height; r++)
                for (c = 0; c < width; c++)
                    usp16[r * dPitch + c] = (USHORT)(((((USHORT)ucp24[r * sPitch + 3 * c + 2]) & 0x00f8) << 8) + // Red
                                                     ((((USHORT)ucp24[r * sPitch + 3 * c + 1]) & 0x00fc)
                                                      << 3) + // Green
                                                     ((((USHORT)ucp24[r * sPitch + 3 * c + 0]) & 0x00f8) >> 3)); // Blue
...
short ConvertToSCREEN32_ARGB(RImage *pImage)
{
    ...
    switch (pImage->m_type)
    {
        ...
        case RImage::BMP24:
        {
            ...
            for (r = 0; r < height; r++)
                for (c = 0; c < width; c++)
                    ulp32[r * dLongPitch + c] = (ULONG)(((ULONG)ucp24[r * sPitch + 3 * c + 2] * 0x10000) +
