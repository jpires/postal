...
//  Static Members:
//////////////////////////////////////////////////////////////////////
short RMultiGrid::ms_sShiftToMask[16] = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383 };
...
// For Debugging
void RMultiGrid::Dump(RImage *pimDst, short sSrcX, short sSrcY, short sDstX, short sDstY, short sW, short sH)
{
    short i, j;
...
void RMultiGrid::DumpTiles(RImage *pimDst)
{
    ...
    for (j = 0; j < sGridH; j++)
    {
        for (i = 0; i < sGridW; i++)
        {
            short sValue = *(m_ppsGridLines[j * (m_sMaskY + 1)] + i);
            if (sValue < 0)
                sNumTiles = MAX(sNumTiles, short(-sValue));
        }
    }
...
short RMultiGrid::Alloc(short sW, short sH)
{
    ...
    if (!(m_psGrid = (short *)calloc(sizeof(short), long(sW) * sH)))
        return FAILURE;
...
short RMultiGrid::AllocGrid(short sScaleW, short sScaleH)
{
    ...
    if (!(m_psTiles = (short *)calloc(lByteTileSize, sMaxNumTiles)))
        return FAILURE;

    //---------------------------------------------Add in the random access:
    short *psTile = m_psTiles;
    if (!(m_ppsTileList = (short **)calloc(sizeof(short *), sMaxNumTiles)))
        return FAILURE;
    for (i = 0; i < sMaxNumTiles; i++, psTile += lShortTileSize)
        m_ppsTileList[i] = psTile;

    short sOff = 0;
    if (!(m_psTileLine = (short *)calloc(sizeof(short), sScaleH)))
        return FAILURE;
    for (i = 0; i < sScaleH; i++, sOff += sScaleW)
        m_psTileLine[i] = sOff;

    //--------------------------------------------- Allocate the coarse grid

    if (!(m_psGrid = (short *)calloc(sizeof(short), long(sGridW) * sGridH)))
        return FAILURE;

    //--------------------------------------------- Add in the random access:

    if (!(m_ppsGridLines = (short **)calloc(sizeof(short *), sFullHeight)))
        return FAILURE;
    for (i = 0; i < sFullHeight; i++)
...
short RMultiGrid::Compress(short sTileW, // Size of tiles to try on this data
                           short sTileH,
...
)
{
    ...
    {
        if (sBlockY == sWholeGridH)
            sScanH = sExtraH;

        short sScanW = m_sMaskX + 1;
        for (sBlockX = 0; sBlockX < sGridW; sBlockX++)
        {
            if (sBlockX == sWholeGridW)
                sScanW = sExtraW;

            sGridVal = *(m_ppsGridLines[sFullY] + sBlockX);
            short *psSrcBlock = psUncompressedData + long(sBlockX) * (m_sMaskX + 1) + long(sFullY) * m_sWidth;
    ...
    for (i = 0; i < sTileNumber; i++, psOldBlocks += sShortSize)
        m_ppsTileList[i] = psOldBlocks;

    if (plNumBlocks)
        *plNumBlocks = --sTileNumber;
    if (plSize)
        *plSize = long(sTileNumber) * (long(sShortSize) * sizeof(short) + sizeof(short *)) +
...
short RMultiGrid::Load(RFile *fp)
{
    ...
    short *psTile = m_psTiles;
    if (!(m_ppsTileList = (short **)calloc(sizeof(short *), sNumTiles)))
        return FAILURE;
    for (i = 0; i < sNumTiles; i++, psTile += sTileLen)
        m_ppsTileList[i] = psTile;
    ...
    short sOff = 0;
    if (!(m_psTileLine = (short *)calloc(sCodeSize, m_sMaskY + 1)))
        return FAILURE;
    for (i = 0; i < m_sMaskY + 1; i++, sOff += m_sMaskX + 1)
        m_psTileLine[i] = sOff;
