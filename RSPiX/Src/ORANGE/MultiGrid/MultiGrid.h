...
class RMultiGrid
{
  public:
    ...
    short GetVal(short sX, short sY, short sClipVal = -1)
    {
        ...
        return *(m_ppsTileList[-sVal] + m_psTileLine[sY & m_sMaskY] + (sX & m_sMaskX));
    }
    ...
    void GetGridDimensions(short *psW, short *psH)
    {
        ...
        *psH = m_sHeight >> m_sShiftY;
        if (m_sWidth & m_sMaskX)
            *psW = *psW + 1; // Partial tiles
        if (m_sHeight & m_sMaskY)
            *psH = *psH + 1; // Partial tiles
    }
    ...
    //
    void Dump(RImage *pimDst, short sSrcX, short sSrcY, short sDstX, short sDstY, short sW, short sH);
    ...
    //
    //////////////////////////////////////////////////////////////////////
    short Compress(short sTileW, // Size of tiles to try on this data
                   short sTileH,
    ...
    {
        if (m_psTiles)
            free(m_psTiles); // This should be freed!
        if (m_psTileLine)
            free(m_psTileLine);
        if (m_ppsGridLines)
            free(m_ppsGridLines);
