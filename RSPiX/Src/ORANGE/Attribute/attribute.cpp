...
USHORT RAttributeMap::GetAttribute(long lX, long lY)
{
    ...
    {
        m_usLastAttribute = m_pusDetailMap[((usBlockData & 0x7fff) * m_sBlockDataSize) +
                                           (((lY % m_sScaleY) * m_sScaleY) + (lX % m_sScaleX))];
...
USHORT RAttributeMap::GetAttribute(long lTopCoord, long lBottomCoord, long lLeftCoord, long lRightCoord)
{
    long lTop = lTopCoord;       // / m_sScaleY;
    long lBottom = lBottomCoord; // / m_sScaleY;
    long lLeft = lLeftCoord;     // / m_sScaleX;
        ...
        for (lCol = lLeft; lCol <= lRight; lCol++)
        {
            ...
            // Lookup the value in the detail map
            {
                usAttrib = m_pusDetailMap[((usAttrib & 0x7fff) * m_sBlockDataSize) +
                                          (((lRow % m_sScaleY) * m_sScaleX) + (lCol % m_sScaleX))];
                usFlags |= usAttrib;
                            ...
                            if (prf->Read(&m_lHeight) == 1)
                            {
                                if (prf->Read(&m_sScaleX) == 1)
                                {
                                    if (prf->Read(&m_sScaleY) == 1)
                                    {
                                        ...
                                        {
                                            if (AllocateMap(m_lWidth * m_lHeight,
                                                            m_sNumDetailMaps * m_sScaleX * m_sScaleY) == SUCCESS)
                                            {
                                                if (prf->Read(m_pusMap, m_lWidth * m_lHeight) ==
                                                    (long)(m_lWidth * m_lHeight))
                                                {
                                                    if (prf->Read(m_pusDetailMap,
                                                                  m_sNumDetailMaps * m_sScaleX * m_sScaleY) ==
                                                        m_sNumDetailMaps * m_sScaleX * m_sScaleY)
                                                    {
                                                        sReturn = SUCCESS;
