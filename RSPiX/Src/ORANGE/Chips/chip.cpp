...
short CChip::Update(void)
{
    ...
    if (m_sSliding == TRUE)
    {
        ...
        if ((short)m_fX == m_sDestX && (short)m_fY == m_sDestY && (short)m_fZ == m_sDestZ)
        {
            // Done.
            m_sSliding = FALSE;
            // Stack if necessary.
...
CChip *CChip::IsColliding(short sTop /*	= FALSE*/)
{
    ...
    for (short i = 0; i < 2 && (pchip == NULL || sTop == TRUE); i++)
    {
        ...
        pchip = (i == 0 ? ms_slistChips.GetPrev(this) : ms_slistChips.GetNext(this));
        ...
        while (pchip != NULL && pchip != this)
        {
            ...
            else
            {
                ...
                if (pchip->m_fX + pchip->m_pimChip->m_sWidth < m_fX || pchip->m_fX > m_fX + (float)m_pimChip->m_sWidth)
                {
                    // Outside.
                }
                else
                ...
                pchip = (i == 0 ? ms_slistChips.GetPrev() : ms_slistChips.GetNext());
            }
        }
    }
...
CChip *CChip::GetChipIn(long lX, long lY, long lW, long lH)
{
    ...
    while (pchip != NULL)
    {
        if ((long)pchip->m_fX < lX || (long)pchip->m_fX > lX + lW || (long)pchip->m_fY < lY ||
