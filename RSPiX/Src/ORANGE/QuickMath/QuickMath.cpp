...
short rspATan(short sDeltaY, short sDeltaX)
{
    ...
    if (sDelY <= sDelX)
        if (sDelX != 0)
            sDeg = ATANQ[long(0.5 + (rspRadToDeg * sDelY) / sDelX)];
        else
            sDeg = 90;
    else if (sDelY)
        sDeg = 90 - ATANQ[long(0.5 + (rspRadToDeg * sDelX) / sDelY)];
    else
        sDeg = 90;
...
short rspATan(double dVal)
{
    ...
    if (dAbsVal <= 1.0)
        sDeg = ATANQ[long(0.5 + rspRadToDeg * dAbsVal)];
    else
        sDeg = 90 - ATANQ[long(0.5 + rspRadToDeg / dAbsVal)];
...
void InitTrig()
{
    ...
    for (i = 0; i < 58; i++)
    {
        ATANQ[i] = short(0.5 + atan(rspDegToRad * i) * rspRadToDeg);
    }
