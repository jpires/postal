...
class RBatch
{
  ...
  public:
    ...
    void init(char *pszFileName,
              char *pszSeparators = " \t,",
              char *pszSpecialCharacters = NULL,
              char cString = '"',
              char cComment = '*',
              short sCharEOL = 1,
              short sLowFilter = 32,
              short sHighFilter = 128)
    {
        clear();
        ...
        if (pszSeparators)
            strcpy(m_pszSeparators, pszSeparators);
    }

    void configure(char *pszSeparators = " \t,",
                   char *pszSpecialCharacters = NULL,
                   char cString = '"',
                   char cComment = '*',
                   short sCharEOL = 1,
