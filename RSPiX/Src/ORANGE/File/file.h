...
class RFile
{
  ...
  public:
    ...
    long Read(char *pcData, long lNum) { return Read((void *)pcData, lNum); }
    ...
    long Read(RPixel32 *ppix32Data, long lNum = 1L) { return Read((U32 *)ppix32Data, lNum); }
  ...
  public:
    ...
    long Write(const char *pcData, long lNum) { return Write((void *)pcData, lNum); }
    ...
    long Write(const RPixel32 *ppix32Data, long lNum = 1L) { return Write((U32 *)ppix32Data, lNum); }
    ...
    // Writes one char.
    long Write(char cVal) { return Write(&cVal, 1); }

    // Writes one U8.
    long Write(U8 u8Val) { return Write(&u8Val); }

    // Writes one S8.
    long Write(S8 s8Val) { return Write(&s8Val); }

    // Writes one U16.
    long Write(U16 u16Val) { return Write(&u16Val); }

    // Writes one S16.
    long Write(S16 s16Val) { return Write(&s16Val); }

    // Writes one RPixel24.
    long Write(RPixel24 pix24Val) { return Write(&pix24Val); }

    // Writes one U32.
    long Write(U32 u32Val) { return Write(&u32Val); }

    // Writes one S32.
    long Write(S32 s32Val) { return Write(&s32Val); }

    // Writes one RPixel32.
    long Write(RPixel32 pix32Val) { return Write(&pix32Val); }

    // Writes one float.
    long Write(float fVal) { return Write(&fVal); }

    // Writes one U64.
    long Write(U64 u64Val) { return Write(&u64Val); }

    // Writes one S64.
    long Write(S64 s64Val) { return Write(&s64Val); }

    // Writes one double.
    long Write(double dVal) { return Write(&dVal); }
  ...
  public: // Querries.
    ...
    short GetEndian(void) { return m_endian; }
    ...
    UCHAR *GetMemory(void) { return m_pucFile; }

    //////////////////////////////////////////////////////////////////////////////
  protected: // Functions for my friends.
    // Sets the instantiable user value.
    void SetUserVal(long lUser) { m_lUser = lUser; }

    // Gets the instantiable user value.
    long GetUserVal(void) { return m_lUser; }
...
#ifdef _DEBUG
short RFileEZLoadDebug(char *FILE_MACRO,
                       long LINE_MACRO,
#else
short RFileEZLoad(
#endif
...
template<class ClassType>
#ifdef _DEBUG
short RFileEZSaveDebug(char *FILE_MACRO,
                       long LINE_MACRO,
#else
short RFileEZSave(
#endif
...
#define RFILE_INSTANTIATE_ANYLOAD(type)                                                                                \
    inline short rspAnyLoad(              /* Returns 0 on success.*/                                                   \
...
#define RFILE_INSTANTIATE_ANYSAVE(type)                                                                                \
    inline short rspAnySave(              /* Returns 0 on success.*/                                                   \
