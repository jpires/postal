    ...
    if (!initialized)
    {
        ...
        else
        {
            ...
            if (lib != NULL)
            {
                ...
                {
                    WCHAR path[MAX_PATH];
                    if (SUCCEEDED(pSHGetFolderPathW(NULL,
                                                    0x001a /*CSIDL_APPDATA*/ | 0x8000 /*CSIDL_FLAG_CREATE*/,
                                                    NULL,
                                                    0,
                                                    path)))
                    {
                        ...
                        snprintf(prefpath,
                                 sizeof(prefpath),
                                 "%S\\RunningWithScissors\\Postal Plus",
                                 (const wchar_t *)path);
                        mkdir(prefpath);
                        snprintf(prefpath,
                                 sizeof(prefpath),
...
// Success.
            ...
            {
#ifdef FILE_VERBOSE
                TRACE("Open(\"%s\", \"%s\", %s): Unable to open file.\n", pszFileName, pszFlags, ms_apszEndian[endian]);
#endif // FILE_VERBOSE
                sRes = -1;
            }
        }
...
//
//////////////////////////////////////////////////////////////////////////////
template<class TYPE> // Type for storing and overflow checking.
inline               // Speed.
  long
  ReadASCII(      // Returns number of complete TYPE items successfully read
                  // and stored.
...
inline               // Speed.
  long
  WriteASCII(      // Returns number of complete TYPE items successfully
                   // written.
  ...
  long lNum)         // In:  Number of 16 bit items to write.
{
    ...
    else
    {
        ...
        for (lSrc = 0L; lSrc < lNum && lWritten >= 0L; lRes += lWritten / sizeof(U16))
        {
            ...
            for (lDst = 0L; lSrc < lNum && lDst < sizeof(ms_au8SwapBuf);
