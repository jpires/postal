...
void RProfile::StartProfile(char *pszFieldName)
{
    ...
    //*****************************************************************************
    if (!m_sActive)
        return; // Do not activate if in error state!
    ...
    if (m_sCurDepth > m_sMaxDepth)
        m_sMaxDepth = m_sCurDepth;
...
void RProfile::Report()
{
    ...
    if (!m_lDeceasedTime) // degenerate report at end:
    {
        ...
        if (!gsReportNumber)
        {
            ...
            fprintf(fp,
                    "SYMBOL KEY:  '@' - refers to the time at which something occurred relative to the START of the "
                    "REPORT.\n");
        ...
        if (m_lCount) // safety
            fprintf(fp,
                    ...
                    double(m_lTotTime) / 1000.0,
                    long(m_lCount),
                    double(m_lTotTime) / double(m_lCount * S64(1000)));
        ...
        if (m_sInternalError || m_sCommandError)
        {
            ...
            {
                fprintf(fp,
                        "Internal memory exceeded - the profiler was designed to only\n"
            ...
            fprintf(fp,
                    "You have made a command typo.  You cannot nest timing runs of the \n"
        ...
        for (i = 0; i < m_sNumTracked; i++)
        {
            if (m_aList[i].m_lNumCalls) // safety
            {
                fprintf(fp, "-------------------------------------------------------\n%s:\n", m_aList[i].m_szFieldName);
                fprintf(fp,
                        "				# of passes = %ld, Tot(ms) = %g, Avg(ms) = %g",
                ...
                fprintf(fp,
                        "				MIN(ms): %+8.6g  @+(s): %+8.6g\n",
                        double(m_aList[i].m_lMinTime) / 1000.0,
                        double(m_aList[i].m_lWhenMin) / 1000000.0);
                fprintf(fp,
                        "				MAX(ms): %+8.6g  @+(s): %+8.6g\n",
                        double(m_aList[i].m_lMaxTime) / 1000.0,
                        double(m_aList[i].m_lWhenMax) / 1000000.0);
            }
        }
        fprintf(fp, "-------------------------------------------------------\n");
        //=======================================================================================
...
RProfile::RProfile()
{
    ...
    m_lCount = 0;
    m_lTotTime = m_lFirstTime = m_lBeginTime = m_lDeceasedTime = m_lFastTimeIn = m_lFastTimeOut = 0;
