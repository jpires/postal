...
U8 RAlpha::ms_red[256] = {
    0,
};
U8 RAlpha::ms_green[256] = {
    0,
};
U8 RAlpha::ms_blue[256] = {
    0,
};
...
UCHAR rspMatchColorRGB(long r, long g, long b, short sStart, short sNum, UCHAR *pr, UCHAR *pg, UCHAR *pb, long linc)
{
    long lMatch = 0, i;
        ...
        while (sIndex < m_sAlphaDepth)
        {
            if ((ms_f[sIndex] & sChannel) != 0)
            {
                if (sIndex - sBaseIndex > 1)
                {
                    ...
                    double dDelta = double(pucChannel[sIndex] - pucChannel[sBaseIndex]) / double(sIndex - sBaseIndex);
...
short RAlpha::CreateAlphaRGB(double dOpacity, short sPalStart, short sPalLen)
{
    ...
    for (s = 0; s < 256; s++)
    {
        for (d = 0; d < 256; d++)
        {
            ...
            m_pAlphas[s][d] = (U8)
              rspMatchColorRGB(r.mod, g.mod, b.mod, sPalStart, sPalLen, ms_red, ms_green, ms_blue, 1);
        }
    }
...
short RAlpha::CreateLightEffectRGB(UCHAR *pa,
                                   UCHAR *pr,
                                   UCHAR *pg,
                                   UCHAR *pb,
                                   long linc,
...
//
short RMultiAlpha::CreateLayer(short sLayerNumber, double dOpacity, short sPalStart, short sPalLen)
{
...
short RMultiAlpha::Finish(short sGeneral)
{
    ...
    else // spread out the values from 0 to 255:
    {
        ...
        if (m_pSaveLevels[255] == 0)
            m_pSaveLevels[255] = m_sNumLevels + 1;
        ...
        while (sIndex < 256)
        {
            if (m_pSaveLevels[sIndex] != 0)
            {
                ...
                {
                    // fill in inbetween stuff
                    double dDelta = double(m_pSaveLevels[sIndex] - m_pSaveLevels[sBaseIndex]) /
                                    double(sIndex - sBaseIndex);
...
///////////////////////////////////////////////////////////////////////////
UCHAR ***RMultiAlpha::pppucCreateFastMultiAlpha(short sStartSrc,
                                                short sNumSrc, // color indices
                                                short sStartDst,
                                                short sNumDst,
                                                long *plAlignedSize)
{
...
///////////////////////////////////////////////////////////////////////////
short RMultiAlpha::QueryFastMultiAlpha(short sNumSrcCol,
                                       short sNumDstCol,
                                       long lTotMem,
                                       long *plHeaderSize,
                                       long *plDataSize)
{
    ...
    if (plHeaderSize)
        *plHeaderSize = lHeaderPerLev * sNumLev + lHeaderMem;
    if (plDataSize)
        *plDataSize = lDataPerLev * sNumLev;
...
void RFastMultiAlphaWrapper::Clear()
{
    ...
    m_sStartSrc = m_sStartDst = m_sNumSrc = m_sNumDst = m_sNumLayers = 0;
...
short RFastMultiAlphaWrapper::Attach(UCHAR ***pppucFMA,
                                     short sStartSrc,
                                     short sNumSrc,
                                     short sStartDst,
                                     short sNumDst,
...
short RFastMultiAlphaWrapper::IsSrcValid(RImage *pimSrc)
{
    ...
    for (j = pimSrc->m_sHeight; j; j--, pDstLine += lP)
    {
        ...
        {
            UCHAR ucPix = *pDst;
            if ((ucPix < sStartSrc) || (ucPix > sLastSrc))
                return FALSE;
        }
    }
