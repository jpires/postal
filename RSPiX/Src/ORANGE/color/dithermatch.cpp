...
//==============================================
//
short rspDither(RImage *pimSrc,  // MUST BE 24-bit!
...
)
{
    ...
    if (pimDst->CreateImage(pimSrc->m_sWidth, pimSrc->m_sHeight, RImage::BMP8))
    {
    ...
    for (j = sH; j; j--)
    {
        for (i = 0; i < sW; i++, pSrc += lSrcXP, pDst += sDirection, sErrPos += sDirection)
        {
            ...
            UCHAR ucIndex = rspMatchColorRGB(lRed, lGreen, lBlue, sStartMap, sNumMap, pRed, pGreen, pBlue, lInc);
            *pDst = ucIndex;
            ...
            if (sCurErrorR < -csClipValue)
                sCurErrorR = -csClipValue;
            if (sCurErrorR > +csClipValue)
                sCurErrorR = csClipValue;

            if (sCurErrorG < -csClipValue)
                sCurErrorG = -csClipValue;
            if (sCurErrorG > +csClipValue)
                sCurErrorG = csClipValue;

            if (sCurErrorB < -csClipValue)
                sCurErrorB = -csClipValue;
            if (sCurErrorB > +csClipValue)
                sCurErrorB = csClipValue;
...
//==============================================
//
short rspSimpleMap(RImage *pimSrc,  // MUST BE 24-bit!
                   RImage *pimDst,  // MUST be 8-bit
                   short sStartMap, // palette index
                   short sNumMap,   // # of colors
                   UCHAR *pRed,     // Palette to match to
...
//==============================================
//
short rspDither(long lBackR, // Don't dither to this color!
                long lBackG,
                long lBackB,
...
//==============================================
//
short rspDither(UCHAR ucForeAlpha, // lower limit for foreground
                UCHAR ucBack,      // index to make BKGD
...
//==============================================
//
short rspSimpleMap(UCHAR ucForeAlpha, // alpha threshhold
...
)
{
    ...
    for (j = pimSrc->m_sHeight; j; j--, pSrcLine += lSrcP, pDstLine += lDstP)
    {
        ...
        for (i = 0; i < pimSrc->m_sWidth; i++, pSrc += lSrcXP, pDst++)
        {
            ...
            {
                ucIndex = rspMatchColorRGB(lRed, lGreen, lBlue, sStartMap, sNumMap, pRed, pGreen, pBlue, lInc);
                *pDst = ucIndex;
            }
        }
        // Give progress feedback
