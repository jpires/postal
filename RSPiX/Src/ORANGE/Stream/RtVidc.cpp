...
//
//////////////////////////////////////////////////////////////////////////////
short CRtVidc::DecompressFrame(PVIDC_RT_HDR pvidchdr, CNFile *pfile, ULONG ulFlags, PBMI pbmiIn, PBMI pbmiOut)

{
    ...
    //		== ICERR_OK)
    HBITMAP hbm = (HBITMAP)ICImageDecompress(pvidchdr->hic,
                                             0L,
                                             (BITMAPINFO *)pbmiIn,
    ...
    if (hbm != NULL)
    {
        ...
        if (hDC != NULL)
        {
            ...
            if (GetDIBits(hDC,
                          hbm,
                          0,
                          pvidchdr->pImage->lHeight,
                          pvidchdr->pImage->pData,
                          (BITMAPINFO *)&pbmiOut,
                          DIB_RGB_COLORS) != FALSE)
            {
                // Success.
            }
            else
...
//
//////////////////////////////////////////////////////////////////////////////
short CRtVidc::Use(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime)
{
    ...
    else
    {
        ...
        if (pvidchdr->pImage != NULL)
        {
            ...
            bmiOut.bmiHeader.biSizeImage = (pvidchdr->pImage->lPitch * bmiOut.bmiHeader.biHeight *
            ...
            {
                pvidchdr->hic = ICLocate(ICTYPE_VIDEO,
                                         pvidchdr->ulFCCHandler,
                                         &bmiIn.bmiHeader,
                                         &bmiOut.bmiHeader,
                                         ICMODE_FASTDECOMPRESS);
            ...
            // Test.
            ASSERT(ICDecompressQuery(pvidchdr->hic, (BITMAPINFO *)&bmiIn, (BITMAPINFO *)&bmiOut) == ICERR_OK);
            ...
            if (DecompressFrame(pvidchdr, &file, ulFlags, &bmiIn, &bmiOut) == 0)
            {
                ...
                if (ICDecompressGetPalette(pvidchdr->hic, (BITMAPINFOHEADER *)&bmiIn, (BITMAPINFOHEADER *)&bmiOut) ==
                    ICERR_OK)
                {
                    if (ucFlags & VIDC_CHUNK_PALETTE)
                    {
                        ...
                        memcpy(pvidchdr->pImage->pPalette->pData,
                               pal.pData +
                                 pvidchdr->pImage->pPalette->sStartIndex * pvidchdr->pImage->pPalette->sPalEntrySize,
                               pvidchdr->pImage->pPalette->sNumEntries * pvidchdr->pImage->pPalette->sPalEntrySize);
...
//
//////////////////////////////////////////////////////////////////////////////
short CRtVidc::UseStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pRtVidc)
{
