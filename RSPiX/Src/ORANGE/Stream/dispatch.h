...
typedef UCHAR *(*ALLOC_DISPATCHFUNC)(long lSize, USHORT usType, UCHAR ucFlags, long lUser);
...
typedef void (*FREE_DISPATCHFUNC)(UCHAR *puc, USHORT usType, UCHAR ucFlags, long lUser);
...
typedef short (*USE_DISPATCHFUNC)(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long lUser);
...
class CDispatch
{
  ...
  public: // Methods.
    ...
    // Returns 0 on success.
    short AddItem(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime);

  public: // Querries.
    // Returns the time from the override function if set or, if not set, from
    // Blue.
    long GetTime(void) { return (m_fnTime != NULL ? (*m_fnTime)(m_lTimeUser) : Blu_GetTime()); }
    ...
    short IsEmpty(void) { return m_slistRtItems.IsEmpty(); }
  ...
  protected: // Internal methods.
    ...
    void UseCall(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime);
    // Callback dispatcher (calls the implied this version).
    static void UseCallStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pDispatch);

    // Handles alloc callbacks from filter.
    UCHAR *AllocCall(long lSize, USHORT usType, UCHAR ucFlags);
    // Callback dispatcher (calls the implied this version).
    static UCHAR *AllocCallStatic(long lSize, USHORT usType, UCHAR ucFlags, long l_pDispatch);

    // Handles free callbacks from filter.
    void FreeCall(UCHAR *puc, USHORT usType, UCHAR ucFlags);
    // Callback dispatcher (calls the implied this version).
    static void FreeCallStatic(UCHAR *puc, USHORT usType, UCHAR ucFlags, long l_pDispatch);
