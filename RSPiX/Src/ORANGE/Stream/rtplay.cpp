...
//
//////////////////////////////////////////////////////////////////////////////
short CRtPlay::RtInfoCall(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime)
{
    short sError = 0;
...
//
//////////////////////////////////////////////////////////////////////////////
short CRtPlay::RtInfoCallStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pRtPlay)
{
    ...
    if (m_usState != usState)
    {
        ...
        else
        {
            TRACE("SetState(): Attempt to change to %s state rejected by %d "
                  "handlers!\n",
                  (usState == RT_STATE_STOPPED
                     ? "STOPPED"
                     : (usState == RT_STATE_STARTING
                          ? "STARTING"
                          : (usState == RT_STATE_BEGIN
                               ? "BEGIN"
                               : (usState == RT_STATE_PLAYING
                                    ? "PLAYING"
                                    : (usState == RT_STATE_PAUSE
                                         ? "PAUSE"
                                         : (usState == RT_STATE_ABORTING
                                              ? "ABORTING"
...
short CRtPlay::Open(char *pszFileName)
{
    ...
    if (Close() == 0)
    {
        ...
        if (m_filewin.Open(pszFileName) == 0)
        {
            ...
            if (m_filewin.SetSize(m_lWindowSize, m_lInputPaneSize, m_lFilterPaneSize) == 0)
            {
                // Successfully opened.
            }
            else
