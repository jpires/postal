...
class CRtVidc
{
  ...
  protected: // Internal methods.
    ...
    short DecompressFrame(PVIDC_RT_HDR pvidchdr, CNFile *pfile, ULONG ulFlags, PBMI pbmiIn, PBMI pbmiOut);

    // Use handler for RtVidc buffers.
    // Returns RET_FREE if done with data on return, RET_DONTFREE otherwise.
    short Use(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime);
    // Static entry point for above.
    static short UseStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pRtVidc);
