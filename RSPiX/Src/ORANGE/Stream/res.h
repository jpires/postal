...
class CRes
{
  ...
  public: // Methods.
    ...
    void SetFreeOnClose(short sFreeOnClose) { m_sFreeOnClose = sFreeOnClose; }
  ...
  protected: // Internal methods.
    ...
    short UseCall(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime);
    // Callback dispatcher (calls the implied this version).
    static short UseCallStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pRes);

    // Handles alloc callbacks from dispatch.
    UCHAR *AllocCall(long lSize, USHORT usType, UCHAR ucFlags);
    // Callback dispatcher (calls the implied this version).
    static UCHAR *AllocCallStatic(long lSize, USHORT usType, UCHAR ucFlags, long l_pRes);

    // Handles free callbacks from filter.
    void FreeCall(UCHAR *puc, USHORT usType, UCHAR ucFlags);
    // Callback dispatcher (calls the implied this version).
    static void FreeCallStatic(UCHAR *puc, USHORT usType, UCHAR ucFlags, long l_pRes);

    // Hooks calls to CNFile's file Open (NOT memory opens).
    // Returns 0 if file found.
    static short FileOpenHook(CNFile *pfile, char *pszFileName, char *pszFlags, short sEndian, long lUser);
