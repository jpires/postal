...
#define RT_STATE_STARTING                                                                                              \
    0x0001                       // Preparing to stream (building up enough
...
#define RT_MSG_STOPPED                                                                                                 \
    0x0000 // The last thing a message handler will
           // receive from a stream session, indicat-
           // ing that streaming is done completely.
#define RT_MSG_STARTING                                                                                                \
    0x0001 // The first thing a message handler will
           // receive indicating we are preparing to
           // stream.
#define RT_MSG_BEGIN                                                                                                   \
    0x0002                     // This let's the handler know it's time
...
class CRtPlay
{
  ...
  public: // Methods.
    ...
    void SetTimeFunc(RTTIMEFUNC fnTime) { m_rttime.SetTimeFunc(fnTime); }

    // Sets the channels to play.
    void SetChannels(USHORT usFilter) { m_filter.SetFilter(usFilter); }

    // Sets the window and panes sizes for the file window (which is set
    // imeediately after opening).
    void SetSizes(long lWindowSize, long lInputPaneSize, long lFilterPaneSize)
    {
  ...
  public: // Querries.
    // Get current status.
    USHORT GetState(void) { return m_usState; }
  ...
  protected: // Internal methods.
    ...
    short RtInfoCall(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime);
    // Callback dispatcher (calls the implied this version).
    // Returns RET_FREE if puc should be freed and RET_DONTFREE, otherwise.
    static short RtInfoCallStatic(UCHAR *puc, long lSize, USHORT usType, UCHAR ucFlags, long lTime, long l_pRtPlay);

    // Handles current state.  Called by Blue's critical handler list.
    void Critical(void);
    // Static version of above.
    static void CriticalStatic(ULONG ul_pRtPlay) { ((CRtPlay *)ul_pRtPlay)->Critical(); }
