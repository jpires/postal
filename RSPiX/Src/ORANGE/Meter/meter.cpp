                             ...
                             // internal m_im.
{
    ...
    if (m_guiMeter.Create(sMeterX, sMeterY, sMeterW, sMeterH, m_im.m_sDepth) == 0)
    {
        // Set hot area to entire button.
        m_guiMeter.GetClient(&(m_guiMeter.m_hot.m_sX),
        ...
        if (lTime >= m_lNextUpdate)
        {
            ...
            switch (m_dtType)
            {
                ...
                case Needle:
                    ...
                    // Draw min.
                    Rectangle(m_u32Overflow, pimDst, sMeterX + sMeterMin, sMeterY, 1, sMeterH);
                    // Draw avg.
                    Rectangle(m_u32Needle, pimDst, sMeterX + sMeterVal, sMeterY, 1, sMeterH);
                    // Draw max.
                    Rectangle(m_u32Overflow, pimDst, sMeterX + sMeterMax, sMeterY, 1, sMeterH);
                    break;
                case Bar:
                    // Compute adaptor.
                    fAdaptor = (float)sMeterW / (float)(m_lMax - m_lMin);
                    ...
                    sMeterMax = (short)((float)m_lMaxValue * fAdaptor);
                    // Draw min.
                    Rectangle(m_u32Overflow, pimDst, sMeterX, sMeterY, sMeterMin, sMeterH);
                    // Draw avg.
                    Rectangle(m_u32Needle, pimDst, sMeterX + sMeterMin, sMeterY, sMeterVal - sMeterMin, sMeterH);
                    // Draw max.
                    Rectangle(m_u32Overflow, pimDst, sMeterX + sMeterVal, sMeterY, sMeterMax - sMeterVal, sMeterH);
                    break;
                case Histogram:
                {
                    // Compute adaptor.
                    ...
                    for (l = 0; l < (METER_HISTOGRAM_HISTORY - 1); l++, sBarPos += sBarWidth)
                    {
                        ...
                        // Draw avg bar.
                        Rectangle(m_u32Needle, pimDst, sBarPos, sMeterY + (sMeterH - sVal), sBarWidth, sVal);
                    }

                    short sPosY = sMeterY + sMeterH - sMeterMax;
                    short sDiff = sMeterMax - sMeterVal;
                    Rectangle(m_u32Overflow, pimDst, sBarPos, sPosY, sBarWidth, sDiff);

                    // Draw avg.
                    sPosY += sDiff;
                    sDiff = sMeterVal - sMeterMin;
                    Rectangle(m_u32Needle, pimDst, sBarPos, sPosY, sBarWidth, sDiff);

                    // Draw max.
                    sPosY += sDiff;
                    // Draw min.
                    Rectangle(m_u32Overflow, pimDst, sBarPos, sPosY, sBarWidth, sMeterMin);
                    break;
                }
            }
