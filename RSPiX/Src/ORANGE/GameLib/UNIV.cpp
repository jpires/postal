...
short CUniverse::SetSection(short sWorld,   // World to load
                            short sStage,   // Stage to load
                            short sSection) // Section to load
{
...
short CUniverse::LoadAnimData()
{
    ...
    ANIMSET *pAnimSet = (ANIMSET *)malloc(sizeof(ANIMSET) + (pChunkHeader->dwSub - 1) * sizeof(ANIM *));
    ...
    for (lAnim = 0; lAnim < pAnimSet->lNumAnims; lAnim++)
    {
        ...
        for (sFrame = 0; sFrame < pAnimSet->apAnims[lAnim]->sNumFrames; sFrame++)
        {
            ...
            pAnimSet->apAnims[lAnim]->aFrames[sFrame].pImage = (IMAGE *)malloc(pChunkHeader->dwSize);
...
void CUniverse::SetAnimOffsets(ANIM *pAnimation) // Pointer to ANIM structure
{
    short s;
    ...
    for (s = 0; s < pAnimation->sNumFrames; s++)
    {
        if (pAnimation->aFrames[s].pImage)
            ...
            pAnimation->aFrames[s].pImage = (IMAGE *)((BYTE *)pAnimation->aFrames[s].pImage + (DWORD)m_pImageData -
                                                      (DWORD)4);
        if (pAnimation->aFrames[s].pZoneSet)
        {
            //			pAnimation->aFrames[s].pZoneSet += (BYTE*) m_pAnimData;
            pAnimation->aFrames[s].pZoneSet = (ZONESET *)((BYTE *)pAnimation->aFrames[s].pZoneSet + (DWORD)m_pAnimData -
                                                          (DWORD)4);
...
void CUniverse::SetZonesetOffsets(ZONESET *pZoneset) // Pointer to ZONESET structure
{
    short s;
    ...
    for (s = 0; s < MAX_ZONETYPES; s++)
    {
        if (pZoneset->apZoneLists[s])
            ...
            pZoneset->apZoneLists[s] = (ZONELIST *)((BYTE *)pZoneset->apZoneLists[s] + (DWORD)m_pAnimData - (DWORD)4);
    }
}
...
PALETTE *CUniverse::LoadPalette(LPSTR lpstrFilePalette) // Palette file to be loaded
{
    return NULL;
}
