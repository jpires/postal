...
#define POS2PIXEL(lPos, o) ((short)((m_oOrientation == o) ? ((float)(lPos - m_lMinPos) * m_fPos2PixelRatio + 0.5) : 0))
...
// value.
#define PIXELS2POS(sPixX, sPixY) PIXEL2POS((m_oOrientation == Vertical) ? (sPixY) : (sPixX))

// Sets a value pointed to if ptr is not NULL.
#define SET_IF_NOT_NULL(pval, val) (((pval) != NULL) ? *(pval) = (val) : (0))
...
class RScrollBar : public RGuiItem
{
  ...
  public: // Methods.
    ...
    void CopyBorderInfoTo(RGuiItem *pgui) // In: GuiItem to copy to.
    {
    ...
    long GetPos(void) { return m_lCurPos; }
  ...
  protected: // Internal functions.
    ...
    static void UpHotCall(RHot *phot,       // Ptr to RHot that generated event.
                          RInputEvent *pie) // In:  Most recent user input event.
                                            // Out: pie->sUsed = TRUE, if used.
    {
        ((RScrollBar *)(phot->m_ulUser))->UpHotCall(pie);
    }
    static void DownHotCall(RHot *phot,       // Ptr to RHot that generated event.
                            RInputEvent *pie) // In:  Most recent user input event.
                                              // Out: pie->sUsed = TRUE, if used.
    {
        ((RScrollBar *)(phot->m_ulUser))->DownHotCall(pie);
    }
    static void ThumbHotCall(RHot *phot,       // Ptr to RHot that generated event.
                             RInputEvent *pie) // In:  Most recent user input event.
                                               // Out: pie->sUsed = TRUE, if used.
    {
