...
void CRSPiXBlue::SetDisplayImage(CImage *pim, short sFlip /*= FALSE*/)
{
    ...
    Blu_SetDisplayBuf(pim->pData, lWinWidth, pim->lHeight, pim->sDepth, sFlip);

    Blu_SetRedrawBuf(pim->pData, lWinWidth, pim->lHeight, 0, 0, 0, 0, pim->lWidth, pim->lHeight, pim->sDepth, sFlip);
    ...
    MoveWindow(0, 0, pim->lWidth - (rcClient.left - rcWindow.left), pim->lHeight - (rcClient.top - rcWindow.top));
...
void CRSPiXBlue::SetDisplayPalette(CPal *ppal)
{
    ...
    switch (ppal->ulType)
    {
        ...
        case PDIB:
        case PSYS:
            Blu_SetPalette((RGBQUAD *)ppal->pData, ppal->sStartIndex, ppal->sNumEntries);
            break;
        case P555:
            Blu_SetPalette((PRGBT5)ppal->pData, ppal->sStartIndex, ppal->sNumEntries);
            break;
        case P888:
        case PFLX:
            Blu_SetPalette((PRGBT8)ppal->pData, ppal->sStartIndex, ppal->sNumEntries);
            break;
        case P565:
...
void CRSPiXBlue::PreSubclassWindow()
{
    ...
    if (Blu_Init(AfxGetApp()->m_hInstance, m_hWnd) == 0)
    {
        ...
        if (Blu_CreateDisplay(rcClient.right, rcClient.bottom, (short)Blu_GetDisplayInfo(DI_MONITOR_COLORDEPTH)) == 0)
        {
            // Success.
