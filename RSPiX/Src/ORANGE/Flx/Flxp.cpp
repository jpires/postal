...
short CFlx::WritePixelDelta(FLX_BUF *pbufNext, FLX_BUF *pbufPrev, UCHAR *pBuf, long *plChunkSize)
{
    ...
    *plChunkSize = 0;
    ...
    if (pbufPrev == NULL)
    {
        ...
        long lSizeBRUN =
          CompressBRUN(pbufNext->pbPixels, pbufNext->sPitch, 0, 0, m_filehdr.sWidth, m_filehdr.sHeight, pBuf);
    ...
    else
    {
        ...
        if (m_filehdr.wMagic == FLX_MAGIC_FLI)
        {
            ...
            for (y = 0; y < m_filehdr.sHeight; y++)
            {
                ...
                if (sError != 0)
                {
                    ...
                    if (sLineCount != 0)
                    {
                        // Other previous lines has been compressed already.  Determine whether to maintain a empty
                        // delta line count. Increment the empty line count.
                        sEmptyLineCount++;
                    }
                }
                else
        ...
        else
        {
            ...
            for (y = 0; y < m_filehdr.sHeight; y++)
            {
                ...
                {
                    // The current line compressed.  We need to reset the line skip count to zero.  Also increment line
                    // count.
                    sLineSkipCount = 0;
                    sLines++;
            ...
            if (sAlign == 1)
            {
                ...
                *(pbDst + (ULONG)lSize++) = (UCHAR)sSkipCount;
                ...
                if ((x < (sAdjustedPitch - 1)) && (pbSrcNext[dwOffset + x] == pbSrcNext[dwOffset + x + 1]))
                {
                    ...
                    x++;
                    while ((nBytes < 127) && (x < sAdjustedPitch) &&
                ...
                else
                {
                    ...
                    *(pbDst + (ULONG)lSize++) = pbSrcNext[dwOffset + x + 1];
                    ...
                    while ((nBytes < 127) && (x < (sAdjustedPitch - 1)) &&
                           ((pbSrcNext[dwOffset + x] != pbSrcPrev[dwOffset + x]) ||
                            (pbSrcNext[dwOffset + x + 1] != pbSrcPrev[dwOffset + x + 1])) &&
                           !((x < (sAdjustedPitch - 3)) && (pbSrcNext[dwOffset + x] == pbSrcNext[dwOffset + x + 2]) &&
                             (pbSrcNext[dwOffset + x + 1] == pbSrcNext[dwOffset + x + 3])))
                    {
                        *(pbDst + (ULONG)lSize++) = pbSrcNext[dwOffset + x];
                        *(pbDst + (ULONG)lSize++) = pbSrcNext[dwOffset + x + 1];
...
//
/////////////////////////////////////////////////////////////////////////////
long CFlx::CompressBRUN(UCHAR *pbIn,   // Pointer to input (pixels to be compressed)
