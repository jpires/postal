...
class CFlx
{
  public:
    ...
    short Open(char *pszFileName,      // Full path and filename of flic file
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short Create(char *pszFileName,      // Full path and filename of flic file
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short ReadFrame(short sFrameNum,    // Frame number to be read
                    FLX_BUF *pbufRead); // Buffer for frame being read

    // Read the next flic frame (if flic was just opened, this will read frame 1).
    // Returns 0 if successfull, non-zero otherwise.
    short ReadNextFrame(FLX_BUF *pbufRead); // Buffer for frame being read
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short WriteNextFrame(FLX_BUF *pbufWrite); // Buffer of frame to be written
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short WriteFinish(FLX_BUF *pbufFirst, // Buffer of first frame that was written or NULL
    ...
    // Returns 0 if successfull, non-zero otherwise.
    short WriteFirstFrame(FLX_BUF *pbufWrite); // Buffer of frame to be written
  ...
  private:
    ...
    short CompressLineDelta(short y,                   // Current line to compress, used to calculate offset.
    ...
    long CompressBRUN(UCHAR *pbIn,   // Pointer to input (pixels to be compressed)
