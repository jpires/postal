...
class RSList
{
  ...
  public:
    ...
    LISTDATA GetCurrent() { return (m_pnCurrent ? m_pnCurrent->ldData : NULL); }

    short IsEmpty() { return (short)(m_pnHead == NULL); }
    ...
    short Compare(PNODE pn1, PNODE pn2)
    {
        ...
        return ((*pn1->psk) < (*pn2->psk)) ? -1 : ((*pn1->psk) > (*pn2->psk)) ? 1 : 0;
    }

    // Set or clear the compare function used to compare nodes.
    void SetCompareFunc(CMPCALL cc) { m_fncmp = cc; }

  public:
    RSList(short sInitialize = TRUE) // If this flag is FALSE, no initialization
