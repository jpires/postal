                             ...
                             // image.
{
    ...
    // Allocate eraser . . .
    RImage imEraser;
    if (imEraser.CreateImage(m_im.m_sWidth, m_im.m_sHeight, RImage::BMP8, 0, m_im.m_sDepth) == 0)
    {
    }
    else
    {
    ...
    while (m_ulId == 0 && sError == 0)
    {
        ...
        // Erase old.
        rspBlit(&imEraser, pimDst, 0, 0, dr.sX, dr.sY, dr.sW, dr.sH);
  ...
  short sAddH /*= 0*/)  // Additional height for guis that require more.
{
    ...
    short sTotalBorderThickness = pgui->GetTopLeftBorderThickness() + pgui->GetBottomRightBorderThickness();
