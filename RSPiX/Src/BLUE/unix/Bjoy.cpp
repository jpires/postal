...
static void calcAxis(SDL_GameController *controller,
...
static inline void calcButton(SDL_GameController *controller,
...
static void GetAxesNew(SDL_GameController *controller,
                       Sint16 *out_axis_MoveUpDown,
                       Sint16 *out_axis_MoveLeftRight,
                       Sint16 *out_axis_FireUpDown,
                       Sint16 *out_axis_FireLeftRight)
{
    // deadzone shit
#define DEADZONE_PERCENT 25.f
    ...
    if (axis_MoveUpDown < deadzone && axis_MoveUpDown > -deadzone && axis_MoveLeftRight < deadzone &&
        axis_MoveLeftRight > -deadzone)
    {
        axis_MoveUpDown = 0;
        axis_MoveLeftRight = 0;
    }
    if (axis_FireUpDown < deadzone && axis_FireUpDown > -deadzone && axis_FireLeftRight < deadzone &&
        axis_FireLeftRight > -deadzone)
    {
        axis_FireUpDown = 0;
        axis_FireLeftRight = 0;
    }

    if (out_axis_MoveUpDown)
        *out_axis_MoveUpDown = axis_MoveUpDown;
    if (out_axis_MoveLeftRight)
        *out_axis_MoveLeftRight = axis_MoveLeftRight;
    if (out_axis_FireUpDown)
        *out_axis_FireUpDown = axis_FireUpDown;
    if (out_axis_FireLeftRight)
        *out_axis_FireLeftRight = axis_FireLeftRight;

    // TRACE("SDL Axes: %f.5 %f.5 %f.5 %f.5\n", axis_MoveUpDown, axis_MoveLeftRight, axis_FireUpDown,
    // axis_FireLeftRight);
}
...
#if defined(ALLOW_TWINSTICK)
    GetAxesNew(controller,
               &ms_ajsCurr[sJoy].axis_MoveUpDown,
               &ms_ajsCurr[sJoy].axis_MoveLeftRight,
               &ms_ajsCurr[sJoy].axis_FireUpDown,
               &ms_ajsCurr[sJoy].axis_FireLeftRight);
// #else
#endif
