  ...
  long sh)
{
    ...
    short sRes = ClipQuiet(&sX, &sY, &sW, &sH, (short)sx, (short)sy, (short)sw, (short)sh);
    ...
    while (rspQueryVideoMode(&sModeColorDepth, &sModeWidth, &sModeHeight, &sModePages) == 0)
    {
        ...
        if (sModeColorDepth == sDepth && sPages == sModePages)
        {
            ...
            if (sWidth <= sModeWidth && sHeight <= sModeHeight)
            {
                ...
                float fFactorOld = ((float)sBestModeWidth * (float)sBestModeHeight) / ((float)sWidth * (float)sHeight);
                float fFactorNew = ((float)sModeWidth * (float)sModeHeight) / ((float)sWidth * (float)sHeight);
    ...
    if (sModeFound != FALSE)
    {
        ...
        if (psScaling != nullptr)
        {
            ...
            if (sScaling != FALSE)
            {
                ...
                // requested mode . . .
                if (sWidth * 2 <= sBestModeWidth && sHeight * 2 <= sBestModeHeight)
                {
...
//
//////////////////////////////////////////////////////////////////////////////
extern short rspGetVideoMode(short *psDeviceDepth,             // Hardware display color depth returned here
                                                               // (unless NULL).
...
// FALSE indicates not to use this garbage.
{
    ...
    sdlWindow =
      SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, RequestedWidth, RequestedHeight, flags);
    if (!sdlWindow)
    {
        char buf[128];
    ...
    if (!sdlRenderer)
    {
        ...
        SDL_snprintf(buf,
                     sizeof(buf),
                     "Couldn't create %s renderer: %s",
                     bRequestedRenderer ? "requested" : "a",
                     SDL_GetError());
        fprintf(stderr, "POSTAL: %s\n", buf);
...
//
///////////////////////////////////////////////////////////////////////////////
extern void rspUpdateDisplay(short sX,      // x coord of upper-left corner of area to update
                             short sY,      // y coord of upper-left corner of area to update
                             short sWidth,  // Width of area to update
                             short sHeight) // Height of area to update
{
...
//
///////////////////////////////////////////////////////////////////////////////
extern void rspSetPaletteEntries(short sStartIndex,     // Palette entry to start copying to (has no effect on source!)
...
//
///////////////////////////////////////////////////////////////////////////////
void rspSetPaletteEntry(short sEntry,  // Palette entry (0 to 255)
...
//
///////////////////////////////////////////////////////////////////////////////
void rspGetPaletteEntry(short sEntry,   // Palette entry (0 to 255)
...
//
///////////////////////////////////////////////////////////////////////////////
extern void rspGetPaletteEntries(short sStartIndex,       // Palette entry to start copying from
                                 short sCount,            // Number of palette entries to do
...
//
///////////////////////////////////////////////////////////////////////////////
extern void rspSetPaletteMaps(short sStartIndex,       // Map entry to start copying to (has no effect on source!)
...
//
///////////////////////////////////////////////////////////////////////////////
extern void rspGetPaletteMaps(short sStartIndex,       // Map entry to start copying from (has no effect on dest!)
                              short sCount,            // Number of map entries to do
                              unsigned char *pucRed,   // Pointer to first red component to copy to
                              unsigned char *pucGreen, // Pointer to first green component to copy to
                              unsigned char *pucBlue,  // Pointer to first blue component to copy to
...
///////////////////////////////////////////////////////////////////////////////
extern void rspLockPaletteEntries(short sStartIndex, // Palette entry at which to start locking.
...
///////////////////////////////////////////////////////////////////////////////
extern void rspUnlockPaletteEntries(short sStartIndex, // Palette entry at which to start locking.
                                    short sCount)      // Number of palette entries to lock.
{
    // Set up iterator pointer.
    short *psLock = &(asPalEntryLocks[sStartIndex]);
