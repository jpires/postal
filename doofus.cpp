...
CDoofus::WeaponType CDoofus::ms_awtId2Type[TotalIDs] = {
    NoWeapon,      InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon,     Rocket,
    Grenade,       InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon,     Napalm,
    InvalidWeapon, InvalidWeapon, Firebomb,      InvalidWeapon, InvalidWeapon, InvalidWeapon,     InvalidWeapon,
    InvalidWeapon, InvalidWeapon, InvalidWeapon, ProximityMine, InvalidWeapon, InvalidWeapon,     InvalidWeapon,
    Pistol,        MachineGun,    ShotGun,       InvalidWeapon, TimedMine,     BouncingBettyMine, RemoteControlMine,
    InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon, Heatseeker,    InvalidWeapon,     Assault,
    InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon, InvalidWeapon,     InvalidWeapon,
    InvalidWeapon, Flamer,        DeathWad,      DoubleBarrel,  Uzi,           AutoRifle,         SmallPistol,
    Dynamite,
};

char *CDoofus::ms_apszActionNames[] = {
    "Guard",        "Advance", "Retreat", "Engage", "Popout",  "Run&Shoot", "Hide",
    "Advance-Hold", "Walk",    "Panic",   "March",  "Madness", "Help",
};
...
// Startup object
////////////////////////////////////////////////////////////////////////////////
short CDoofus::Startup() // Returns 0 if successfull, non-zero otherwise
{
    CCharacter::Startup();
...
UCHAR CDoofus::SelectRandomBouy()
{
    UCHAR ucSelect = 0;
    CBouy *pBouy = nullptr;
    ...
    if (m_panimCur->m_ptransRigid)
    {
        ...
        while (!bFoundPath && sTries < 2)
        {
            ...
            */
            ...
            if (m_pRealm->IsPathClear(m_dX + dMuzzleX,
                                      m_dY + dMuzzleY,
                                      m_dZ + dMuzzleZ,
                                      (short)dRotAttempt,
                                      3,
...
void CDoofus::Update()
{
    if (!m_sSuspend)
    {
        ...
        if (m_state == State_Idle || m_state == State_Wait || m_state == State_Hunt)
            Logic_PylonDetect();
...
void CDoofus::Logic_PositionSet()
{
    if (!ReevaluateState())
    {
        long lThisTime = m_pRealm->m_time.GetGameTime();
...
void CDoofus::Logic_PositionMove()
{
    long lThisTime = m_pRealm->m_time.GetGameTime();
    long lTimeDifference = lThisTime - m_lPrevTime;
...
void CDoofus::Logic_BeSafe() {}

////////////////////////////////////////////////////////////////////////////////
// Logic_Firefight - Shoot and move
////////////////////////////////////////////////////////////////////////////////

void CDoofus::Logic_Firefight() {}
...
void CDoofus::Logic_PylonDetect()
{
    CSmash *pSmashPylon;
    ...
    if (m_pRealm->m_smashatorium.QuickCheckClosest(&m_smashDetect, CSmash::Pylon, 0, 0, &pSmashPylon))
    {
        ASSERT(pSmashPylon);
...
void CDoofus::Logic_Shoot()
{
    ...
    switch (m_eWeaponType)
    {
        ...
        default:
            if (m_u16IdWeapon != CIdBank::IdNil)
            {
                if (WhileHoldingWeapon(ms_u32CollideBitsInclude, ms_u32CollideBitsDontcare, ms_u32CollideBitsExclude) ==
                    true)
                {
...
void CDoofus::Logic_RunShootBegin()
{
    long lThisTime = m_pRealm->m_time.GetGameTime();
    long lElapsedTime = lThisTime - m_lPrevTime;
    double dStartX = m_dX;
...
void CDoofus::Logic_RunShootWait()
{
    long lThisTime = m_pRealm->m_time.GetGameTime();
...
void CDoofus::Logic_PanicContinue()
{
    m_state = State_PanicBegin;
}
...
void CDoofus::Logic_WalkContinue()
{
...
bool CDoofus::AvoidFire()
{
    ...
    if (m_pRealm->m_smashatorium.QuickCheck(&m_smashAvoid, CSmash::Fire, CSmash::Good | CSmash::Bad, 0, &pSmashed))
    {
        bFireDanger = true;
...
void CDoofus::YellForHelp()
{
    ...
    m_pRealm->m_smashatorium.QuickCheckReset(&smashYell, CSmash::Character | CSmash::Bad, 0, CSmash::Good);
...
bool CDoofus::ReevaluateState()
{
    ...
    if (m_eSuggestedAction != m_eCurrentAction && m_state != State_Burning && m_state != State_BlownUp &&
        m_state != State_Die && m_state != State_Dead && m_state != State_Writhing)
    {
        switch (m_eSuggestedAction)
        {
            case Action_Guard:
                if (m_panimCur != &m_animStand && m_panimCur != &m_animCrouch && m_panimCur != &m_animSearch)
                {
                    m_panimCur = &m_animStand;
                    m_lAnimTime = 0;
                }
...
void CDoofus::OnShotMsg(Shot_Message *pMessage)
{
    ...
    if (m_state != State_Burning && m_state != State_BlownUp && m_state != State_Die && m_state != State_Dead)
    {
        ...
        else
        {
            ...
            if (lThisTime > m_lShotTimeout)
            {
                ...
                if (m_state != State_Shot && m_state != State_Writhing)
                {
                    m_ePreviousState = m_state;
                    m_panimPrev = m_panimCur;
                    m_state = State_Shot;
...
{
    if (m_state != State_BlownUp && m_state != State_Die && m_state != State_Dead)
    {
...
////////////////////////////////////////////////////////////////////////////////
CWeapon *CDoofus::PrepareWeapon() // Returns the weapon ptr or NULL.
{
...
CWeapon *CDoofus::ShootWeapon(CSmash::Bits bitsInclude, CSmash::Bits bitsDontcare, CSmash::Bits bitsExclude)
{
    ...
    if (m_state != State_ShootRun)
    {
        ...
        switch (m_pRealm->m_flags.sDifficulty)
        {
            ...
            case 4:
                m_dRot = m_dAnimRot = m_dShootAngle = dSaveDirection = rspMod360(FindDirection() - 6 +
                                                                                 (GetRandom() % 13));
                break;

            case 5:
                m_dRot = m_dAnimRot = m_dShootAngle = dSaveDirection = rspMod360(FindDirection() - 5 +
                                                                                 (GetRandom() % 11));
                break;

            case 6:
                m_dRot = m_dAnimRot = m_dShootAngle = dSaveDirection = rspMod360(FindDirection() - 4 +
                                                                                 (GetRandom() % 9));
                break;
            ...
            case 10:
                m_dRot = m_dAnimRot = m_dShootAngle = dSaveDirection = rspMod360(FindDirection() - 3 +
                                                                                 (GetRandom() % 7));
                break;
...
short CDoofus::GetResources()
{
    ...
    if (ms_lWeaponResRefCount++ == 0)
    {
        ...
        for (i = 0; i < NumWeaponTypes; i++)
        {
            ...
            {
                sLoadResult = ms_aanimWeapons[i].Get(ms_awdWeapons[i].pszResName,
                                                     nullptr,
                                                     nullptr,
                                                     nullptr,
