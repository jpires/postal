...
// USE GLOBAL MASK
//
void g_alphaBlit(RImage *pimSrc,   // Source image. (wall)
...
void Verify()
{
    short i;
    sFirst = FALSE;
    for (i = 0; i < strlen((char *)sCheckSum1); i++)
...
#define TILE_ANGLE_DEL (10.0 / 360.0)
    ...
    while (bContinue)
    {
        ...
        rspBlitRot(short(dCurAngle), pimF, pimBuffer, sCurX - sdx, sCurY - sdy, dsx, dsy);
        rspUnlockBuffer();
...
void SetAll()
{
    ...
    for (i = 0; i < strlen((char *)sCheckSum2); i++)
        sCheckSum2[i] = 255 - sCheckSum2[i];
    for (i = 0; i < strlen((char *)sCheckSum3); i++)
        pct[i] = 255 - sCheckSum3[i];
    RImage *pimF, *pimB;
    if (rspGetResource(&g_resmgrSamples, (char *)sCheckSum2, &pimF) != SUCCESS)
        return;
