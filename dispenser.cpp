...
////////////////////////////////////////////////////////////////////////////////
void LogicItemCall(RGuiItem *pguiLogicItem); // In:  Logic item that was pressed.
  ...
  ULONG ulFileVersion)  // In:  Version of file format to load.
{
    ...
    if (sResult == 0)
    {
        ...
        {
            // Convert to 3D.
            m_pRealm->MapY2DtoZ3D(m_sZ, &m_sZ);
        }
            ...
            switch (m_logictype)
            {
                    ...
                    if (lCurTime >= m_lNextUpdate)
                    {
                        ...
                        if (m_alLogicParms[1] - m_alLogicParms[0] > 0)
                        {
                            ...
                            m_lNextUpdate = lCurTime + m_alLogicParms[0] +
                                            (GetRand() % (m_alLogicParms[1] - m_alLogicParms[0]));
                        }
                        else
                        {
                            m_lNextUpdate = lCurTime + m_alLogicParms[0];
                ...
                case DistanceToDude:
                {
                    if (lCurTime >= m_lNextUpdate)
                    {
                        ...
                        {
                            if ((lDudeDist >= m_alLogicParms[0] || m_alLogicParms[0] == 0) &&
                                (lDudeDist <= m_alLogicParms[1] || m_alLogicParms[1] == 0))
                            {
                                // Create a thing . . .
                                CThing *pthing;
                                if (InstantiateDispensee(&pthing, false) == 0)
...
// 'Infinite' checkbox.
////////////////////////////////////////////////////////////////////////////////
static void UpdateMaxDispensees(RGuiItem *pgui_pmb) // In:  Multibtn that was pressed.
{
...
short CDispenser::EditModify() // Returns 0 if successfull, non-zero otherwise
{
    ...
    if (m_idDispenseeType == TotalIDs || (pau8KeyStatus[RSP_SK_ALT] & 1) == 0)
    {
        ...
        if (pguiRoot != nullptr)
        {
            ...
            if (plbThings && plbLogics && pbtnModify && peditMaxDispensees && pmbInfiniteDispensees)
            {
...
{
    // Map from 3d to 2d coords
    Map3Dto2D((short)m_sX, (short)m_sY, (short)m_sZ, &(prc->sX), &(prc->sY));
    ...
    if (m_idDispenseeType > 0 && m_idDispenseeType < TotalIDs)
    {
        ...
        if (CThing::Construct(m_idDispenseeType, m_pRealm, ppthing) == 0)
        {
            if (m_fileDispensee.IsOpen() != FALSE)
            {
                ...
                if ((*ppthing)->Load(&m_fileDispensee,
                                     bEditMode,
  ...
  CThing *pthing)                  // In:  Instance of Dispensee to render.
{
    ...
    if (m_bEditMode == true)
    {
        ...
        {
            // Map from 3d to 2d coords
            Map3Dto2D((short)m_sX, (short)m_sY, (short)m_sZ, &(m_rcDispensee.sX), &(m_rcDispensee.sY));
        ...
        {
            // Clear.
            rspRect(0, // In:  Not black, but transparent.
                    &m_imRender,
                    0,
                    0,
                    m_imRender.m_sWidth,
            ...
            else
            {
                ...
                print.print(&m_imRender, 0, 0, "%s", ms_aClassInfo[m_idDispenseeType].pszClassName);
            }
#if 1
            // Draw dispenser icon on top.
            rspBlit(m_pim,                                           // Src image.
