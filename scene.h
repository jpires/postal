...
class CScene
{
  ...
  public:
    ...
    void RemoveSprites(short sLayer); // In:  Layer number (0 to n-1)
    ...
    void UpdateSprite(CSprite *pSprite); // In:  Sprite to add

    // Remove sprite (safe to call even if sprite isn't in scene).
    void RemoveSprite(CSprite *pSprite); // In:  Sprite to remove

    // Render specified area of scene into specified image
    void Render(short sSrcX,    // In:  Source (scene) x coord
    ...
    Render3D(RImage *pimDst,     // Destination image.
             short sDstX,        // Destination 2D x coord.
             short sDstY,        // Destination 2D y coord.
