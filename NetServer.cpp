  ...
  RSocket::BLOCK_CALLBACK callback) // In:  Blocking callback
{
    ...
    // Setup listen socket
    sResult = m_socketListen.Open(m_usBasePort + Net::ListenPortOffset,
                                  RSocket::typStream,
    ...
    if (sResult == 0)
    {
        ...
        if (sResult == 0)
        {
            ...
            sResult = m_socketAntenna.Open(m_usBasePort + Net::AntennaPortOffset,
                                           RSocket::typDatagram,
...
void CNetServer::Update()
{
    ...
    if (!m_socketListen.IsError())
    {
        ...
        {
            if ((m_aClients[id].m_msgr.GetState() == CNetMsgr::Disconnected) &&
    ...
    if (!m_bGameStarted)
    {
        ...
        if (serr == 0)
        {
            ...
            if ((lReceived == sizeof(buf1)) && (buf1[0] == Net::BroadcastMagic0) && (buf1[1] == Net::BroadcastMagic1) &&
                (buf1[2] == Net::BroadcastMagic2) && (buf1[3] == Net::BroadcastMagic3))
            {
...
// Get message
////////////////////////////////////////////////////////////////////////////////
void CNetServer::GetMsg(NetMsg *pmsg) // Out: Message is returned here
{
        ...
        if (!m_aClients[id].m_bLoggedIn)
        {
            if (pmsg->msg.nothing.ucType == NetMsg::LOGIN)
            {
                if (pmsg->msg.login.ulMagic == CNetMsgr::MagicNum)
                {
                    ...
                    if ((pmsg->msg.login.ulVersion >= CNetMsgr::MinVersionNum) &&
                        (pmsg->msg.login.ulVersion <= CNetMsgr::CurVersionNum))
                    {
                        // Compatible.
                    }
                    else
                    {
                        ...
                        if ((pmsg->msg.login.ulVersion & CNetMsgr::MacVersionBit) ^
                            (CNetMsgr::MinVersionNum & CNetMsgr::MacVersionBit))
                        {
            ...
            switch (pmsg->msg.nothing.ucType)
            {
                ...
                case NetMsg::JOIN_REQ:
                    if (!m_bGameStarted)
                    {
                        ...
                        RSocket::SetAddressPort(m_usBasePort + Net::FirstPeerPortOffset + id,
                                                &m_aClients[id].m_address);
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::SendMsg(Net::ID id,   // In:  ID to send message to
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::SendMsg(U16 mask,     // In:  Bit-mask indicating who to send to
                         NetMsg *pmsg) // In:  Message to send
{
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::SendMsg(NetMsg *pmsg) // In:  Message to send
{
    for (Net::ID id = 0; id < Net::MaxNumIDs; id++)
    {
        if (m_aClients[id].m_state == CClient::Joined)
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::DropClient(Net::ID id) // In:  Client to drop
{
    ASSERT(id != Net::InvalidID);
    ASSERT(id < Net::MaxNumIDs);
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::SetupGame(short sRealmNum,          // In:  Realm number
...
////////////////////////////////////////////////////////////////////////////////
void CNetServer::StartGame(Net::ID idServer,     // In:  Server's client's ID
...
// Abort game
////////////////////////////////////////////////////////////////////////////////
void CNetServer::AbortGame(unsigned char ucReason) // In:  Why game was aborted
{
...
////////////////////////////////////////////////////////////////////////////////
bool CNetServer::NextRealm(Net::SEQ seq) // In:  The seq on which to go to next realm
{
...
// Start the process of dropping a client during a game
////////////////////////////////////////////////////////////////////////////////
void CNetServer::StartDroppingClientDuringGame(Net::ID id)
{
    // Tell all clients to drop this client (including the one being dropped)
