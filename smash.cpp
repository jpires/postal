...
{
    if (!pSmashee->m_pThing)
        return SUCCESS; // not a dude
    if (pSmashee->m_pThing->GetClassID() != CThing::CDudeID)
        return SUCCESS; // not a dude
    ...
    if (ABS2(pSmashee->m_sphere.sphere.X - pSphere->X, pSmashee->m_sphere.sphere.Z - pSphere->Z) >
        SQR(lCylR + pSphere->lRadius))
...
bool CSmashatorium::QuickCheckNext(CSmash **ppSmashee)
{
    ...
    if (!m_pSmasher)
        return false; // reset at end of search
    ...
    while (pSmashee) // compare this with what we want
    {
        ...
        { // Avoid redundancy
            ...
            if (!(pSmashee->m_bits & m_exclude) && ((pSmashee->m_bits & ~m_dontcare) & m_include) &&
                pSmashee != m_pSmasher)
            {
            ...
            if (pCurrentList->m_sNum)
            {
                ...
                while (pLink != &pCurrentList->m_slTail)
                {
                    ...
                    { // Avoid redundancy
                        ...
                        if (!(pSmashee->m_bits & exclude) && ((pSmashee->m_bits & ~dontcare) & include) &&
                            pSmashee != pSmasher)
                        {
  ...
  CSmash *pSmasher)                    // Out: Smash that should be excluded from search.
{
    ...
    {
        if ((lLeft < 0) || (lRight >= m_sWorldW))
            return false;
    }

    // Check for case of reverse clip out:
    if ((lLeft >= m_sWorldW) || (lRight < 0))
    ...
    if (sVerticalStrip) // do special clipping:
    {
        ...
        if (lClipRight < lClipLeft)
            return false; // vertical strip off screen
        ...
        if (lClipRightY >= m_sWorldH)
            lClipRightY = m_sWorldH;
        if (lClipLeftY < 0)
            lClipLeftY = 0;
        if (lClipLeftY >= m_sWorldH)
            lClipLeftY = m_sWorldH;
    }
    ...
    {                  // Note that the vertical case has
        ...
        if (lDelY > 0) // lClipLeftY < lClipRightY:
        {
            if (lClipLeftY < 0)
            {
                ...
                if ((lClipLeft < 0) || (lClipLeft >= m_sWorldW))
                    return false;
            ...
            if (lClipRightY >= m_sWorldH)
            {
                ...
                if ((lClipRight < 0) || (lClipRight >= m_sWorldW))
                    return false;
    ...
    {
        if ((lClipLeftY < 0) || (lClipLeftY >= m_sWorldH))
            return false;
    }
                            ...
                            pSmashee != pSmasher)
                        {
                            if (pSmashee->m_sphere.Collide(pline) == COLLISION)
                            {
                                if (CollideCyl(pSmashee, pline) == SUCCESS)
                                {
                                    ...
                                    lCurDist2 = ABS2(pSmashee->m_sphere.sphere.X - pline->X1,
...
void CSmashatorium::Update(CSmash *pSmash) // In:  CSmash to be updated
{
    ...
    if (pSmash->m_pThing->m_pRealm->m_flags.bEditing && (!pSmash->m_pThing->m_pRealm->m_flags.bEditPlay))
    {
    ...
    if ((lD > m_sTileW) || (lD > m_sTileH))
    {
        ...
        if ((lX <= -lD) || (lY <= -lD) || (lX >= m_sWorldW) || (lY >= m_sWorldH))
        {
            // We have FULL CLIP OUT!
            if (pSmash->m_sInGrid)
                RemoveFat(pFat); // set's InGrid to false
            else
        ...
        if ((lX != pFat->m_lX) || (lY != pFat->m_lY) || (!pSmash->m_sInGrid))
        {
            ...
            short sGridX, sGridY;
            if (lX < 0)
                sGridX = -m_psAccessX[-lX] - 1; // mirror it!
            else
                sGridX = m_psAccessX[lX];

            if (lY < 0)
                sGridY = -m_psAccessY[-lY] - 1; // mirror it!
            else
                sGridY = m_psAccessX[lY];
    ...
    if ((lX <= -m_sTileW) || (lY < -m_sTileH) || (lX >= m_sWorldW) || (lY >= m_sWorldH))
    {
        // We have FULL CLIP OUT!
        if (pSmash->m_sInGrid)
            Remove(pSmash); // set's InGrid to false
        else
            pSmash->m_sInGrid = FALSE;
...
void CSmashatorium::Add(CSmash *pSmash, CSmashatoriumList *pList)
{
    ...
    ASSERT(pSmash);
    if (pSmash->m_sInGrid)
        return; // Don't need to re-add it!
    if (pSmash->m_pFat)
    {
    ...
    m_sNumInSmash++;
    if (m_sNumInSmash > m_sMaxNumInSmash)
        m_sMaxNumInSmash = m_sNumInSmash;
}
...
{
    ASSERT(pFatSmash);
    if (pFatSmash->m_pParent->m_sInGrid)
        return; // Don't need to re-add it!
    //=====================================
    ...
    //-------------------------------------
    for (j = 0; j < pFatSmash->m_sClipH;
         j++, pList += m_sGridW - pFatSmash->m_sClipW, pLink += pFatSmash->m_sW - pFatSmash->m_sClipW)
    {
...
{
    if (pSmash->m_sInGrid == FALSE)
        return; // don't need to remove it!
    if (pSmash->m_pFat)
    {
...
{
    ASSERT(pFatSmash);
    if (pFatSmash->m_pParent->m_sInGrid == FALSE)
        return; // don't need to remove it!
    //===========================================
