            ...
            if (sMenuItem == ms_sResetItem || sMenuItem == ms_sResetItemOld)
            {
                ...
                for (sInputIndex = 0; sInputIndex < CInputSettings::NumInputFunctions && sError == 0; sInputIndex++)
                {
                    ...
                    switch (pmenu->u32Id)
                    {
                        case KEYBOARD_MENU_ID:
                            // Use keys.
                            pasPlayInputs[sInputIndex] = sMenuItem == ms_sResetItemOld
                                                           ? CInputSettings::ms_ainputinfoOld[sInputIndex].u8DefaultKey
                                                           : CInputSettings::ms_ainputinfo[sInputIndex].u8DefaultKey;
                            break;
                        case MOUSE_MENU_ID:
                            // Use mouse buttons.
                            pasPlayInputs[sInputIndex] =
                              sMenuItem == ms_sResetItemOld
                                ? CInputSettings::ms_ainputinfoOld[sInputIndex].sDefMouseButtons
                                : CInputSettings::ms_ainputinfo[sInputIndex].sDefMouseButtons;
                            break;
                        case JOYSTICK_MENU_ID:
                            // Use joy buttons.
                            pasPlayInputs[sInputIndex] =
                              sMenuItem == ms_sResetItemOld
                                ? CInputSettings::ms_ainputinfoOld[sInputIndex].sDefJoyButtons
                                : CInputSettings::ms_ainputinfo[sInputIndex].sDefJoyButtons;
                            break;
                    }
                    ...
                    if (pmenu->u32Id == JOYSTICK_MENU_ID)
                        pgui->SetText("%s",
                                      g_apszJoyButtonDescriptions[JoyBitfieldToIndex(pasPlayInputs[sInputIndex])]);
                    else if (pmenu->u32Id == MOUSE_MENU_ID)
                        pgui->SetText("%s",
                                      g_apszMouseButtonDescriptions[MouseBitfieldToIndex(pasPlayInputs[sInputIndex])]);
                    else
                        pgui->SetText("%s", papszInputDescriptions[pasPlayInputs[sInputIndex]]);
                    pgui->Compose();
                }
                ...
                switch (ie.sEvent)
                {
                    ...
                    case RSP_MB2_PRESSED:
                        if (RGuiItem::ms_pguiFocus && ms_bListenForInput == false)
                        {
                            ...
                            InputSettingsDlg_Choice(GetCurrentMenu(), RGuiItem::ms_pguiFocus->m_ulUserData);
                        }
                        break;
                }
                break;
