...
static char *ms_apszStillResNames[] = { "3d/barrel_still.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszSpinResNames[] = { "3d/barrel_spin.sop",
                                       "3d/barrel_spin.mesh",
                                       "3d/barrel_spin.tex",
                                       "3d/barrel_spin.hot",
            ...
            case CBarrel::State_Burning:
                if (lThisTime > m_lTimer)
                {
                    ...
                    for (i = 0; i < ms_sNumFires; i++)
                    {
                        ...
                        {
                            if (pFire->Setup(m_dX - 20 + (GetRandom() % 40),
                                             m_dY,
                                             m_dZ - 20 + (GetRandom() % 40),
                                             4000 + (GetRandom() % 9000),
                                             false,
