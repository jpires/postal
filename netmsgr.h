...
////////////////////////////////////////////////////////////////////////////////
inline void GetSocketAddress(CBufQ *pBuf, RSocket::Address *paddress)
{
...
////////////////////////////////////////////////////////////////////////////////
inline void PutSocketAddress(CBufQ *pBuf, RSocket::Address *paddress)
{
...
class NetMsg
{
  ...
  public:
    ...
    typedef struct Err
    {
        enum
        {
            Size = 1 + sizeof(Error) + sizeof(ULONG)
        };
        unsigned char ucType; // Message type
    ...
    typedef struct LoginDeny
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.loginDeny.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.loginDeny.ucType); }
    } LoginDeny;
    ...
    typedef struct Logout
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.logout.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.logout.ucType); }
    } Logout;
    ...
    typedef struct JoinReq
    {
        enum
        {
            Size = 1 + Net::MaxPlayerNameSize + 1 + 1 + 2
        };
        unsigned char ucType;                // Message type
    ...
    typedef struct JoinAccept
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.joinAccept.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.joinAccept.ucType); }
    } JoinAccept;
    ...
    typedef struct Joined
    {
        enum
        {
            Size = 1 + 1 + sizeof(RSocket::Address) + Net::MaxPlayerNameSize + 1 + 1 + 2
        };
    ...
    typedef struct ChangeReq
    {
        enum
        {
            Size = 1 + Net::MaxPlayerNameSize + 1 + 1
        };
        unsigned char ucType;                // Message type
        char acName[Net::MaxPlayerNameSize]; // Name
        unsigned char ucColor;               // Color number
    ...
    typedef struct Changed
    {
        enum
        {
            Size = 1 + 1 + Net::MaxPlayerNameSize + 1 + 1
        };
        unsigned char ucType;                // Message type
        Net::ID id;                          // ID
        char acName[Net::MaxPlayerNameSize]; // Name
    ...
    typedef struct DropReq
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.dropReq.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.dropReq.ucType); }
    } DropReq;
    ...
    typedef struct DropAck
    {
        enum
        {
            Size = 1 + sizeof(Net::SEQ) + sizeof(Net::SEQ)
        };
        unsigned char ucType;        // Message type
    ...
    typedef struct InputData
    {
        ...
        {
            Size = -1
        };                    // -1 indicates variable-sized message
        unsigned char ucType; // Message type
        ...
        static void Write(NetMsg *pmsg, CBufQ *pBuf)
        {
            ...
            pmsg->msg.inputData.lSize = 1 + 4 + 1 + sizeof(Net::SEQ) + 2 +
                                        ((long)(pmsg->msg.inputData.sNum) * sizeof(UINPUT));
    ...
    typedef struct SetupGame
    {
        enum
        {
            Size = 1 + 2 + Net::MaxRealmNameSize + 2 + 2 + 2 + 2 + 2 + 2
        };
    ...
    typedef struct StartGame
    {
        enum
        {
            Size = 1 + 1 + 2 + Net::MaxRealmNameSize + 2 + 2 + 2 + 2 + 2 + sizeof(Net::SEQ) + 2 + 2
        };
        unsigned char ucType;                    // Message type
    ...
    typedef struct ReadyRealm
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.readyRealm.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.readyRealm.ucType); }
    } ReadyRealm;
    ...
    typedef struct BadRealm
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.badRealm.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.badRealm.ucType); }
    } BadRealm;
    ...
    typedef struct StartRealm
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.startRealm.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.startRealm.ucType); }
    } StartRealm;
    ...
    typedef struct Proceed
    {
        ...
        static void Read(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Get(&pmsg->msg.proceed.ucType); }

        static void Write(NetMsg *pmsg, CBufQ *pBuf) { pBuf->Put(&pmsg->msg.proceed.ucType); }
    } Proceed;
  ...
  public:
    ...
    static U8 *AllocVar(NetMsg *pmsg, long lSize) { return pmsg->AllocVar(lSize); }

    static void FreeVar(NetMsg *pmsg) { pmsg->FreeVar(); }
...
class CNetMsgr
{
  ...
  public:
    ...
    // Destructor
    ////////////////////////////////////////////////////////////////////////////////
    void Reset(bool bCleanly = false)
    {
    ...
    // Accept connection
    ////////////////////////////////////////////////////////////////////////////////
    short Accept(const RSocket *psocketListen,     // In:  Listen socket to accept connection on
                 RSocket::BLOCK_CALLBACK callback) // In:  Socket callback
    {
        ASSERT(m_state == Disconnected);
    ...
    ////////////////////////////////////////////////////////////////////////////////
    void Disconnect(bool bCleanly = false) // In:  true means cleanly, false means immediately
    {
    ...
    // Get message
    ////////////////////////////////////////////////////////////////////////////////
    bool GetMsg(NetMsg *pmsg); // Out: Message is returned here

    ////////////////////////////////////////////////////////////////////////////////
    // Send message
    ////////////////////////////////////////////////////////////////////////////////
    void SendMsg(NetMsg *pmsg,          // In:  Message to send
    ...
    ////////////////////////////////////////////////////////////////////////////////
    const RSocket::Address &GetAddress(void) { return m_address; }

    ////////////////////////////////////////////////////////////////////////////////
    // Determine the time of the most-recently received or sent message
    ////////////////////////////////////////////////////////////////////////////////
    long GetMostRecentMsgReceiveTime(void) { return m_lMsgRecvTime; }

    long GetMostRecentMsgSentTime(void) { return m_lMsgSentTime; }
