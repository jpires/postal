...
////////////////////////////////////////////////////////////////////////////////
inline void SetPushBtnState(RGuiItem *pguiRoot,    // In:  Root GUI.
...
SameSigns_Vector(RP3d *v1, // In: vector 1
                 RP3d *v2) // In: vector 1
{
...
GetIntersectSubT(RP3d &pt0,   // In: line segment point 0
...
PointInsideTri(RP3d &pt0A,     // In: point 1 of triangle
...
TriAIntersectsLineSegmentB(RP3d &normalA, // In: normal vector of triangle A
...
////////////////////////////////////////////////////////////////////////////////
bool TrianglesIntersectLineSegment(
...
////////////////////////////////////////////////////////////////////////////////
void CreatePalette(RGuiItem *pgui) // In:  GUI to contain palette.
{
...
////////////////////////////////////////////////////////////////////////////////
void SetText(RGuiItem *pguiRoot,  // In:  Root GUI.
...
////////////////////////////////////////////////////////////////////////////////
void ValidateTextures(RTexture *ptex, // In:  Texture to validate.
                      short sNum)     // In:  Number of textures it should have.
{
    if (ptex->m_sNum < sNum)
    {
        short sRes = rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNO,
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::DoModal(CAnim3D *panim,         // In:  3D animation to paint on.
                       ...
                       const RString &strFile) // In:  Filename to save modified texture as.
{
    ...
    if (m_pguiRoot)
    {
        ...
        // Center.
        m_pguiRoot->Move(pimComposite->m_sWidth / 2 - m_pguiRoot->m_im.m_sWidth / 2,
        ...
        while (m_bQuit == false)
        {
            ...
            // Output //////////////////////////////////////////////
            rspRect(0, &m_pguiAnim->m_im, 0, 0, m_pguiAnim->m_im.m_sWidth, m_pguiAnim->m_im.m_sHeight, &rcClient);

            DoOutput(&sprite,
                     &sopView,
                     trans,
                     m_bSpotLight ? pltSpot : pltAmbient,
                     &m_pguiAnim->m_im,
                     sOffsetX,
                     sOffsetY,
                     rcClient);
            ...
            if (m_bQuit)
            {
                ...
                if (m_bModified)
                {
                    ...
                    short sRes = rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNOCANCEL,
                                           g_pszAppName,
    ...
    {
        rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, g_pszAppName, g_pszFileOpenError_s, FullPathVD(c_strGuiFile));
    }
}
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::DoOutput(CSprite3 *psprite,   // In:  3D data to render.
...
{
    m_scene.Render3D(pimDst,      // Destination image.
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::ProcessManip(bool bButtonDown,  // In:  true if mouse button down.
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetManip(Manip manip) // In:  New manipulation type.
{
    if (m_pguiRoot)
    {
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetColor(U8 u8Color) // In:  New color index.
{
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetToNotify(long lBtnId,             // In:  ID of btn whose callback will be set.
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetToNotify(long lId,                     // In:  ID of gui whose callback will be set.
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetToNotify(long lId,                      // In:  ID of scrollbar whose callback will be set.
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::SetStatusText(const char *pszFrmt, // In:  Format specifier ala sprintf.
                             ...)                 // In:  Arguments as specified by format.
{
...
//////////////////////////////////////////////////////////////////////////////
void CTexEdit::ComposeTransform(RTransform &trans) // In:  Transform to compose in.
{
    trans = m_transRot;
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::QuitCall(RGuiItem *pgui)
{
    m_bQuit = true;
}
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::ManipCall(RGuiItem *pgui)
{
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::ColorCall(RGuiItem *pgui, RInputEvent *pie)
{
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::ApplyCall(RGuiItem *pgui)
{
    Apply();
}
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::SaveCall(RGuiItem *pgui)
{
    Save();
}
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::RevertCall(RGuiItem *pgui)
{
    Revert();
}
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::SpotCall(RGuiItem *pgui)
{
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::BrightnessCall(RScrollBar *psb)
{
    long lMin, lMax;
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::AdjustCall(RGuiItem *pgui)
{
    // Get frequency.
    ...
    U8 au8Green[256];
    U8 au8Blue[256];
    rspGetPaletteEntries(0,           // Palette entry to start copying from
    ...
    m_texWork.Unmap(au8Red, au8Green, au8Blue, sizeof(U8));

    // Adjust colors.
    m_texWork.Adjust(fAdjust, lFreq);

    // Remap onto palette.
    m_texWork.Remap(c_sPalStart, c_sPalNum, au8Red, au8Green, au8Blue, sizeof(U8));
...
////////////////////////////////////////////////////////////////////////////////
void CTexEdit::AnimCall(RGuiItem *pgui, RInputEvent *pie)
{
    if (pie->type == RInputEvent::Mouse)
    {
        switch (pie->sEvent)
