...
void ScoreInit()
{
    ...
    ms_print.SetColor(gsStatusFontForeIndex, gsStatusFontBackIndex, gsStatusFontShadowIndex);
...
bool ScoreUpdateDisplay(RImage *pim,
                        RRect *prc,
                        CRealm *pRealm,
                        CNetClient *pclient,
                        short sDstX,
                        ...
                        CHood *pHood)
{
    ...
    if (lCurTime > g_scoreboard.m_lLastScoreDrawTime + SCORE_UPDATE_INTERVAL)
    {
        ...
        if (pHood)
            rspBlit(pHood->m_pimTopBar,
                    pim,
                    0,
                    0,
                    sDstX,
                    sDstY,
                    pHood->m_pimTopBar->m_sWidth,
                    pHood->m_pimTopBar->m_sHeight);
        ...
        switch (pRealm->m_ScoringMode)
        {
            case CRealm::MPFrag:
                ...
                for (i = 0; i < sNumDudes; i++)
                {
                    ...
                    ms_print.print(rcBox.sX, rcBox.sY, "%s", pclient->GetPlayerName(i));

                    rcBox.sY = prc->sY + STATUS_PRINT_Y2;
                    ms_print.SetDestination(pim, &rcBox);
                    ms_print.print(rcBox.sX, rcBox.sY, "%d", g_scoreboard.m_asScores[i]);
                }
                break;
            ...
            case CRealm::Standard:
                ...
                ms_print.print(pim,
                               rcDst.sX,
                               rcDst.sY,
                               //					"      Population %d                        Hostiles %d   Killed %d
                               //(%d%% / %d%%)",
                               g_apszScoreDisplayText[pRealm->m_ScoringMode],
                               pRealm->m_sPopulation,
                               pRealm->m_sHostiles,
                               pRealm->m_sHostileKills,
                               pRealm->m_sHostileKills * 100 /
                                 ((pRealm->m_sHostileBirths != 0) ? pRealm->m_sHostileBirths : 1),
            ...
            case CRealm::TimedGoal:
                ...
                ms_print.print(
                  ...
                  sSeconds,
                  pRealm->m_sPopulationDeaths,
                  pRealm->m_sKillsGoal - pRealm->m_sPopulationDeaths,
                  pRealm->m_sPopulation);
                break;
                             ...
                             // duration, if not -1).
{
    ...
    if (pRealm->m_ScoringMode >= CRealm::Timed && pRealm->m_ScoringMode <= CRealm::MPLastManTimedFrag &&
        GetInputMode() != INPUT_MODE_PLAYBACK)
    {
        ...
        switch (pRealm->m_ScoringMode)
        {
            ...
            case CRealm::Timed:
                sprintf(szScoringExplanation,
                        g_apszScoreExplanations[pRealm->m_ScoringMode],
                        CreateTimeString(pRealm->m_lScoreInitialTime));
                // Number of deaths.
        ...
        if (pRealm->m_flags.bMultiplayer == false)
        {
            ...
            for (sSrcIndex = 0, sDstIndex = 0; sDstIndex < MAX_HIGH_SCORES; sDstIndex++)
            {
                ...
                {
                    if (sOpenRes == 0)
                        scores.GetVal((char *)pRealm->m_rsRealmString,
                                      szKeyName,
                                      (long)3600000,
                                      &(alScores[sDstIndex]));
                    else
        ...
        else
        {
            ...
            ASSERT(false == 0);
            bool abAlreadyCopied[MAX_HIGH_SCORES] = {
                false,
            };
        ...
        if (rspGetResource(&g_resmgrShell, HIGHSCORE_DIALOG_FILE, (RDlg **)&pguiRoot) == 0)
        {
                    ...
                    if (astrNames[sScoreIndex][0] != '\0' || sPlayersScorePosition == sScoreIndex)
                    {
                        ...
                        if (rspGetResourceInstance(&g_resmgrShell, HIGHSCORE_ITEM_FILE, &pguiItem) == 0)
                        {
                            ...
                            if (pguiName && pguiScore)
                            {
                                ...
                                {
                                    // Value.
                                    pguiScore->SetText("%ld %s",
                                                       alScores[sScoreIndex],
                                                       g_apszScoreUnits[pRealm->m_ScoringMode]);
                                }
                                else
                                {
                                    // Time.
                                    pguiScore->SetText("%s %s",
                                                       CreateTimeString(alScores[sScoreIndex]),
                                                       g_apszScoreUnits[pRealm->m_ScoringMode]);
                                }
            ...
            if (ptextExplain1 != nullptr && ptextExplain2 != nullptr && plbScores != nullptr && bGotAllScoreItems == true)
            {
                ...
                // Center the GUI root.
                pguiRoot->Move(g_pimScreenBuf->m_sWidth / 2 - pguiRoot->m_im.m_sWidth / 2,
                ...
                if (sPlayersScorePosition >= 0 && pRealm->m_flags.bMultiplayer == false)
                {
                    ...
                    pguiPlayersName->GetText(astrNames[sPlayersScorePosition],
                                             sizeof(astrNames[sPlayersScorePosition]));
                }
