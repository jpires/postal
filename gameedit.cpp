...
#define MAP_X                                                                                                          \
    MAX(ms_pguiRealmBar->m_sX + ms_pguiRealmBar->m_im.m_sWidth, ms_pguiGUIs->m_sX + ms_pguiGUIs->m_im.m_sWidth)
...
#define DUDE_STATUS_RECT_H                                                                                             \
    (g_GameSettings.m_sDisplayInfo                                                                                     \
       ? (INFO_STATUS_RECT_Y - DUDE_STATUS_RECT_Y)                                                                     \
       : (g_pimScreenBuf->m_sHeight - DUDE_STATUS_RECT_Y)) // (g_pimScreenBuf->m_sHeight - DUDE_STATUS_RECT_Y)
...
typedef struct TAG_Line
{
    ...
    bool operator==(const TAG_Line &rhs) const
    {
        return (this->sX0 == rhs.sX0 && this->sY0 == rhs.sY0 && this->sX1 == rhs.sX1 && this->sY1 == rhs.sY1);
    }
...
static CPylon *ms_pylonEdit = nullptr; // NULL for none.
...
static void DrawCursor(short sCursorX,    // In:  Cursor hotspot x coord
...
static short NewRealm(CRealm *prealm);

static short CloseRealm(CRealm *prealm);

static short LoadRealm(CRealm *prealm);
...
static short SaveRealm(CRealm *prealm);

static short SaveRealmAs(CRealm *prealm);
...
static void OnEnlargeDisplay(CCamera *pcamera, // Camera to update.
                             CRealm *prealm);  // Realm to update.

// Reduces the display area.
static void OnReduceDisplay(CCamera *pcamera, // Camera to update.
                            CRealm *prealm);  // Realm to update.
...
// is specified size.
static short SetCameraArea(); // Returns 0 on success.
...
static void ResetHotPriorities(); // Returns nothing.
...
// Clear all views.
static void ClearViews(); // Returns nothing.
...
static short SizeShowAttribsSprite(); // Returns 0 on success.
...
// Init().
static void KillFileCounter(); // Returns nothing.
...
extern void GameEdit()
{
    ...
    rspGetVideoMode(&ms_sInitialDeviceDepth,
...
// Display message if necessary
    ...
    rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK,
              "Postal Editor",
    ...
    if (InitCursor() == 0)
    {
        ...
        if (ms_pguiRealmBar != nullptr && ms_pguiPickObj != nullptr && ms_pguiLayers != nullptr && ms_pguiCameras != nullptr &&
            ms_pguiGUIs != nullptr && ms_pguiMap != nullptr && ms_pguiNavNets != nullptr && ms_pguiShowAttribs && ms_pguiInfo)
        {
            ...
            for (sLayer = CRealm::LayerBg; sLayer < CRealm::TotalLayers; sLayer++)
            {
                ...
                if (pguiItem != nullptr)
                {
                    ...
                    ((RPushBtn *)pguiItem)->m_state = (prealm->m_scene.m_pLayers[sLayer].m_bHidden == false)
                                                        ? RPushBtn::On
                                                        : RPushBtn::Off;
                    // Realize state.
                    pguiItem->Compose();
            ...
            // ---------- Show Attribs --------
            SetMBValsAndCallback(ms_pguiShowAttribs,
                                 GUI_ID_ATTRIB_LAYERS,
                                 (U32)(&ms_u16LayerMask),
                                 REALM_ATTR_LAYER_MASK,
                                 1);
            SetMBValsAndCallback(ms_pguiShowAttribs,
                                 GUI_ID_ATTRIB_HEIGHT,
                                 (U32)(&ms_u16TerrainMask),
                                 REALM_ATTR_HEIGHT_MASK,
                                 1);
            SetMBValsAndCallback(ms_pguiShowAttribs,
                                 GUI_ID_ATTRIB_NOWALK,
                                 (U32)(&ms_u16TerrainMask),
        ...
        {
            rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, g_pszCriticalErrorTitle, g_pszAssetsMissingError);
        }
    ...
    rspSetVideoMode(ms_sInitialDeviceDepth,
                    ms_sInitialDeviceWidth,
                    ms_sInitialDeviceHeight,
                    ms_sInitialDisplayWidth,
                    ms_sInitialDisplayHeight);
  ...
  short *psCursorZ)  // Out: Cursor Z position.
{
    ...
    else
    {
        ...
        if (ms_pylonEdit != nullptr)
        {
            ...
            {
                case 1:
                    rspRect(TRIGGER_RGN_DRAW_INDEX, // *** FUDGE ***.
        ...
        else
        {
            ...
            switch (sCursorEvent)
            {
                ...
                case CURSOR_LEFT_DRAG_BEGIN:
                    ...
                    if (ms_pthingSel != nullptr)
                    {
                        ...
                        if (ms_pthingSel->GetClassID() != CThing::CHoodID)
                        {
                            ...
                            rspSetMouse((ms_photSel->m_sX - pcamera->m_sScene2FilmX) + sOffsetX,
                ...
                case CURSOR_NOTHING:
                    ...
                    if (ms_sMoving != FALSE)
                    {
                        ...
                        Maprealm2Screen(prealm, pcamera, sCursorX, 0, sCursorZ, &sScreenX, &sScreenY);
                        ...
                        if (sScreenX < DRAG_SCROLL_BUFFER)
                        {
                            ...
                            {
                                ms_sbHorz.SetPos(ms_sbHorz.GetPos() +
                                                 (sScreenX - DRAG_SCROLL_BUFFER) * EDGE_MOVEMENT_MULTIPLIER);
                            }
                        ...
                        else if (sScreenX > pcamera->m_sViewW - DRAG_SCROLL_BUFFER)
                        {
                            ...
                            {
                                ms_sbHorz.SetPos(ms_sbHorz.GetPos() +
                                                 (sScreenX - (pcamera->m_sViewW - DRAG_SCROLL_BUFFER)) *
                                                   EDGE_MOVEMENT_MULTIPLIER);
                            }
                        ...
                        if (sScreenY < DRAG_SCROLL_BUFFER)
                        {
                            if (ms_bDragScroll == true)
                            {
                                ms_sbVert.SetPos(ms_sbVert.GetPos() +
                                                 (sScreenY - DRAG_SCROLL_BUFFER) * EDGE_MOVEMENT_MULTIPLIER);
                            }
                        ...
                        else if (sScreenY > pcamera->m_sViewH - DRAG_SCROLL_BUFFER)
                        {
                            ...
                            {
                                ms_sbVert.SetPos(ms_sbVert.GetPos() +
                                                 (sScreenY - (pcamera->m_sViewH - DRAG_SCROLL_BUFFER)) *
                                                   EDGE_MOVEMENT_MULTIPLIER);
                            }
            ...
            switch (ms_lPressedId)
            {
                ...
                case GUI_ID_PLAY_REALM:
                    ...
                    {
                        // ***LOCALIZE***.
                        rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK, "Play a Realm", "There is no hood.  Cannot play.");
                    }
                ...
                case GUI_ID_TOGGLE_LAYER:
                {
                    ...
                    if (pguiSel != nullptr)
                    {
                        ...
                        // Toggle.
                        prealm->m_scene.m_pLayers[layer].m_bHidden = (((RPushBtn *)pguiSel)->m_state == RPushBtn::On)
                                                                       ? false
                                                                       : true;
                    }
                    break;
                ...
                case GUI_ID_PASTE:
                    ...
                    PasteItem(prealm,    // In:  The realm to paste into.
                              sCursorX,  // x
                              sCursorY,  // y
                              sCursorZ); // z
                    break;
                ...
                default:
                    ...
                    if (ms_lPressedId >= LIST_ITEM_GUI_ID_BASE &&
                        ms_lPressedId < LIST_ITEM_GUI_ID_BASE + CThing::TotalIDs && ms_sMoving == FALSE)
                    {
                        CThing *pthingNew;
                        RHot *photNew;
  ...
  short *psEvent)      // Out: Event type.
{
    ...
    *psEvent = CURSOR_NOTHING;
    ...
    if (ms_sDragState == 1)
    {
        ...
        if (((rspGetMilliseconds() - lDragTime) >= DRAG_MIN_TIME) || (ABS(sMouseX - sDragX) >= DRAG_MIN_X) ||
...
static void KillCursor()
{
    ...
    m_pimCursorBase = nullptr;
    delete m_pimCursorTip;
    m_pimCursorTip = nullptr;
}
                       ...
                       CCamera *pcamera) // In:  Camera on prealm.
{
    ...
    rspBlit(m_pimCursorBase, pimDst, sBaseX2 - CURSOR_BASE_HOTX, sBaseY2 - CURSOR_BASE_HOTY);

    // Draw the top offset from the base by y coord (needs to be negated here!)
    rspBlit(m_pimCursorTip, pimDst, sTipX2 - CURSOR_TIP_HOTX, sTipY2 - CURSOR_TIP_HOTY);
...
#else
        rspLine((UCHAR)255, pimDst, sBaseX2, sBaseY2, sTipX2, sTipY2);
#endif
    }
}
...
static short CloseRealm(CRealm *prealm)
{
    ...
    {
        // Check for save . . .
        switch (rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNOCANCEL, g_pszAppName, g_pszSaveFileQuery))
        {
            case RSP_MB_RET_YES:
...
static short LoadRealm(CRealm *prealm)
{
    ...
    if (sResult == 0)
    {
        ...
        // ***LOCALIZE***
        sResult = rspOpenBox("Load Realm", ms_szFileName, ms_szFileName, sizeof(ms_szFileName), ".rlm");
        if (sResult == 0)
        {
            ...
            if (sResult == 0)
            {
                ...
                if (prealm->m_asClassNumThings[CThing::CHoodID] > 0)
                {
                    ...
                    phood->m_phot = ms_photHood = new RHot(rc.sX,                   // Position.
                                                           rc.sY,                   // Position.
                                                           rc.sW,                   // Dimensions.
                                                           rc.sH,                   // Dimensions.
                    ...
                    if (ms_photHood != nullptr)
                    {
                        ...
                        while (pList->m_powner != nullptr && sResult == 0)
                        {
                            ...
                            if (pthing->GetClassID() != CThing::CHoodID)
                            {
                                ...
                                pthing->m_phot = new RHot(rc.sX,                   // Position.
                                                          rc.sY,                   // Position.
                                                          rc.sW,                   // Dimensions.
                                                          rc.sH,                   // Dimensions.
                                                          ThingHotCall,            // Callback.
...
#ifdef DISABLE_EDITOR_SAVE_AND_PLAY
    rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK, "Postal Editor", "Sorry, but the save feature is disabled.");
    sResult = -1;
#else
    // Attempt to get filename . . .
    // ***LOCALIZE***
    if (rspSaveBox("Save Realm", ms_szFileName, ms_szFileName, sizeof(ms_szFileName), ".rlm") == 0)
    {
...
//
////////////////////////////////////////////////////////////////////////////////
static void PlayRealm(CRealm *pEditRealm, // In:  Realm to play.
                      CThing *pthingSel)  // In:  Currently selected CThing which can
                                          // be used to give PlayRealm() a hint on which
                                          // of several things the user wants to use.
                                          // For example, a selected warp is the used
...
{
#ifdef DISABLE_EDITOR_SAVE_AND_PLAY
    rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK, "Postal Editor", "Sorry, but the play feature is disabled.");
#else
        ...
        if (sResult == 0)
        {
            ...
            if (sResult == 0)
            {
                ...
                if (prealm->Startup() == 0)
                {
                    ...
                    {
                        pdudeLocal->m_sTextureIndex = MAX(
                          (short)0,
                          MIN((short)(CDude::MaxTextures - 1), g_GameSettings.m_sPlayerColorIndex));
                    ...
                    RRect rcDudeStatus(DUDE_STATUS_RECT_X, DUDE_STATUS_RECT_Y, DUDE_STATUS_RECT_W, DUDE_STATUS_RECT_H);
                    // Setup rectangular area for realm's status.
                    RRect rcRealmStatus(REALM_STATUS_RECT_X,
                                        REALM_STATUS_RECT_Y,
                                        REALM_STATUS_RECT_W,
                                        REALM_STATUS_RECT_H);
                    // Setup rectangular area for display info.
                    RRect rcInfoStatus(INFO_STATUS_RECT_X, INFO_STATUS_RECT_Y, INFO_STATUS_RECT_W, INFO_STATUS_RECT_H);
                    ...
                    while (bDone == false)
                    {
                        ...
                        if (pthingTrack != nullptr)
                        {
                            ...
                            // Convert to realm.
                            MapScreen2Realm(prealm,                          // In:  Realm.
  ...
  RFile *pfile /* = NULL*/)  // In:  Optional file to load from (instead of EditNew()).
{
    ...
    {
        // ***LOCALIZE***
        rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, "Editor: ", "Can't have multiple CHood's!");
            ...
            if (!(id == CThing::CBouyID && prealm->GetCurrentNavNet() == nullptr))
            {
                ...
                if (CThing::ConstructWithID(id, prealm, ppthing) == 0)
                {
                    ...
                    if (sError == 0)
                    {
                        ...
                        (*ppthing)->m_phot = *pphot = new RHot(rc.sX,                   // Position.
                                                               rc.sY,                   // Position.
                                                               rc.sW,                   // Dimensions.
                                                               rc.sH,                   // Dimensions.
                                                               ThingHotCall,            // Callback.
  ...
  short sPosZ)         // New position.
{
    ...
    // Move to 3D position.
    pthing->EditMove(sPosX, sPosY, sPosZ);
  ...
  short sDisplayH)           // New height of display area.
{
    ...
    sError =
      rspSuggestVideoMode(sDeviceD, sDisplayW, sDisplayH, 1, sDeviceScaling, &sDeviceW, &sDeviceH, &sDeviceScaling);

    // If we are not supposed to change the device dimensions . . .
    if (g_GameSettings.m_sUseCurrentDeviceDimensions != FALSE)
    {
        // Get current device settings.
        rspGetVideoMode(&sDeviceD, &sDeviceW, &sDeviceH);
        // Limit display area.
    ...
    if (sError == 0)
    {
        ...
        sError = rspSetVideoMode(sDeviceD, sDeviceW, sDeviceH, sDisplayW, sDisplayH, 1, sDeviceScaling);
  ...
  CRealm *prealm)               // Realm to update.
{
    ...
    rspGetVideoMode(&sDeviceD, nullptr, nullptr, nullptr, &sDisplayW, &sDisplayH);

    // New area.
    if (SetDisplayArea(sDeviceD, sDisplayW + sAdjustX, sDisplayH + sAdjustY) == 0)
    {
        SizeUpdate(pcamera, prealm);
    }
    else
  ...
  CRealm *prealm)        // Realm to update.
{
    ...
    // Get current settings.
    rspGetVideoMode(&sDisplayD, nullptr, nullptr, nullptr, &sDisplayW, &sDisplayH);
    ...
    if (pcamera != nullptr)
    {
        ...
        pcamera->SetView(pcamera->m_sSceneViewX, pcamera->m_sSceneViewY, sViewW, sViewH);
    ...
    if (ms_sbVert.Create(sViewW, 0, SCROLL_BAR_THICKNESS, sViewH + SCROLL_BAR_THICKNESS, sDisplayD) == 0)
    {
        // Set scroll range.
        if (phood != nullptr)
        {
    ...
    if (ms_sbHorz.Create(0, sViewH, sViewW, SCROLL_BAR_THICKNESS, sDisplayD) == 0)
    {
        // Set scroll range.
        if (phood != nullptr)
        {
  ...
  CCamera *pcamera)       // In:  View of prealm.
{
    ...
    if (ms_bDrawNetwork && m_pBouyLink0 != nullptr && m_pBouyLink1 != nullptr && m_pBouyLink0->Visible() &&
        m_pBouyLink1->Visible())
    {
        ...
        Maprealm2Screen(prealm, pcamera, m_pBouyLink0->GetX(), 0, m_pBouyLink0->GetZ(), &sBouyLink0X, &sBouyLink0Y);

        short sBouyLink1X, sBouyLink1Y;
        Maprealm2Screen(prealm, pcamera, m_pBouyLink1->GetX(), 0, m_pBouyLink1->GetZ(), &sBouyLink1X, &sBouyLink1Y);

        rspLine(249, g_pimScreenBuf, sBouyLink0X, sBouyLink0Y, sBouyLink1X, sBouyLink1Y);
    }
    else if (m_pBouyLink0 != nullptr && m_pBouyLink1 == nullptr)
    {
        // These lines show the paths the characters would take so
        ...
        rspLine(249, g_pimScreenBuf, sBouyLink0X, sBouyLink0Y, sMouseX, sMouseY);
    }
}
...
static void UpdateNetLines(CNavigationNet *pNavNet)
{
    ...
    if (pNavNet)
    {
        ...
        for (ibouy = pNavNet->m_NodeMap.begin(); ibouy != pNavNet->m_NodeMap.end(); ibouy++)
        {
            pLinkedBouy = (*ibouy).second->m_aplDirectLinks.GetHead();
            while (pLinkedBouy)
            {
                AddNewLine((*ibouy).second->GetX(), (*ibouy).second->GetZ(), pLinkedBouy->GetX(), pLinkedBouy->GetZ());
  ...
  CCamera *pcamera)      // In:  View of prealm.
{
    ...
    if (ms_bDrawNetwork)
    {
        for (i = m_NetLines.begin(); i != m_NetLines.end(); i++)
        {
            ...
            // is ignored.
            short sBouyLink0X, sBouyLink0Y;
            Maprealm2Screen(prealm, pcamera, (*i).sX0, 0, (*i).sY0, &sBouyLink0X, &sBouyLink0Y);

            short sBouyLink1X, sBouyLink1Y;
            Maprealm2Screen(prealm, pcamera, (*i).sX1, 0, (*i).sY1, &sBouyLink1X, &sBouyLink1Y);

            rspLine(250, g_pimScreenBuf, sBouyLink0X, sBouyLink0Y, sBouyLink1X, sBouyLink1Y);
        }
    }
}
  ...
  CRealm *prealm)     // Realm to draw.
{
    ...
    // Draw scene.
    pview->cam.Snap(pview->sViewW,      // In:  View's width
...
static void ClearGUI( // Returns nothing.
  RGuiItem *pgui)     // In:  GUI's whose area we should clean.
{
    rspRect(RSP_BLACK_INDEX, g_pimScreenBuf, pgui->m_sX, pgui->m_sY, pgui->m_im.m_sWidth, pgui->m_im.m_sHeight);
}
...
// Do all views.
////////////////////////////////////////////////////////////////////////////////
static void DoViews(RInputEvent *pie) // Input event to process.
{
  ...
  CRealm *prealm)       // Realm to map.
{
    ...
    if (ms_pguiMapZone != nullptr)
    {
        ...
        if (imFilm.CreateImage(sViewW, sViewH, RImage::BMP8) == 0)
        {
            // Take a shot.
            camera.Snap(sViewW,             // In:  View's width
  ...
  short sDropZ)       // In:  Drop z position.
{
    ...
    if (ms_sMoving != FALSE)
    {
        ...
        // Final position.
        MoveThing(ms_pthingSel, ms_photSel, sDropX, sDropY, sDropZ);
...
////////////////////////////////////////////////////////////////////////////////
extern void Edit_Menu_Continue()
{
    StopMenu();
...
// editor.
////////////////////////////////////////////////////////////////////////////////
extern void Edit_Menu_ExitEditor()
{
    ms_lPressedId = GUI_ID_EXIT;
...
)
{
    ...
    if (prealm->m_pTriggerMap)
        delete prealm->m_pTriggerMap;
    ...
    pTriggers = CreateRegionMap(prealm->m_phood->GetWidth(), prealm->m_phood->GetHeight(), 8, 32, 32);
    if (!pTriggers)
    {
  ...
  CThing *pthingPylon)              // In:  Pylon whose trigger area we want to
{
    ...
    if (pthingPylon != nullptr)
    {
        ...
        if (ms_argns[pylon->m_ucID].pimRgn == nullptr)
        {
            ...
            // position.
            pthingPylon->m_pRealm->Map3Dto2D(pylon->GetX(),
                                             0,
                                             pylon->GetZ(),
  ...
  CRealm *prealm)     // In:  Current Realm
{
    if (pthingDel != nullptr)
    {
        ...
        switch (pthingDel->GetClassID())
        {
            case CThing::CHoodID:
                // *** LOCALIZE ***
                rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, "Delete Hood", "You may not delete the hood.");
                SetSel(nullptr, nullptr);
                pthingDel = nullptr;
                photDel = nullptr;
                break;
            ...
            case CThing::CGameEditThingID:
                // *** LOCALIZE ***
                rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, "Delete Hood", "You may not delete the CGameEditThing.");
                SetSel(nullptr, nullptr);
                pthingDel = nullptr;
                photDel = nullptr;
                break;
            ...
            case CThing::CBouyID:
                ...
                if ((CBouy *)ms_pthingSel == m_pBouyLink0 || (CBouy *)ms_pthingSel == m_pBouyLink1)
                    m_pBouyLink0 = m_pBouyLink1 = nullptr;
  ...
  CRealm *prealm)     // In:  Current realm
{
    ...
    sprintf(szTitle,
            ...
            (pthingDel != nullptr) ? CThing::ms_aClassInfo[pthingDel->GetClassID()].pszClassName : "CThing");
    // VERIFY . . .
    if (rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNO,
                  szTitle,
                  "Are you sure you want to perform this group delete?!") == RSP_MB_RET_YES)
    {
...
{
    char szTitle[512];
    sprintf(szTitle, "Delete Everything except bouys, pylons, warps, soundthings and soundrelays?");
    // VERIFY . . .
    if (rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNO, szTitle, "Are you sure you want to perform such a delete?") ==
        RSP_MB_RET_YES)
    {
        CListNode<CThing> *plnDel;
        CListNode<CThing> *plnTail;
    ...
    if (pthingCopy != nullptr)
    {
        ...
        switch (pthingCopy->GetClassID())
        {
            ...
            default:
            {
                ...
                // Open paste buffer.
                if (ms_filePaste.Open(PASTE_BUFFER_INITIAL_SIZE, PASTE_BUFFER_GROW_SIZE, RFile::NeutralEndian) == 0)
                {
    ...
    if (prealm->m_phood != nullptr)
    {
        ...
        *psRealmX = sRealmX2;
        ...
        if (prealm->m_pTerrainMap != nullptr)
        {
            ...
            *psRealmY = prealm->GetHeight(*psRealmX, *psRealmZ);
        }
        else
        {
            // Safety.
  ...
  short *psScreenY)          // Out: Screen y coord.
{
    ...
    short sViewX2;
    short sViewY2;
    prealm->Map3Dto2D(sRealmX, sRealmY, sRealmZ, &sViewX2, &sViewY2);
...
static short SizeShowAttribsSprite() // Returns 0 on success.
{
    ...
    if (ms_spriteAttributes.m_pImage)
    {
        ...
        if (ms_spriteAttributes.m_pImage->CreateImage(ms_pcameraCur->m_sViewW, ms_pcameraCur->m_sViewH, RImage::BMP8) ==
            0)
        {
...
static void MyRFileCallback(long lBytes)
{
    ...
    if ((lNow - ms_lRFileCallbackTime) > MY_RFILE_CALLBACK_INTERVAL)
    {
        ...
        // when necessary.
        rspRect(RSP_BLACK_INDEX, g_pimScreenBuf, ms_sFileOpTextX, ms_sFileOpTextY, ms_sFileOpTextW, ms_sFileOpTextH);

        // Draw text in cleared area.
        ms_printFile.print(g_pimScreenBuf,
                           ms_sFileOpTextX,
                           ms_sFileOpTextY,
        ...
        // Update the display.
        rspUpdateDisplay(ms_sFileOpTextX, ms_sFileOpTextY, ms_sFileOpTextW, ms_sFileOpTextH);
...
////////////////////////////////////////////////////////////////////////////////
inline void SetPosInfo( // Returns nothing.
  ...
  bool bTitle /*= false*/)       // In:  true to update the title info as well.
{
    if (ms_pguiInfo)
    {
        if (bTitle == true)
        {
            ...
            if (ms_pthingSel)
            {
                ms_pguiInfo->SetText("Info for \"%s\" [%u]",
...
// if invalid.
////////////////////////////////////////////////////////////////////////////////
inline void Pos2Str( // Returns nothing.
  ...
  short sTotalItemsToProcess) // In:  Total items to process.
{
    ...
    if (sLastItemProcessed == 0)
    {
        ...
        rspUpdateDisplay(lProgressX - 1, lProgressY - 1, lProgressW + 2, lProgressH + 2);
    }
    ...
    if ((lNow - lLastCallTime) > PROGRESS_CALLBACK_INTERVAL)
    {
        ...
        // Update display.
        rspUpdateDisplay(lProgressX + lLastProgressPos, lProgressY, lNewProgressPos - lLastProgressPos, lProgressH);
        ...
        if (pau8KeyStatus[KEY_ABORT_REALM_OPERATION])
        {
            if (rspMsgBox(RSP_MB_ICN_QUERY | RSP_MB_BUT_YESNO,
                          g_pszAppName,
                          "Are you sure you want to abort this operation?") == RSP_MB_RET_YES)
            {
                bContinue = false;
            }
        }
