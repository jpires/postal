...
class CFire : public CThing
{
  ...
  public:
    ...
    // is touching.
    void MessagesOn(void) { m_bSendMessages = true; }

    // Turns messages off which allows for fire that is just a visual effect
    void MessagesOff(void) { m_bSendMessages = false; }

    inline short IsBurning(void) { return m_eFireAnim != Smoke && m_eFireAnim != SmallSmoke; }

    // Get the time left to live.
    // Check IsBurning() to determine whether this applies to the fire or
    // the smoke.
    long GetTimeLeftToLive(void) { return m_lBurnUntil - m_pRealm->m_time.GetGameTime(); }
