    ...
    if (sResult == 0)
    {
        ...
        switch (ulFileVersion)
        {
            ...
            case 44:
                ...
                // Some extra safety (to avoid crashing)
                if (m_dShadowLength < 0.0)
                    m_dShadowLength = 0.0;
                if (m_dShadowLength > 1.5)
                    m_dShadowLength = 1.5;
    ...
    if (pguiEdit != nullptr)
    {
        ...
        if (pguiBaseName != nullptr)
        {
            ...
            if (DoGui(pguiEdit) == GUI_ID_OK)
            {
                ...
                if (m_dScale3d < 0.2)
                    m_dScale3d = 0.2; // some safety
                if (m_dScale3d > 5.0)
                    m_dScale3d = 5.0; // some safety

                pguiShadowLength->GetText(szShadowLength, sizeof(szShadowLength));

                m_dShadowLength = atof(szShadowLength);
                if (m_dShadowLength < 0.0)
                    m_dShadowLength = 0.2; // some safety
                if (m_dShadowLength > 1.5)
                    m_dShadowLength = 1.5; // some safety
            ...
            for (lIndex = 0; lIndex < MaxLayers; lIndex++)
            {
                ...
                if (m_apspryAlphas[lIndex] != nullptr)
                {
                    ...
                    while (p)
                    {
                        ...
                        pSprite2->m_sLayer = CRealm::LayerAlpha1 +
                                             (short)lIndex * (CRealm::LayerAlpha2 - CRealm::LayerAlpha1);
                ...
                if (m_apspryOpaques[lIndex] != nullptr)
                {
                    ...
                    while (p)
                    {
                        ...
                        pSprite2->m_sPriority = pSprite->m_sZ;
                        pSprite2->m_pImage = pSprite->m_pImage;
                        pSprite2->m_sLayer = CRealm::LayerOpaque1 +
                                             (short)lIndex * (CRealm::LayerOpaque2 - CRealm::LayerOpaque1);
...
#define MAX_SMASHEE_H 72 // 40	once we deal with fat objects
            ...
            m_pRealm->m_smashatorium.Alloc(m_pRealm->GetRealmWidth(),
                                           m_pRealm->GetRealmHeight(),
...
////////////////////////////////////////////////////////////////////////////////
inline short SpryLoadConv( // Returns 0 on success.
  ...
  RImage::Type type)       // In:  Destination type.
{
    ...
    if (rspGetResource(presmgr, pszFileName, ppspry) == 0)
    {
    ...
    if (!m_bResourcesExist)
    {
        ...
        else
        {
            TRACE("GetResources(): Successfully loaded %hd Alpha layer%s and %hd Opaque layer%s.\n",
                  sNumAlphaLayersLoaded,
                  (sNumAlphaLayersLoaded == 1 ? "" : "s"),
                  sNumOpaqueLayersLoaded,
                  (sNumOpaqueLayersLoaded == 1 ? "" : "s"));
        }

        sprintf(szFileName, "%s.mp1", szBasePath);

        // Load new multi layer attribute maps
        if (rspGetResource(&(m_pRealm->m_resmgr), szFileName, &m_pTerrainMap) != 0)
        {
            sResult = -1;
