...
//////////////////////////////////////////////////////////////////////////////
inline short GetMaxHeight(CRealm *pRealm, // In:  Realm in question.
...
//
//////////////////////////////////////////////////////////////////////////////
inline bool GetTerrainAngle( // true, if terrain angle determined.
...
//				fPosZ		-= fRateZ;
    ...
    short sIndex = 0;
    if (GetMaxHeight(pRealm,
                     sX - TERRAIN_CHECK_DIST - TERRAIN_CHECK_GRANULARITY / 2,
                     sZ - TERRAIN_CHECK_DIST - TERRAIN_CHECK_GRANULARITY / 2,
                     TERRAIN_CHECK_GRANULARITY,
                     TERRAIN_CHECK_GRANULARITY) > sY)
        sIndex = LEFT_TOP;
    if (GetMaxHeight(pRealm,
                     sX + TERRAIN_CHECK_DIST + TERRAIN_CHECK_GRANULARITY / 2,
                     sZ - TERRAIN_CHECK_DIST - TERRAIN_CHECK_GRANULARITY / 2,
  ...
  SampleMasterID smid /*= g_smidBulletFire*/) // In:  Use ammo sample.
{
    ...
    while (bHit == false && sRicochets <= sMaxRicochets && bImpact == false)
    {
        ...
        bHit = Fire(sAngleY,      // In:  Angle of launch in degrees (on X/Z plane).
        ...
        {
            // If on screen . . .
            if (*psX > 0 && *psX < pRealm->GetRealmWidth() && *psZ > 0 && *psZ - *psY < pRealm->GetRealmHeight())
            {
#if 0
				// If out of ricochets . . .
  ...
  SampleMasterID smid /*= g_smidBulletFire*/) // In:  Use ammo sample.
{
    ...
    // Start sample.
    PlaySample(smid, SampleMaster::Weapon, DistanceToVolume(sX, sY, sZ, BULLET_SND_HALF_LIFE));
}
  ...
  CRealm *pRealm)           // In:  Realm in which to fire.
{
    ...
    // Start sample.
    PlaySample((GetRand() % 2) ? g_smidRicochet1 : g_smidRicochet2,
               SampleMaster::Weapon,
  ...
  bool bTracer /*= true*/)   // In:  Draw a tracer at random point along path.
{
    ...
    CSmash *psmash;
    if (pRealm->m_smashatorium.QuickCheckClosest(&line, bitsInclude, bitsDontCare, bitsExclude, &psmash) == true)
    {
...
#define TRACER_ACCUMMULATION_STEP_MIN 10
            ...
            {
                sRand += (GetRand() % (TRACER_ACCUMMULATION_STEP_MAX - TRACER_ACCUMMULATION_STEP_MIN)) +
                         TRACER_ACCUMMULATION_STEP_MIN;
            ...
            pRealm->Map3Dto2D(fStartX, fStartY, fStartZ, &(psl2d->m_sX2), &(psl2d->m_sY2));
            pRealm->Map3Dto2D(fStartX + fRateX * sRand2,
...
// (static)
///////////////////////////////////////////////////////////////////////////////
void CBulletFest::UpdateTracerColor(CRealm *prealm) // In:  Calling realm.
{
...
// (static).
///////////////////////////////////////////////////////////////////////////////
short CBulletFest::Preload(CRealm *prealm) // In:  Calling realm.
{
