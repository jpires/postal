...
#define END_OF_TIME                                                                                                    \
    0x7fffffff // Unfortunately, we won't be dead (or,
            ...
            if ((lCurTime >= m_lNextStartTime) || (m_sWhichTime < 0))
            {
                ...
                if (m_sWhichTime == 0 || (m_sWhichTime > 0 && m_bRepeats == true))
                {
                    ...
                    if (m_sUseLooping && m_lNumLoopBacks >= 0)
                    {
                        ...
                        m_lStopLoopingTime = lCurTime + (m_lNumLoopBacks + 1) * (lLoopEndTime - lLoopStartTime) +
                                             lLoopStartTime;
                    }
                    else
                    {
  ...
  RGuiItem *pgui)       // In:  GUI pointer that was pressed.
{
    ...
    if (pguiName)
    {
        ...
        do
        {
            ...
            if (sResult > 0)
            {
                rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK, g_pszAppName, g_pszGenericMustBeRelativePath_s, pszSakpath);
            }
...
{
    Map3Dto2D(m_dX, m_dY, m_dZ, &(prc->sX), &(prc->sY));
