...
static char *ms_apszResNames[] = { "3d/napalmcan.sop",
                                   "3d/napalmcan.mesh",
                                   "3d/napalmcan.tex",
                                   "3d/napalmcan.hot",
                                   "3d/napalmcan.bounds",
        ...
        switch (m_eState)
        {
            ...
            case CWeapon::State_Go:
                ...
                else
                {
                    ...
                    if (dNewY < sHeight && m_dY < sHeight)
                    {
                        ...
                        PlaySample(g_smidNapalmHit,
                                   SampleMaster::Weapon,
                                   DistanceToVolume(m_dX,
                                                    m_dY,
                                                    m_dZ,
                                                    SideEffectSndHalfLife)); // In:  Initial Sound Volume (0 - 255)
                    }
                    else
                        m_dY = dNewY;
                }
            ...
            case CWeapon::State_Slide:
                ...
                if (dDistance > ms_dMinFireInterval)
                {
                    ...
                    {
                        if (pFire->Setup(m_dX - 20 + (GetRand() % 40),
                                         m_dY,
                                         m_dZ - 20 + (GetRand() % 40),
                                         4000 + (GetRand() % 9000),
                                         false,
                                         CFire::LargeFire) != SUCCESS)
                            delete pFire;
                        else
...
short CNapalm::Preload(CRealm *prealm) // In:  Calling realm.
{
    CAnim3D anim;
    RImage *pimage;
