...
void CThing3d::EditRect(RRect *pRect)
{
    if (m_panimCur != NULL)
    {
        if (m_panimCur->m_psops)
        {
            ...
            m_sprite.m_sRadius = sqrt(
              ABS2(apt3dDst[1].x - apt3dDst[0].x, apt3dDst[1].y - apt3dDst[0].y, apt3dDst[1].z - apt3dDst[0].z));

            Map3Dto2D(m_dX + apt3dDst[0].x, m_dY + apt3dDst[0].y, m_dZ + apt3dDst[0].z, &(pRect->sX), &(pRect->sY));
        }
    }
    else
    {
...
// specified time.
////////////////////////////////////////////////////////////////////////////////
inline bool UpdateVelocity( // Returns true if velocity reaches zero because of the
...
// limits.
////////////////////////////////////////////////////////////////////////////////
void CThing3d::UpdateVelocities(double dSeconds,    // Seconds since last update.
  ...
  Shot_Message *pshotmsg) // In:  Message to handle.
{
    if (pshotmsg->sDamage > 0)
    {
        ...
        AddForceVector(pshotmsg->sDamage * DAMAGE2VEL_RATIO, // In:  Magnitude of additional vector.
  ...
  Burn_Message *pburnmsg) // In:  Message to handle.
{
    ...
    if (m_pRealm->m_idbank.GetThingByID((CThing **)&pfire, m_u16IdFire) != 0)
    {
        ...
        if (CThing::ConstructWithID(CThing::CFireID, m_pRealm, (CThing **)&pfire) == 0)
        {
            ...
            pfire->Setup(m_dX,                      // In:  New x coord
  ...
  short *psLightBits)               // Out: Tally of light bits set, if not NULL.
{
    ...
    for (p2d = m_pap2dAttribCheckPoints; p2d->sX != ATTRIB_CHECK_TERMINATOR; p2d++)
    {
        u16CurAttrib = m_pRealm->GetEffectAttribute((short)sX + p2d->sX, (short)sZ + p2d->sZ);
  ...
  short *psLayer)        // Out: Combined layer.
{
    ...
    for (p2d = m_pap2dAttribCheckPoints; p2d->sX != ATTRIB_CHECK_TERMINATOR; p2d++)
    {
        u16CombinedLayer |= m_pRealm->GetLayer((short)sX + p2d->sX, (short)sZ + p2d->sZ);
    }
  ...
  RTransform *ptrans)            // In:  Transform for positioning child.
{
    ...
    {
        DetachChild(&(pthing3d->m_sprite), // In:  Child sprite to detach.
