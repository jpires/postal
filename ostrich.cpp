...
static char *ms_apszStandResNames[] = { "3d/ostrich_stand.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszRunResNames[] = { "3d/ostrich_run.sop",
                                      "3d/ostrich_run.mesh",
                                      "3d/ostrich_run.tex",
                                      "3d/ostrich_run.hot",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszWalkResNames[] = { "3d/ostrich_walk.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszShotResNames[] = { "3d/ostrich_shot.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszBlownupResNames[] = { "3d/ostrich_blownup.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszHideResNames[] = { "3d/ostrich_hide.sop",
...
/// Die Animation files
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszDieResNames[] = { "3d/ostrich_die.sop",
                                      "3d/ostrich_die.mesh",
                                      "3d/ostrich_die.tex",
                                      "3d/ostrich_die.hot",
...
static RP3d ms_apt3dAttribCheck[] = {
    { -6, 0, -6 }, { 0, 0, -6 }, { 6, 0, -6 }, { -6, 0, 6 }, { 0, 0, 6 }, { 6, 0, 6 },
};
            ...
            case State_Burning:
                if (!CCharacter::WhileBurning())
                {
                    if (m_stockpile.m_sHitPoints <= 0)
                    {
                        ...
                        PlaySample(g_smidOstrichDie, SampleMaster::Voices);
                    }
                    else
                    {
                        m_state = State_Panic;
...
void COstrich::OnShotMsg(Shot_Message *pMessage)
{
    ...
    if (m_state != State_BlownUp &&
        //	    m_state != State_Shot &&
        m_state != State_Die && m_state != State_Dead)
    {
        ...
        {
            PlaySample(g_smidOstrichShot, SampleMaster::Voices);
            m_state = State_Shot;
            AlertFlock();
        }
    }
...
void COstrich::OnExplosionMsg(Explosion_Message *pMessage)
{
    if (m_state != State_BlownUp)
    {
        ...
        PlaySample(g_smidOstrichBlownup, SampleMaster::Voices);
        AlertFlock();
    }
}
...
void COstrich::OnBurnMsg(Burn_Message *pMessage)
{
    ...
    if (m_state != State_Burning && m_state != State_BlownUp && m_state != State_Die && m_state != State_Dead)
    {
        ...
        m_panimCur = &m_animRun;
        m_lAnimTime = 0;
        PlaySample(g_smidOstrichBurning, SampleMaster::Voices);
        AlertFlock();
    }
}
...
{
    if (m_state != State_Die && m_state != State_Dead && m_state != State_BlownUp && m_state != State_Shot &&
        m_state != State_Burning && m_state != State_Panic && m_state != State_Hide)
    {
        m_state = State_Panic;
        m_panimCur = &m_animRun;
