...
static SampleMaster::SoundInstance ms_aSoundInstances[NUM_CHANNELS] = {
    0,
};

// Stores the Sound Levels (0-255) for each sound category
static short ms_asCategoryVolumes[SampleMaster::MAX_NUM_SOUND_CATEGORIES] = {
    255,
};

// Stores the Sound Category for each sound channel:
static SampleMaster::SoundCategory ms_aeSoundTypes[NUM_CHANNELS] = {
...
char *SampleMaster::ms_apszSoundCategories[SampleMaster::MAX_NUM_SOUND_CATEGORIES] = {
    g_apszSoundCategories[Unspecified],  g_apszSoundCategories[BackgroundMusic], g_apszSoundCategories[Weapon],
    g_apszSoundCategories[UserFeedBack], g_apszSoundCategories[Destruction],     g_apszSoundCategories[Ambient],
    g_apszSoundCategories[Demon],        g_apszSoundCategories[Voices],          g_apszSoundCategories[Pain],
  ...
  RSnd *psnd)     // This RSnd.
{
    ...
    if (psample != nullptr)
    {
        ...
        // Either release the sample, or purge and
        // release//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
short SetCategoryVolume(SampleMaster::SoundCategory eType, short sVolume /* = SampleMaster::UserMaxVolume*/)
{
    ...
    //
    ms_asCategoryVolumes[eType] =
      sVolume * SampleMaster::ms_asQualityCategoryAdjustors[g_GameSettings.m_eCurSoundQuality][eType] *
      SampleMaster::MaxVolume / (SampleMaster::UserMaxVolume * SampleMaster::UserMaxVolume);
...
//////////////////////////////////////////////////////////////////////////////
short GetCategoryVolume(SampleMaster::SoundCategory eType /*  = SampleMaster::SoundCategory::Unspecified */)
{
    if ((eType < SampleMaster::Unspecified) || (eType >= SampleMaster::MAX_NUM_SOUND_CATEGORIES))
    {
        return -1;
    ...
    // For the sake of using integer math, we don't do these operations in the above order.
    //
    return ms_asCategoryVolumes[eType] * SampleMaster::UserMaxVolume * SampleMaster::UserMaxVolume /
           (SampleMaster::ms_asQualityCategoryAdjustors[g_GameSettings.m_eCurSoundQuality][eType] *
            SampleMaster::MaxVolume);
}
...
//////////////////////////////////////////////////////////////////////////////
short SetInstanceVolume(SampleMaster::SoundInstance si, // make sure it is YOUR sound
...
)
{
    ...
    if (g_GameSettings.m_sVolumeDistance != FALSE)
    {
        ...
        float fDist2 = ABS2(fX - fSoundX, fY - fSoundY, fZ - fSoundZ);

        if (fDist2 < 1.0)
            return 255; // Dead epicenter
  ...
  SampleMasterID id) // Identifier of sample you want played.
{
    if (id.pszId != nullptr && CAN_PLAY_SAMPLE(id))
    {
        ...
        // Get it into memory.
        // If successful . . .
        if (rspGetResource(&g_resmgrSamples, id.pszId, &psample) == 0)
        {
    ...
    if (id.pszId != nullptr && CAN_PLAY_SAMPLE(id))
    {
        ...
        if (rspGetResource(&g_resmgrSamples, id.pszId, &psample) == 0)
        {
            ...
            if (i < NUM_CHANNELS)
            {
                ...
                if (ms_asndChannels[i].Play(psample,
                                            PLAY_BUF_SIZE,
                                            ms_asndChannels[i].m_sChannelVolume,
                                            ms_asndChannels[i].m_sTypeVolume,
                                            lLoopStartTime,
                                            lLoopEndTime) == 0)
                {
