                        ...
                        if (pfileTmp != nullptr)
                        {
                            ...
                            if (m_sErrorStatus == 0)
                            {
                                ...
                                if (remove(FindCorrectFile(m_pszFileName, "w")) == 0)
                                {
                                    ...
                                    // if (rename(FindCorrectFile(acTmpFileName, "w"), FindCorrectFile(m_pszFileName,
                                    // "w")) == 0)
  ...
  char *pszValue)          // Out: Value returned here
{
    ...
    if (Read() == 0)
    {
        ...
        if (m_sErrorStatus == 0)
        {
            ...
            else if (m_sErrorStatus != 0) // Any other error code means a syntax error
                m_sErrorStatus = 1;
        }
        else
        {
  ...
  S8 *s8Value)             // Out: Value returned here
{
    ...
    if (m_sErrorStatus == 0)
    {
        ...
        if (((*pszEndPtr == '\0') || isspace(*pszEndPtr)) && lRes >= S8_MIN && lRes <= S8_MAX)
            *s8Value = (S8)lRes;
        else
        {
            m_sErrorStatus = 1;
  ...
  U16 *u16Value)           // Out: Value returned here
{
    ...
    {
        lRes = strtol(pszValue, &pszEndPtr, 10);
        if (((*pszEndPtr == '\0') || isspace(*pszEndPtr)) && lRes >= U16_MIN && lRes <= U16_MAX)
            *u16Value = (U16)lRes;
        else
        {
            m_sErrorStatus = 1;
  ...
  S32 *s32Value)           // Out: Value returned here
{
    ...
    if (m_sErrorStatus == 0)
    {
        ...
        if (((*pszEndPtr == '\0') || isspace(*pszEndPtr)) && dRes >= S32_MIN && dRes <= S32_MAX)
            *s32Value = (S32)dRes;
        else
        {
            m_sErrorStatus = 1;
  ...
  U32 *u32Value)           // Out: Value returned here
{
    ...
    {
        dRes = strtod(pszValue, &pszEndPtr);
        if (((*pszEndPtr == '\0') || isspace(*pszEndPtr)) && dRes >= U32_MIN && dRes <= U32_MAX)
            *u32Value = (U32)dRes;
        else
        {
            m_sErrorStatus = 1;
