...
#define INDICATOR_POS_Y(sIndex) (ms_asMenuItemPosY[sIndex] - ms_guiIndicator.m_im.m_sHeight / 2)

// Determines the indiator's X position based on the supplied menu item index.
#define INDICATOR_POS_X(sIndex)                                                                                        \
    (ms_asMenuItemPosX[sIndex] - ms_guiIndicator.m_im.m_sWidth - ms_pmenu->menupos.sIndicatorSpacingX)
  ...
  RRect *prc)         // Where to in image.
{
    ...
    if (pgui == &ms_msgbox)
    {
        ...
        rspRect(ms_pmenu->menuback.u32BackColor, // Color to draw with.
                pim,                             // Dst image.
                prc->sX,
                prc->sY,
                prc->sW,
                prc->sH); // Dst rectangle w/i image.
        ...
        if (ms_pmenu->menuflags & MenuBackTiled)
        {
            ...
            // Tile.
            short sX, sY;
            for (sY = prc->sY + sOffsetY; sY < sMaxY; sY += ms_pimBackground->m_sHeight)
            {
                for (sX = prc->sX + sOffsetX; sX < sMaxX; sX += ms_pimBackground->m_sWidth)
                {
                    rspBlit(ms_pimBackground, // Source image.
                            pim,              // Dest image.
                            0,
                            0,  // Source position.
                            sX, // Dest position.
...
//
//////////////////////////////////////////////////////////////////////////////
inline void SetGuiAttributes(RGuiItem *pguiDst, // Gui to set attributes to.
                      ...
                      // sDirVert is set to 1.
{
    ...
    do
    {
        ...
        // Check bounds.
        if ((ms_sCurItem > ms_sNumMenuItems) || (ms_sCurItem == ms_sNumMenuItems && sDirHorz))
        {
        ...
        else if ((ms_sCurItem < -1) || (ms_sCurItem == -1 && sDirHorz))
        {
            if (ms_sItemsPerColumn > 0)
            {
#if 0
                ...
                short sMaxMenuItem = MAX(ms_sNumMenuItems - 1, 0);
                ms_sCurItem = sMaxMenuItem / ms_sItemsPerColumn * ms_sItemsPerColumn +
                              (ms_sItemsPerColumn + ms_sCurItem);
                // If too large . . .
    ...
    } while (ms_pmenu->ami[ms_sCurItem].sEnabled == FALSE && sNumItemsTraversed <= ms_sNumMenuItems);
    ...
    ms_guiIndicator.Move(INDICATOR_POS_X(ms_sCurItem), INDICATOR_POS_Y(ms_sCurItem));
                        ...
                        // calls.
{
    ...
    if (pmenu != NULL)
    {
        ...
        if (ms_pmenu->menuback.pszFile != NULL)
        {
            ...
            if (rspGetResource(ms_presmgr, ms_pmenu->menuback.pszFile, &ms_pimBackground) == 0)
            {
                ...
                if (ms_pimBackground->m_pPalette != NULL)
                {
                    ...
                    // Set palette.
                    rspSetPaletteEntries(sStartIndex,
        ...
        U8 au8Green[256];
        U8 au8Blue[256];
        rspGetPaletteEntries(0,         // Palette entry to start copying to (has no effect on source!)
        ...
        u32TextColor = rspMatchColorRGB(               // Out: Matched index.
        ...
        u32TextShadowColor = rspMatchColorRGB(           // Out: Matched index.
        ...
        ms_txtHeader.m_u32TextColor = rspMatchColorRGB( // Out: Matched index.
        ...
        ms_txtHeader.m_u32TextShadowColor = rspMatchColorRGB( // Out: Matched index.
        ...
        if (ms_sNumMenuItems > 0)
        {
            ...
            while (sIndex < ms_sNumMenuItems && sPosX < sMaxItemX)
            {
                ...
                for (; sIndex < ms_sNumMenuItems; sIndex++)
                {
                    ...
                    ptxt = ms_msgbox.AddText(ms_pmenu->ami[sIndex].pszText,
                                             sPosX,
                                             sPosY,
                                             sIndex,
                                             SHADOW_X_PIXELS,
                    ...
                    if (ptxt != NULL)
                    {
                        ...
                        {
                            // Remember the extent of the furthest item.
                            sMaxItemExtentX = MAX(sMaxItemExtentX, short(ptxt->m_sX + ptxt->m_im.m_sWidth));
                        }
        ...
        if (ms_pmenu->menuindicator.pszFile != NULL)
        {
            ...
            RImage *pimIndicator;
            if (rspGetResource(ms_presmgr, ms_pmenu->menuindicator.pszFile, &pimIndicator) == 0)
            {
                // Successfully loaded image.

                // Allocate indicator such that it can contain the image . . .
                if (ms_guiIndicator.m_im.CreateImage(pimIndicator->m_sWidth, pimIndicator->m_sHeight, RImage::BMP8) ==
                    0)
                {
                    // Blt into indicator.
                    rspBlit(pimIndicator,
        ...
        if (sRes == 0 && ms_pimComposite)
        {
            ...
            if (ms_imPreMenu.CreateImage(ms_msgbox.m_im.m_sWidth, ms_msgbox.m_im.m_sHeight, RImage::BMP8) == 0)
            {
                // We must lock the composite buffer before reading from it.
                rspLockBuffer();

                // Store screen contents.
                rspBlit(ms_pimComposite,          // Src.
                        &ms_imPreMenu,            // Dst.
