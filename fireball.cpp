...
short CFirestream::Preload(CRealm * /*prealm*/) // In:  Calling realm.
{
    return 0;
}
...
void CFireball::Render()
{
    ...
    if (pAnim) // && m_sCurrentAlphaChannel >= 0)
    {
        ...
        m_sprite.m_sAlphaLevel = MIN_ALPHA + MAX_ALPHA -
                                 (MAX_ALPHA * (m_lTimeToLive - m_pRealm->m_time.GetGameTime())) / m_lTotalFlameTime;
        // Keep in range.
...
short CFireball::Preload(CRealm *prealm) // In:  Calling realm.
{
    short sResult;
    ChannelAA *pRes;
