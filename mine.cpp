        ...
        switch (m_eState)
        {
                ...
                if (lThisTime > m_lTimer)
                {
                    ...
                    switch (m_id)
                    {
                        ...
                        case CBouncingBettyMineID:
                            ...
                            PlaySample(g_smidMineSet,        // Sample to play
            ...
            case CWeapon::State_Armed:
                if (m_pRealm->m_smashatorium
                      .QuickCheck(&m_smash, CSmash::Character, CSmash::Good | CSmash::Bad, 0, &pSmashed))
                {
                    switch (m_id)
                    {
                        case CBouncingBettyMineID:
            ...
            case CWeapon::State_Go:
                if (lThisTime > m_lTimer)
                {
                    ...
                    if (m_dVertVel <= 0.0)
                    {
                        ...
                        PlaySample(g_smidBounceExplode,
                                   SampleMaster::Weapon,
                                   DistanceToVolume(m_dX,
                                                    m_dY,
                                                    m_dZ,
                                                    ExplosionSndHalfLife)); // In:  Initial Sound Volume (0 - 255)
  ...
  short sZ)         // In:  New z coord
{
    ...
    PlaySample(g_smidMineBeep,                                // In:  sound to play
        ...
        switch (m_id)
        {
            case CTimedMineID:
                sResult = rspGetResource(&g_resmgrGame,
                                         m_pRealm->Make2dResPath(TIMEDMINE_FILE),
                                         &m_pImage,
                                         RFile::LittleEndian);
                break;
            ...
            case CProximityMineID:
                sResult = rspGetResource(&g_resmgrGame,
                                         m_pRealm->Make2dResPath(PROXIMITYMINE_FILE),
                                         &m_pImage,
                                         RFile::LittleEndian);
                break;
            ...
            case CBouncingBettyMineID:
                sResult = rspGetResource(&g_resmgrGame,
                                         m_pRealm->Make2dResPath(BOUNCINGBETTYMINE_FILE),
                                         &m_pImage,
                                         RFile::LittleEndian);
                break;
            ...
            case CRemoteControlMineID:
                sResult = rspGetResource(&g_resmgrGame,
                                         m_pRealm->Make2dResPath(REMOTEMINE_FILE),
                                         &m_pImage,
                                         RFile::LittleEndian);
                break;
        }
...
short CMine::Preload(CRealm *prealm) // In:  Calling realm.
{
    short sResult;
    RImage *pim;
    ...
    if (sResult == 0)
    {
        ...
        if (sResult == 0)
        {
            rspReleaseResource(&g_resmgrGame, &pim);
            sResult =
              rspGetResource(&g_resmgrGame, prealm->Make2dResPath(BOUNCINGBETTYMINE_FILE), &pim, RFile::LittleEndian);
            if (sResult == 0)
            {
                rspReleaseResource(&g_resmgrGame, &pim);
            }
