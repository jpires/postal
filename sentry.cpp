    ...
    if (!m_sSuspend)
    {
        ...
        switch (m_state)
        {
            ...
            case CSentry::State_Guard:
                ...
                if (bShootThisTime && m_idDude != CIdBank::IdNil && lSqDistanceToDude < m_lSqDistRange &&
                    lThisTime > m_lTimer && m_sNumRounds > 0)
                {
                           ...
                           // EditRect() pos.
{
    ...
    short sX;
    short sY;
    Map3Dto2D(m_dXBase, m_dYBase, m_dZBase, &sX, &sY);
...
void CSentry::OnShotMsg(Shot_Message *pMessage)
{
    ...
    StartAnim(SENTRY_HIT_RES_NAME, dHitX, m_dY + RAND_SWAY(10), dHitZ, false);
