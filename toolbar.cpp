...
class CToolItem
{
  public:
    ...
    void ArrangeWeapon(ToolWeaponType eType,
    ...
    void ArrangeAmmo(ToolAmmoType eType,
                     CDude::WeaponType eStock,
                     const RRect &prImage,
                     short sTextX,
                     short sTextY,
    ...
    static short Init(CHood *pHood) // do color matching & asset loading
    {
        ...
        ms_sSmallFontColor = rspMatchColorRGB(gmcSmallFont.r,
                                              gmcSmallFont.g,
                                              gmcSmallFont.b,
                                              10,
                                              236,
                                              ...
                                              pHood->m_pimBackground->m_pPalette->Blue(),
                                              4);

        ms_sLargeFontColor = rspMatchColorRGB(gmcLargeFont.r,
                                              gmcLargeFont.g,
                                              gmcLargeFont.b,
                                              10,
                                              236,
        ...
        ms_sWarningColor = rspMatchColorRGB(gmcWarningFont.r,
                                            gmcWarningFont.g,
                                            gmcWarningFont.b,
                                            10,
                                            236,
        ...
        ms_sAmmoGoneColor = rspMatchColorRGB(gmcAmmoGoneFont.r,
                                             gmcAmmoGoneFont.g,
                                             gmcAmmoGoneFont.b,
                                             10,
                                             236,
        ...
        ms_sAttentionColor = rspMatchColorRGB(gmcAttentionFont.r,
                                              gmcAttentionFont.g,
                                              gmcAttentionFont.b,
                                              10,
                                              236,
        ...
        //----------------------------- Match Top bar stuff:
        gsStatusFontForeIndex = rspMatchColorRGB(gmcSolidScore.r,
                                                 gmcSolidScore.g,
                                                 gmcSolidScore.b,
                                                 10,
                                                 236,
        ...
        gsStatusFontShadowIndex = rspMatchColorRGB(gmcShadowScore.r,
                                                   gmcShadowScore.g,
                                                   gmcShadowScore.b,
                                                   10,
                                                   236,
        ...
        if (!ms_pimCompositeBuffer)
        {
            ...
            ms_pimCompositeBuffer->CreateImage(pHood->m_pimEmptyBar->m_sWidth,
                                               pHood->m_pimEmptyBar->m_sHeight,
                                               RImage::BMP8);

            ms_pimCompositeBufferScaled = new RImage;
            ms_pimCompositeBufferScaled->CreateImage(wideScreenWidth, // HACK HACK
                                                     pHood->m_pimEmptyBar->m_sHeight,
                                                     RImage::BMP8);
        }
    ...
    static void RenderBar(CHood *pHood, RImage *pimDst, short sDstX, short sDstY)
    {
        ...
        for (i = NotWeapon + 1; i < NumberOfWeapons; i++)
        {
            ...
            {
                case Bar:
                    pimPlane = pHood->m_pimEmptyBar;
                    break;
                case BarSel:
                    pimPlane = pHood->m_pimEmptyBarSelected;
                    break;
                case Full:
                    pimPlane = pHood->m_pimFullBar;
                    break;
                case FullSel:
                    pimPlane = pHood->m_pimFullBarSelected;
                    break;
            }
        ...
        for (i = NotAmmo + 1; i < NumberOfAmmos; i++)
        {
            ...
            if (ms_aAmmo[i].m_dValue <= 0.1)
                sFontColor = ms_sAmmoGoneColor;
            else if (ms_aAmmo[i].m_bTreasure)
                sFontColor = ms_sAttentionColor;
            ...
            {
                ms_pntTool.print(ms_aAmmo[i].m_rText.sX, ms_aAmmo[i].m_rText.sY, "%3ld", long(ms_aAmmo[i].m_dValue));
            }
        }
        ...
        if (ms_pimCompositeBuffer->m_sWidth != ms_pimCompositeBufferScaled->m_sWidth)
        {
            ...
            for (int n = 0; n < ms_pimCompositeBuffer->m_sHeight; n++)
            {
                for (int x = 0; x < ms_pimCompositeBufferScaled->m_sWidth; x++)
                {
                    ...
                    UCHAR *src = ms_pimCompositeBuffer->m_pData + n * ms_pimCompositeBuffer->m_lPitch +
                                 (int)((float)x * widthScale);
                    *dest = *src;
                }
...
class CInitToolBar
{
  public:
    CInitToolBar()
    {
        ...
        //************* WEAPONS **************
        CToolItem::ms_aWeapons[MachineGun].ArrangeWeapon(MachineGun,
                                                         CDude::SemiAutomatic,
                                                         RRect(123, 443, 48, 23),
                                                         &CToolItem::ms_aAmmo[Bullets]);

        CToolItem::ms_aWeapons[ShotGun].ArrangeWeapon(ShotGun,
                                                      CDude::ShotGun,
                                                      RRect(176, 444, 42, 19),
                                                      &CToolItem::ms_aAmmo[Shells]);

        CToolItem::ms_aWeapons[SprayCannon].ArrangeWeapon(SprayCannon,
                                                          CDude::SprayCannon,
                                                          RRect(221, 442, 44, 22),
                                                          &CToolItem::ms_aAmmo[Spray]);

        CToolItem::ms_aWeapons[MissileLauncher].ArrangeWeapon(MissileLauncher,
                                                              CDude::Rocket,
                                                              RRect(306, 442, 54, 21),
                                                              &CToolItem::ms_aAmmo[Rockets],
                                                              &CToolItem::ms_aAmmo[HeatSeekers]); // double whammy

        CToolItem::ms_aWeapons[NapalmLauncher].ArrangeWeapon(NapalmLauncher,
                                                             CDude::Napalm,
                                                             RRect(413, 441, 49, 23),
                                                             &CToolItem::ms_aAmmo[Napalm]);

        CToolItem::ms_aWeapons[FlameThrower].ArrangeWeapon(FlameThrower,
                                                           CDude::FlameThrower,
                                                           RRect(464, 442, 52, 22),
        ...
        CToolItem::ms_aAmmo[Shells].ArrangeAmmo(Shells,
                                                CDude::ShotGun,
                                                RRect(182, 463, 16, 15),
                                                202,
                                                467,
        ...
        CToolItem::ms_aAmmo[Spray].ArrangeAmmo(Spray,
                                               CDude::SprayCannon,
                                               RRect(228, 463, 16, 15),
                                               247,
                                               467,
        ...
        CToolItem::ms_aAmmo[HeatSeekers].ArrangeAmmo(HeatSeekers,
                                                     CDude::Heatseeker,
                                                     RRect(337, 464, 26, 13),
                                                     364,
                                                     467,
        ...
        CToolItem::ms_aAmmo[Fuel].ArrangeAmmo(Fuel,
                                              CDude::FlameThrower,
                                              RRect(476, 465, 14, 13),
                                              494,
                                              466,
                                              ...
                                              CToolItem::Small);
        //----------------------------------------
        CToolItem::ms_aAmmo[ProximityMine].ArrangeAmmo(ProximityMine,
                                                       CDude::ProximityMine,
                                                       RRect(567, 443, 30, 23),
                                                       575,
...
bool ToolBarRender(CHood *pHood, RImage *pimDst, short sDstX, short sDstY, CDude *pDude, bool bForceRender)
{
    bool bRender = true;
    ...
    if (!bForceRender) // unless forced to render
    {
        if ((rspGetMilliseconds() - CToolItem::ms_lLastTime) > TB_MILLI_INTERVAL_TIME)
        {
            ...
            if (bRender)
                CToolItem::RenderBar(pHood, pimDst, sDstX, sDstY);
