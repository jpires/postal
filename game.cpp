    ...
    if (sResult == 0)
    {
        ...
        while (sCorrectCD == -2)
        {
            ...
            if (sCorrectCD != 0)
            {
                ...
                if (rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_RETRYCANCEL,
...
static short GameCore() // Returns 0 on success.
{
    ...
    bool bMPath = false, bMPathServer = false;
...
// If multiplayer is disabled, leave out this code to make it harder to hack back in
#ifndef MULTIPLAYER_DISABLED
                {
                        ...
                        if (DoNetGameDialog(pnetclient, bBrowse, nullptr, &msg) == 0)
                        {
                            ...
                            if (msg.msg.nothing.ucType == NetMsg::START_GAME)
                            {
                                ...
                                Play(pnetclient,                    // Client
                                     ...
                                     msg.msg.startGame.sRealmNum >= 0 ? false
                                                                      : true, // Whether to play just one realm or not
                                     false,                                   // Not challenge mode
                    ...
                    else
                    {
                        ...
                        rspMsgBox(RSP_MB_ICN_STOP | RSP_MB_BUT_OK,
                                  g_pszAppName,
                                  "The selected network protocol has failed to initialize.  Please contact your system "
                                  "administrator or network vendor.\n");
                    }
                }
                ...
                case ACTION_DEMO_PLAYBACK:
                {
                    ...
                    if (InputDemoInit() == 0)
                    {
                        ...
                        if (m_szDemoFile[0] == '\0')
                        {
                            ...
                            {
                                sprintf(m_szDemoFile,
                                        "%s%d%s",
                                        FullPathHD(DEFAULT_DEMO_PREFIX),
                                        usDemoCount % MAX((short)1, g_GameSettings.m_sNumAvailableDemos),
                                        DEFAULT_DEMO_SUFFIX);
                            }
                        }
                           ...
                           // characters here.
{
    ...
    sprintf(szAudioResDescriptor, "%ld%c%ld", lSamplesPerSec, AUDIO_SAK_SEPARATOR_CHAR, lBitsPerSample);
...
static short OpenSaks()
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (sModeIndex = 0; sModeIndex < NUM_ELEMENTS(amodes) && bSakFound == false; sModeIndex++)
            {
                ...
                GetSoundPaths(amodes[sModeIndex].lSamplesPerSec,
                              amodes[sModeIndex].lBitsPerSample,
                              szSamplesSakSubPath,
                              szSamplesNoSakFullPath);
            ...
            if (bSakFound == false)
            {
                rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK, g_pszAppName, g_pszNoSoundFiles);
    ...
    g_GameSettings.m_eCurSoundQuality = (SampleMaster::SoundQuality)(((lSamplesPerSec / 11025) - 1) * 2 +
                                                                     ((lDevBitsPerSample / 8) - 1));
...
//
////////////////////////////////////////////////////////////////////////////////
extern void Game_ControlsMenu(short sMenuItem)
{
...
extern short Game_SavePlayersGame(char *pszSaveName, // In:  Name of the save file
...
extern short Game_LoadPlayersGame(char *pszSaveName,   // In:  Name of the saved game file to open
                                  short *psDifficulty, // Out: Saved game realm difficulty.
                                  ACTION *paction)     // Out: Saved game action.
{
    RFile rf;
  ...
  bool bWriteLogs)         // In:  true to create log, false to compare.
{
    ...
    {
        if (ms_fileSynchLog.Open(g_GameSettings.m_szSynchLogFile, bWriteLogs ? "wb" : "rb", RFile::LittleEndian) == 0)
        {
            // Success.
        }
        else
  ...
  U32 u32User)       // In:  A user value that is intended to be consistent.
{
    ...
    if (ms_fileSynchLog.IsOpen())
    {
        ...
        else
        {
            ...
            if (fscanf(ms_fileSynchLog.m_fs,
            ...
            {
                // Verify . . .
                if ((rspStricmp(szFileIn, GetFileNameFromPath(pszFile)) != 0) || (lLineIn != lLine) ||
                    (exprIn != expr) || (u32UserIn != u32User))
                {
                    char szOut[2048];
                    sprintf(szOut,
                    ...
                    rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK, "Postal", szOut);
...
//
////////////////////////////////////////////////////////////////////////////////
extern void SeedRand(long lSeed)
{
    m_lRandom = lSeed;
}
...
extern long GetRandomDebug(char *FILE_MACRO, long LINE_MACRO)
{
    ...
    if (m_pfileRandom)
    {
        if (m_bWriteLogs)
        {
            fprintf(m_pfileRandom->m_fs, "%s : %ld rand = %ld\n", GetFileNameFromPath(FILE_MACRO), LINE_MACRO, lNewVal);
        ...
        else
        {
            ...
            fscanf(m_pfileRandom->m_fs, "%s : %ld rand = %ld\n", szSavedFile, &lSavedLine, &lSavedVal);
            //				m_pfileRandom->Read(&lSavedVal);
            //				m_pfileRandom->Read(&lSavedLine);
            //				m_pfileRandom->Read(szSavedFile);

            if ((lSavedVal != lNewVal) || (lSavedLine != LINE_MACRO) ||
                (rspStricmp(szSavedFile, GetFileNameFromPath(FILE_MACRO)) != 0))
            {
                rspMsgBox(RSP_MB_ICN_INFO | RSP_MB_BUT_OK,
                          "Postal",
  ...
  short sBase)             // New brighten value.
{
    ...
    // Update map.
    rspSetPaletteMaps(0, 256, au8RedMap, au8GreenMap, au8BlueMap, sizeof(U8));
...
)
{
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        short sLev = short(128.0 +
                           128.0 * ((((((1.0 - dContrast) * dX) - dBrightness) * dX) + dContrast) * dX + dBrightness));
