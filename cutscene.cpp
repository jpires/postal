...
class CCutSceneInfo
{
  public:
    ...
    ~CCutSceneInfo()
    {
        if (m_bDeleteFont && m_pFont)
            delete m_pFont;
        if (m_pimBGLayer)
            delete m_pimBGLayer;
        if (m_pimTextLayer)
            delete m_pimTextLayer;
        if (m_pmaAlpha)
            delete m_pmaAlpha;
...
)
{
    ...
    if (prCenter)
    {
        ...
        rspBlit(pimBackground, &imSwirl, prCenter->sX, prCenter->sY, 0, 0, prCenter->sW, prCenter->sH);
    ...
    while (((lCurrentTime = (rspGetMilliseconds() - lStartTime)) < lMilliLen) && !(rspGetQuitStatus()))
    {
        ...
        rspBlit(&imSwirl,
                ...
                &rClip);
        // Alpha Blit!
        rspAlphaBlitT(128, pAlpha, &imSwirl, g_pimScreenBuf, sStartX - sOffX, sStartY - sOffY, &rClip);
...
class CSwirlMe
{
  public:
    ...
    void Erase()
    {
        m_sCenX = m_sCenY = m_sRadX = m_sRadY = m_lTimeSpin = m_lCycleTimeX = m_lCycleTimeY = m_lCycleTimeA = 0;
        m_dCenA = m_dRadA = 0.0;
    ...
    ////////////////////////////////////////////////////////////////////////////
    void Update() { rspUpdateDisplay(m_rClip.sX, m_rClip.sY, m_rClip.sW, m_rClip.sH); }
                          ...
                          short sBorderY)
{
        ...
        (ms_pCut->m_pimBGLayer->Load(FullPathVD(szText)) == 0))
    {
        ...
        rspSetPaletteEntries(0,
                             256,
                             ms_pCut->m_pimBGLayer->m_pPalette->Red(0),
    ...
    if ((ms_pCut->m_pmaAlpha->Load(FullPathHD(szText)) == 0) || (ms_pCut->m_pmaAlpha->Load(FullPathVD(szText)) == 0))
    {
    }
    else
    {
    ...
    ms_pCut->m_ucForeText =
      rspMatchColorRGB(FONT_FORE_R, FONT_FORE_G, FONT_FORE_B, 10, 236, au8Red, au8Green, au8Blue, sizeof(U8));
    ms_pCut->m_ucShadowText =
      rspMatchColorRGB(FONT_SHAD_R, FONT_SHAD_G, FONT_SHAD_B, 10, 236, au8Red, au8Green, au8Blue, sizeof(U8));
    ...
    if (ms_pCut->m_pimBGLayer)
    {
        ...
        RRect rcBGClipper(0, 0, ms_pCut->m_pimBGLayer->m_sWidth, ms_pCut->m_pimBGLayer->m_sHeight);

        // Copy progress bar area of bg image to dst with SOURCE clipping
        // just in case image is too small.
        rspBlit(ms_pCut->m_pimBGLayer, // Src.
    ...
    rspBlit(ms_pCut->m_pimTextLayer, ms_pCut->m_pimDst, 0, 0, &rCrop);
    ...
    ms_pCut->m_u8BloodColor =
      rspMatchColorRGB(BLOOD_FORE_R, BLOOD_FORE_G, BLOOD_FORE_B, 1, 254, au8Red, au8Green, au8Blue, sizeof(U8));
}
                            ...
                            short sH)
{
    ...
    if (ms_pCut->m_pimBGLayer)
    {
        ...
        rspRect(*(ms_pCut->m_pimBGLayer->m_pData),
...
static void CutScene_RFileCallback(long lBytes)
{
    static short asWavyY[] = { 0, 1, 2,  0,  -2, -1, 1, 0, -1, 0, 1,  2,  1, 0,
                ...
                if ((lNow - ms_pCut->m_lTimeToUpdate) > PROGRESS_BAR_UPDATE_TIME)
                {
                    ...
                    for (short sBaseX = ms_pCut->m_sLastDistance; sBaseX <= sDistance; sBaseX++)
                    {
                        ...
                        for (short i = 0; i < PROGRESS_BAR_DENSITY; i++)
                        {
                            ...
                            rspPlot(rspBlendColor(150, ms_pCut->m_pmaAlpha, ms_pCut->m_u8BloodColor, u8dst),
                                    ms_pCut->m_pimBGLayer,
                                    sX,
                                    sY);
                        }
