...
////////////////////////////////////////////////////////////////////////////////
void CScene::RemoveSprites(short sLayer) // In:  Layer number (0 to n-1)
{
...
////////////////////////////////////////////////////////////////////////////////
void CScene::SetLayers(short sNumLayers) // In:  Number of layers
{
    // Validate parameters
    ASSERT(sNumLayers > 0);
...
////////////////////////////////////////////////////////////////////////////////
void CScene::UpdateSprite(CSprite *pSprite) // In:  Sprite to add
{
...
////////////////////////////////////////////////////////////////////////////////
void CScene::RemoveSprite(CSprite *pSprite) // In:  Sprite to remove
{
    ASSERT(pSprite != nullptr);
...
CScene::Render3D(RImage *pimDst,    // Destination image.
                 ...
                 RRect *prcDstClip) // Dst clip rect.
{
    ...
    if (g_bSceneDontBlit == false)
    {
        ...
        if (sClipLeft < sDiameter && sClipTop < sDiameter && sClipRight < sDiameter && sClipBottom < sDiameter)
        {
            ...
            m_pipeline.Transform(ps3Cur->m_psop, // Sea of 3D points to form
                                                 // mesh around.
                ...
                m_pipeline.m_sCenY + sRadius < SCREEN_DIAMETER_FOR_3D - sRenderOffY)
            {
                ...
                if (g_GameSettings.m_s3dFog != FALSE)
                {
                    ...
                    m_pipeline.Render(pimRender,        // Dst image.
...
////////////////////////////////////////////////////////////////////////////////
inline void DrawLine2d( // Returns nothing.
...
#else
    // Draw.
    rspLine(u8Color, pimDst, sDstX1, sDstY1, sDstX2, sDstY2, prcDstClip);
#endif
}
...
////////////////////////////////////////////////////////////////////////////////
inline void DrawLine3d( // Returns nothing.
  U8 u8Color,           // Color to draw line with.
  RImage *pimDst,       // Destination image.
  short sDstX1,         // Start pt.
  short sDstY1,         // Start pt.
  ...
  RRect *prcDstClip)           // Dst clip rect.
{
    ...
    // Base box.
    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY1, sZ1, sX2, sY1, sZ, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX2, sY1, sZ, sX, sY1, sZ2, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY1, sZ2, sX1, sY1, sZ, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX1, sY1, sZ, sX, sY1, sZ1, phood, prcDstClip);

    // Top box.
    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY2, sZ1, sX2, sY2, sZ, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX2, sY2, sZ, sX, sY2, sZ2, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY2, sZ2, sX1, sY2, sZ, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX1, sY2, sZ, sX, sY2, sZ1, phood, prcDstClip);

    // Height edges.
    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY1, sZ1, sX, sY2, sZ1, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX2, sY1, sZ, sX2, sY2, sZ, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX, sY1, sZ2, sX, sY2, sZ2, phood, prcDstClip);

    DrawLine3d(psc3->m_u8Color, pimDst, sX1, sY1, sZ, sX1, sY2, sZ, phood, prcDstClip);
}
  ...
  CSprite *psprXRayee) // XRayee, if not NULL.
{
    ...
    if (ps2Cur->m_pimAlpha != nullptr && g_GameSettings.m_sAlphaBlend != FALSE)
    {
        ...
        if (ps2Cur->m_sAlphaLevel < 255)
        {
            ...
            rspGeneralAlphaBlit(ps2Cur->m_sAlphaLevel,    // Adaptable alpha level.
        ...
        else
        {
            ...
            rspGeneralAlphaBlit(phood->m_pmaTransparency, // Levels multialpha table.
                                ps2Cur->m_pimAlpha,       // Alpha mask.
                                ps2Cur->m_pImage,         // Src.
                                pimDst,                   // Dst.
                                ps2Cur->m_sX2 + sDstX,    // 2D Dst coord.
    ...
    else
    {
        ...
        if (ps2Cur->m_sAlphaLevel < 255 && g_GameSettings.m_sAlphaBlend != FALSE)
        {
            ...
            rspAlphaBlitT(ps2Cur->m_sAlphaLevel,    // Homogeneous alpha level.
                          phood->m_pmaTransparency, // Levels multialpha table.
                          ps2Cur->m_pImage,         // Src.
                          pimDst,                   // Dst.
                          ps2Cur->m_sX2 + sDstX,    // 2D Dst coord.
      ...
      // that the C++ compares do.
                ...
                if (((ps2Cur->m_sInFlags & CSprite::InAlpha) ||
                     ((ps2Cur->m_sInFlags & CSprite::InOpaque) && m_bXRayAll == true)) &&
                    psprXRayee && (g_GameSettings.m_sXRayEffect != FALSE || m_bXRayAll == true))
#endif
                {
                    ...
                    if (rect2.ClipTo(&rect1) != -1)
                    {
                        ...
                        g_alphaBlit(ps2Cur->m_pImage,         // Source image.
                                    pimDst,                   // Destination image.
        ...
        if (!(pSprite->m_sInFlags & CSprite::InHidden))
        {
            ...
            if (g_bSceneDontBlit == false)
            {
                ...
                if (pSprite->m_pszText != nullptr)
                {
                    ...
                    if (sW > 0 && sH > 0)
                    {
                        ms_print.SetColumn(sX, sY, sW, sH);

                        // Print it.
                        ms_print.print(sX, sY, "%s", pSprite->m_pszText);
                    }
                }
                // ****TEMP****
            }
...
///////////////////////////////////////////////////////////////////////////////
void CScene::Render(short sSrcX,    // In:  Source (scene) x coord
                    ...
                    CHood *phood)   // In:  The hood involved.
{
    ...
    CSprite *psprXRayee = nullptr; // XRayee when not NULL.
                                            ...
                                            // m_dScale3d declaration).
{
    ...
    /////////////////////////////////////////////////////////////////////////////
    // Some extra safety
    if (dScale3d <= c_dMinScale)
        dScale3d = c_dMinScale;
    if (dScale3d > c_dMaxScale)
        dScale3d = c_dMaxScale;
    ...
    // 16-bit z-buffer
    //
    m_pipeline.m_tScreen.Scale(double(sScreenSize) / dModelSize,
                               double(sScreenSize) / dModelSize,
                               REAL(65535.0 / MODEL_ZSPAN)); // Use MODEL_ZSPAN for consistent fog thickness
    // Mimic this in transform that contains no Z scaling.
    m_transNoZScreen.Scale(double(sScreenSize) / dModelSize,
                           double(sScreenSize) / dModelSize,
                           double(sScreenSize) / dModelSize);
  ...
  RRect *prcDstClip /*= NULL*/) // Dst clip rect.
{
    ...
    if (!(ps3->m_sInFlags & CSprite::InHidden))
    {
...
#if 1
        // Render.
        Render3D(pimDst,      // Destination image.
                 sDstX,       // Destination 2D x coord.
                 sDstY,       // Destination 2D y coord.
        ...
        while (psprite)
        {
            ...
            {
                // Render 'em.
                DeadRender3D(pimDst, (CSprite3 *)psprite, phood, sDstX + ps3->m_sX2, sDstY + ps3->m_sY2, prcDstClip);
            }
