    ...
    if (!m_sSuspend)
    {
        ...
        switch (m_eState)
        {
            ...
            case CWeapon::State_Chase:
            {
                ...
                if (m_dZ > ms_sOffScreenDist + m_pRealm->GetRealmHeight() || m_dZ < -ms_sOffScreenDist ||
                    m_dX > ms_sOffScreenDist + m_pRealm->GetRealmWidth() || m_dX < -ms_sOffScreenDist)
                {
                    // Blow Up
...
short CDeathWad::Preload(CRealm *prealm) // In:  Calling realm.
{
    CAnim3D anim;
    RImage *pimage;
  ...
  double *pdCurZ)             // Out: Position of inside terrain status change.
{
    ...
    while (dDistance > 0 && *pbInTerrain == bInitiallyInTerrain)
    {
...
////////////////////////////////////////////////////////////////////////////////
void CDeathWad::FeedWad(CStockPile *pstockpile) // In:  Src for WAD's arsenal.
{
    // Take needed ammo.
