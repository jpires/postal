...
static char *ms_apszStandResNames[] = { "3d/bandg_stand.sop",
                                        "3d/bandg_stand.mesh",
                                        "3d/bandg_stand.tex",
                                        "3d/bandg_stand.hot",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszRunResNames[] = { "3d/bandg_run.sop",
                                      "3d/bandg_run.mesh",
                                      "3d/bandg_run.tex",
                                      "3d/bandg_run.hot",
                                      "3d/bandg_run.bounds",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszMarchResNames[] = { "3d/bandg_march.sop",
                                        "3d/bandg_march.mesh",
                                        "3d/bandg_march.tex",
                                        "3d/bandg_march.hot",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszShotResNames[] = { "3d/bandg_shot.sop",
                                       "3d/bandg_shot.mesh",
                                       "3d/bandg_shot.tex",
                                       "3d/bandg_shot.hot",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszBlownupResNames[] = { "3d/bandg_blownup.sop",
...
// An array of pointers to resource names (one for each channel of the animation)
static char *ms_apszOnFireResNames[] = { "3d/bandg_onfire.sop",
    ...
    if (!m_sSuspend)
    {
        ...
        switch (m_state)
        {
            ...
            case State_March:
                ...
                if (ms_idBandLeader == GetInstanceID())
                {
                    ...
                    if (ms_siBandSongInstance != 0)
                    {
                        ...
                        SetInstanceVolume(ms_siBandSongInstance,
            ...
            case State_Mingle:
                ...
                if (bNoWalk == true || (sHeight - dNewY > 10)) // && m_bAboveTerrain == false && m_dExtHorzVel == 0.0))
                {
                    m_ucDestBouyID = SelectRandomBouy();
                    m_ucNextBouyID = m_pNavNet->FindNearestBouy(m_dX, m_dZ);
                    m_sNextX = m_pNextBouy->GetX();
...
{
    if (m_state != State_BlownUp && m_state != State_Shot && m_state != State_Die && m_state != State_Dead)
    {
...
void CBand::OnPanicMsg(Panic_Message *pMessage)
{
    if (m_state != State_Die && m_state != State_Dead && m_state != State_BlownUp && m_state != State_Shot &&
        m_state != State_Burning && m_state != State_Panic)
    {
        m_state = State_Panic;
        m_panimCur = &m_animOnFire;
