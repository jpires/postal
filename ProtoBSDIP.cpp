...
// Create socket
                ...
                if (m_sock != INVALID_SOCKET)
                {
                    ...
                    if (sOptionFlags & RSocket::optDontBlock)
                    {
                        ...
                        {
                            sResult = -1;
                            TRACE("RProtocolBSDIP::Open(): Error setting non-blocking mode, error returned by "
                                  "ioctlsocket(): %ld\n",
                                  WSAGetLastError());
                        }
                    }
                    ...
                    if ((sOptionFlags & RSocket::optDontCoalesce) && !sResult)
                    {
                        ...
                        {
                            int optval = 1;
                            if (setsockopt(m_sock, IPPROTO_TCP, TCP_NODELAY, (char *)&optval, sizeof(optval)) ==
                                SOCKET_ERROR)
                            {
                                sResult = -1;
                                TRACE("RProtocolBSDIP::Open(): Error setting TCP_NODELAY, error returned by "
                                      "setsockopt(): %ld\n",
                                      WSAGetLastError());
                            }
                        }
                    ...
                    if ((sOptionFlags & RSocket::optDontWaitOnClose) && !sResult)
                    {
                        ...
                        {
                            int optval = 1;
                            if (setsockopt(m_sock, SOL_SOCKET, SO_DONTLINGER, (char *)&optval, sizeof(optval)) ==
                                SOCKET_ERROR)
                            {
                                sResult = -1;
                                TRACE("RProtocolBSDIP::Open(): Error setting SO_DONTLINGER, error returned by "
                                      "setsockopt(): %ld\n",
                                      WSAGetLastError());
                            }
                        }
    ...
    if (m_sock != INVALID_SOCKET)
    {
        if (m_bListening)
        {
            ...
            if (((RProtocolBSDIP *)pProtocolClient)->m_sock == INVALID_SOCKET)
            {
                ...
                ((RProtocolBSDIP *)pProtocolClient)->m_sock = accept(m_sock,
                                                                     (SOCKADDR *)&(addressClient.address),
                                                                     (int *)(&addressClient.lAddressLen));
                if (((RProtocolBSDIP *)pProtocolClient)->m_sock != INVALID_SOCKET)
                {
  ...
  RSocket::Address *paddress) // In:  Address to send to
{
    ...
    if (m_sock != INVALID_SOCKET)
    {
        if (!m_bListening)
        {
            ...
            ms_funcnum = RSocket::OtherFunc;
            *plActualBytes =
              sendto(m_sock, (char *)pBuf, lNumBytes, 0, (SOCKADDR *)&(paddress->address), paddress->lAddressLen);
            if (*plActualBytes == SOCKET_ERROR)
            {
                *plActualBytes = 0;
                int iErr = WSAGetLastError();
  ...
  RSocket::Address *paddress)      // Out: Source address returned here
{
    ...
    if (m_sock != INVALID_SOCKET)
    {
        if (!m_bListening)
        {
            ...
            ms_funcnum = RSocket::OtherFunc;
            *plActualBytes =
              recvfrom(m_sock, (char *)pBuf, lMaxBytes, 0, (SOCKADDR *)&(addr.address), (int *)(&addr.lAddressLen));
            if (*plActualBytes != SOCKET_ERROR)
            {
...
// SetCallback
//////////////////////////////////////////////////////////////////////////////
void RProtocolBSDIP::SetCallback(RSocket::BLOCK_CALLBACK callback)
{
    m_callback = callback;
}
...
////////////////////////////////////////////////////////////////////////////////
// static
void RProtocolBSDIP::CreateBroadcastAddress(unsigned short usPort,      // In:  Port to broadcast to
...
//////////////////////////////////////////////////////////////////////////////
/* static */
void RProtocolBSDIP::SetAddressPort(USHORT usPort,              // In:  New port number
